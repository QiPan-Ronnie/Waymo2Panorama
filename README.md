...
**Status (2026-06-09, post first-principles audit)**: **read `agent/2026-06-09-fable5-firstprinciples-analysis.md` first.** The audit found the ERP virtual centre has been pinned to the **ego origin** (1.8–2.2 m from the cameras) since L1 — every depth-aware "seam wall" number (incl. DB-79's curb/wall 55–88 px) inherited that ~5–20× amplification; the ring-camera **centroid** is only 0.27–0.30 m away (in-sector effective baseline 0.01–0.06 m). **Active brief DB-80**: re-run the DB-79 render-back battery at c\*=centroid (pre-registered: curb/wall ≤15 px = model confirmed; >30 px = wall finally real). DB-79's measurements stand; its practical conclusion is re-scoped. Downstream (Xinhan/Cosmos) trains on perfect-360s masked to our stitched shape → target = "stitched band ≈ slice of a perfect 360 + honest holes + no fabricated geometry" (collapses the A/B fork). Safest visual base remains L1 `hard_select` + DB-78 flow view-interp + sky-only outpaint. *(The 2026-05-27 sections below are historical.)*
