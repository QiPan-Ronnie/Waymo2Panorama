"""
新-C IPM multi-region video driver — Pi3 + ground+sky+building region
segmentation + IPM-style projection per region, per frame -> mp4.

Per frame:
  1. Pi3 forward on 7 letterboxed cams -> local_points + conf
  2. For each cam: ipm_project_multi_region (calls segment_regions_from_pi3 +
     ipm_project_ground + ipm_project_sky + optional ipm_project_building +
     sphere fallback, returns merged_rgb + merged_weight)
  3. multiband_blend(merged_slabs, merged_weights) -> ERP frame
  4. Append to mp4

GPU required (Pi3). Per frame ~4-6s. Building branch ablated (off by default
matching production deliverable).

Usage on Colab A100:
    python scripts/run_ipm_multi_region_video.py \
        --log-dir /content/drive/MyDrive/.../02a00399-... \
        --pi3-repo /content/Pi3 \
        --out-dir /content/drive/MyDrive/.../outputs/ipm_multi_region_video \
...
def main() -> int:
    ...
    ap.add_argument("--enable-building", action="store_true",
                    help="enable building RANSAC branch (default off, matches production)")
    ...
    from waymo2panorama.projection.ipm_multi_region import ipm_project_multi_region
    from waymo2panorama.blending.multiband import multiband_blend

    log_id = args.log_dir.name
    out_dir = args.out_dir / log_id
    out_dir.mkdir(parents=True, exist_ok=True)
    mp4_path = out_dir / "ipm_multi_region_video.mp4"
    done_marker = out_dir / "done.json"

    print("[mr-video] Loading Pi3X...", flush=True)
    t_load = time.time()
    ...
    print(f"[mr-video] Pi3 loaded in {time.time() - t_load:.1f}s, device={device}, bf16={use_bf16}", flush=True)
    ...
    print(f"[mr-video] Rendering {n_frames} frames", flush=True)
    ...
    for i, idx in enumerate(indices):
        ...
        local_points = res["local_points"][0].detach().float().cpu().numpy()
        ...
        for c_i, cam in enumerate(cams):
            slab = ipm_project_multi_region(
                image=imgs_lb[cam], K=K_lb[cam], T_ego_cam=T_cam[cam],
                local_points_cam=local_points[c_i], conf=conf[c_i],
                erp_hw=erp_hw,
                enable_building=args.enable_building,
                enable_sky_routing=True,
            )
            merged_slabs.append(slab.merged_rgb)
            merged_weights.append(slab.merged_weight)
