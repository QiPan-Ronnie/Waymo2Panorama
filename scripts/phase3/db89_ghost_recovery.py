...
GAIN_STRENGTH = 0.5   # DB-184b (SHIPPED 2026-07-30, 3-frame A/B on 00a6ffc1 a095/a099/a100, band mode): solve_gains_for is a ONE-scalar-per-camera-per-channel, zero-mean-anchored least squares over LiDAR co-visible points — and it OVERSHOOTS by ~2x, because the pairwise log-median difference of a co-visible surface is not purely an exposure difference (view-dependent BRDF + vignetting + flare leak into it). Measured, two axes: seam |dRGB| 14.64 -> 13.81 AND tonal deviation from what the sensor actually recorded 12.5% -> 6.1% — a Pareto win, both axes better at 0.5. side_right territory mean RGB [59.6,58.4,62.6] R/B=0.953 (full gain, user-reported "dim and purple") -> [73.2,70.3,73.0] R/B=1.003 at 0.5, vs the raw camera [76.9,74.2,75.6] R/B=1.017. NOTE clipping is NOT equivalent: np.clip(gains,-0.2,0.2) scored 15.38 (worse than doing nothing) because truncating some cameras and not others breaks the RELATIVE relation; only an isotropic scale preserves it. 1.0 = pre-DB-184b behaviour. Validated in band mode (GROUND_MODE="off"); fill/worldbev paths inherit the same gains and were not separately eyeballed.
