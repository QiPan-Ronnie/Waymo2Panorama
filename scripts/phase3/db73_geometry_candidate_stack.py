...
OUT_DIR = ROOT / "deliverables" / "layered_target_raycaster" / "db73_geometry_candidate_stack"
OUT_DIR = OUT_DIR / "phase1b_sparse_geometry_gate"
REMOTE_OUT = "/content/drive/MyDrive/koi_waymo2pano_colab/results/layered_target_raycaster/db73_geometry_candidate_stack/phase1b_sparse_geometry_gate"
REMOTE_RESULT = REMOTE_OUT + "/DB73_remote_result.json"
REMOTE_SUMMARY = REMOTE_OUT + "/DB73_batch_summary.json"

LOCAL_REMOTE_RESULT = OUT_DIR / "DB73_remote_result.json"
LOCAL_SUMMARY = OUT_DIR / "DB73_batch_summary.json"
MANIFEST = OUT_DIR / "DB73_manifest.json"
BOARD = OUT_DIR / "DB73_full_review_board.jpg"
ROI_SHEET = OUT_DIR / "DB73_same_roi_comparison_sheet.jpg"
FETCH_DIR = OUT_DIR / "fetch"

FETCH_ITEMS = {
    "summary": ("DB73_batch_summary.json", 16),
    "remote_result": ("DB73_remote_result.json", 16),
    "board": ("DB73_full_review_board.jpg", 60),
    "roi_sheet": ("DB73_same_roi_comparison_sheet.jpg", 60),
    ...
    "bmw_geometry_operator": ("02a00399_a000_bmw_geometry_operator_id_map.png", 16),
    "bmw_geometry_eligible": ("02a00399_a000_bmw_geometry_eligible_mask.png", 16),
    "bmw_geometry_depth": ("02a00399_a000_bmw_geometry_plane_depth_map.png", 16),
    ...
    "clean_geometry_operator": ("0bae3b5e_a030_clean_far_geometry_operator_id_map.png", 16),
    "clean_geometry_eligible": ("0bae3b5e_a030_clean_far_geometry_eligible_mask.png", 16),
}
...
def build_local_board(summary: dict[str, Any], status: dict[str, Any]) -> None:
    ...
    draw = ImageDraw.Draw(board)
    draw.text((30, 24), "DB73 global geometry-candidate stack", fill=(242, 242, 242), font=font(27))
    ...
    lines = [
        ...
        "Scope: raw 7-camera ERP slabs + geometry-label optimizer only; no warp/IPM/inpaint/generation/model.",
    ]
    y = 66
    for line in lines:
    ...
    panels = [
        ...
        ("BMW DB73 geometry-candidate RGB", FETCH_DIR / "02a00399_a000_bmw_optimized_source_owned_rgb.png"),
        ...
        ("Clean DB73 geometry-candidate RGB", FETCH_DIR / "0bae3b5e_a030_clean_far_optimized_source_owned_rgb.png"),
    ]
    ...
    board.save(OUT_DIR / "DB73_local_review_board.jpg", quality=92)
...
OUT = {
    "db": "DB-73",
    "phase": "phase0_phase1_source_derived_geometry_candidate_stack",
    ...
    "scope": {
        ...
        "source_derived_geometry_labels": True,
        "abstain_label": True,
        "raw_camera_sourced_rgb_only": True,
        "source_derived_ground_plane_backprojection_used": True,
        "geometry_labels": "raw cam0..6 plus ground-plane cam0..6 plus abstain",
        "flow_warp": False,
...
def optimize_labels(slabs, weights, label0, valid, comps, cfeat, lidar_label, lidar_valid, variant):
    ...
    for idx, (x0, y0, x1, y1) in enumerate(boxes):
        ...
        for sid in range(k):
            if sid >= GEOM_OFFSET and not bool(variant.get("allow_geometry", False)):
                continue
            sv = cfeat["valid_stack"][sid][sl]
            frac = float(sv[vm].mean()) if bool(vm.any()) else 0.0
            min_frac = float(variant.get("geom_min_frac", 0.18)) if sid >= GEOM_OFFSET else float(variant.get("raw_min_frac", 0.18))
            if frac < min_frac:
                continue
            ...
            if sid >= GEOM_OFFSET:
                base += float(variant.get("geometry_w", 0.30))
...
def build_ground_geometry_candidates(log_dir, anchor_ts, frame, cam_names, images, Ks, Ts, raw_slabs, raw_weights, label0, valid, base):
    import cv2
    import numpy as np
    ...
    info = {
        "status": "not_started",
        "candidate_family": "ground_plane_source_derived",
    ...
    try:
        ...
        if ground is None:
            info["status"] = "ground_plane_fit_failed"
        ...
        for cam_i, cam_name in enumerate(cam_names):
            calib = frame.calibrations[cam_name]
            grgb, _ga, gw = render_camera_to_erp(
                image=images[cam_i],
                K=calib.K,
            ...
            gw[~eligible] = 0.0
            grgb = grgb.astype(np.uint8)
            grgb[~eligible] = 0
            g_slabs.append(grgb)
            g_weights.append(gw)
        info.update({
            "status": "ground_plane_candidates_complete",
        ...
        info["status"] = "ground_plane_candidates_blocked"
...
def load_case(case_spec, workdir):
    ...
    g_slabs, g_weights, geom_eligible, tall_veto, plane_depth, geom_info = build_ground_geometry_candidates(
        log_dir, anchor_ts, frame, cam_names, images, Ks, Ts, slabs, weights, label0, valid, base
    )
    all_slabs = slabs + g_slabs
    all_weights = weights + g_weights
    geometry = {
        "eligible": geom_eligible,
        "tall_veto": tall_veto,
        "plane_depth": plane_depth,
        "info": geom_info,
    }
    return run_name, all_slabs, all_weights, valid, label0, base, lidar, geometry
...
def build_case_boards(run_name, base, best_rgb, label0, best_label, boundary0, best_boundary, changed, abstain, comps, cfeat, summary, comp_stats):
    ...
    geom_selected = (best_label >= GEOM_OFFSET) & (best_label < ABSTAIN)
    geom_overlay = overlay(best_rgb, geom_selected, (40, 255, 110), 0.70)
    risk_rgb = np.dstack([
    ...
    save_rgb(REMOTE_OUT / f"{run_name}_risk_components_board.jpg", stack_grid([
        ...
        ("geometry selected", geom_overlay),
    ...
    save_rgb(REMOTE_OUT / f"{run_name}_route_cost_components_board.jpg", stack_grid([
        ...
        ("DB73 optimized geometry-candidate", draw_rois(best_rgb) if run_name.startswith("02a00399") else best_rgb),
        ...
        ("geometry-selected overlay", draw_rois(geom_overlay) if run_name.startswith("02a00399") else geom_overlay),
...
def roi_sheet_for_bmw(base, best_rgb, label0, best_label, changed, abstain, boundary0, best_boundary):
    ...
    draw.text((30, 24), "DB73 BMW marked ROI sheet", fill=(240, 240, 240))
    for r, (name, roi) in enumerate(MARKED_ROIS.items()):
        ...
        crops = [
            ...
            ("DB73 geometry-candidate", best_rgb[y0:y1, x0:x1]),
    ...
    sheet.save(REMOTE_OUT / "DB73_same_roi_comparison_sheet.jpg", quality=92)


def run_one_case(case_spec, workdir):
    import numpy as np
    t0 = time.time()
    run_name, slabs, weights, valid, label0, base, lidar, geometry = load_case(case_spec, workdir)
    ...
    save_u8(REMOTE_OUT / f"{run_name}_geometry_operator_id_map.png", operator_map * 255)
    save_u8(REMOTE_OUT / f"{run_name}_geometry_eligible_mask.png", geometry["eligible"].astype(np.uint8) * 255)
    save_u8(REMOTE_OUT / f"{run_name}_geometry_tall_veto_mask.png", geometry["tall_veto"].astype(np.uint8) * 255)
    save_u8(REMOTE_OUT / f"{run_name}_geometry_plane_depth_map.png", normalize_u8(geometry["plane_depth"], valid))
    ...
    inventory = {
        ...
        "geometry_candidate_valid_fraction": {str(i): float(cfeat["valid_stack"][GEOM_OFFSET + i].mean()) for i in range(max(0, len(slabs) - GEOM_OFFSET))},
        "geometry_candidate_status": geometry["info"],
        "geometry_selected_fraction": float(((best_label >= GEOM_OFFSET) & (best_label < ABSTAIN) & valid).mean()),
    ...
    claim = {
        ...
        "selected_pixels_from_raw_rotation_or_ground_plane_slabs": True,
        "geometry_pixels_raw_camera_backprojected_on_conservative_ground_plane": True,
        "geometry_operator_selected_fraction": float(((best_label >= GEOM_OFFSET) & (best_label < ABSTAIN) & valid).mean()),
        "abstain_pixels_kept_as_hard_select_for_visualization": True,
        "generated_mask": 0,
        "warp_vector_map": "absent",
        "operator_map": "0=raw_source_selection, 1=source_derived_ground_plane_candidate, abstain marked separately",
        "sensor_truth_claim": "source-derived geometry candidate pending vision; not Bosch/source-faithful repair unless sidecars and vision pass",
    }
...
def build_full_board(summary):
    ...
    draw = ImageDraw.Draw(board)
    draw.text((30, 24), "DB73 - full ERP source-derived geometry candidate stack", fill=(240, 240, 240))
    lines = [
        "RGB candidates are raw-camera sourced: rotation slabs plus conservative ground-plane backprojection labels. No inpaint/generation/model/DB32 edit.",
        f"status={summary.get('status')} classification={summary.get('claim_classification')}",
    ...
    panels = [
        ...
        ("BMW DB73 geometry-candidate", REMOTE_OUT / "02a00399_a000_bmw_optimized_source_owned_rgb.png"),
        ...
        ("clean DB73 geometry-candidate", REMOTE_OUT / "0bae3b5e_a030_clean_far_optimized_source_owned_rgb.png"),
    ...
    board.save(REMOTE_OUT / "DB73_full_review_board.jpg", quality=92)
...
try:
    ...
    classification = "source-derived geometry candidate pending vision" if bmw_ok_metric and not clean_degraded else "diagnostic/source-derived geometry candidate or rejected pending vision"
    summary = {
        "status": "DB73_phase0_phase1_complete",
        "claim_classification": classification,
        "source_faithful_repair_claim_allowed": False,
        "phase2_operator_eligibility_allowed": False,
        "geometry_candidate_result_requires_vision": True,
    ...
    (REMOTE_OUT / "DB73_batch_summary.json").write_text(json.dumps(json_safe(summary), indent=2), encoding="utf-8")
    OUT["status"] = "DB73_phase0_phase1_completed"
    OUT["summary"] = summary
except Exception as exc:
    OUT["status"] = "DB73_phase0_phase1_failed_or_blocked"
...
def remote_bash() -> str:
    ...
    return (
        ...
        "exec(compile(code, '<db73_geometry_candidate_stack_remote>', 'exec'))\n"
        "PY"
    )


def parse_json_from_log(log_tail: str) -> dict[str, Any] | None:
    if "DB73_JSON_BEGIN" not in log_tail or "DB73_JSON_END" not in log_tail:
        return None
    body = log_tail.split("DB73_JSON_BEGIN", 1)[1].split("DB73_JSON_END", 1)[0].strip()
...
def run_remote(timeout_s: int) -> dict[str, Any]:
    ...
    run_ok = bool(remote_result.get("status") == "DB73_phase0_phase1_completed")
    complete_outputs = bool((summary or {}).get("status") == "DB73_phase0_phase1_complete")
    manifest: dict[str, Any] = {
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "status": "db73_geometry_candidate_stack",
        "scope": {
        ...
        "drive_output_location": "results/layered_target_raycaster/db73_geometry_candidate_stack/phase1b_sparse_geometry_gate/",
        "decision": {
            "run_ok": run_ok,
        ...
        "claim_boundary": [
            "DB73 Phase0/Phase1 is a source-derived geometry candidate stack plus global label optimization.",
            "RGB candidate pixels are sourced from raw cameras either by rotation-only slabs or conservative ground-plane backprojection labels.",
            ...
            "Vision review decides whether this is a source-derived geometry candidate, diagnostic/operator-eligibility evidence, presentation-only, or rejected.",
        ],
    }
    ...
    manifest["outputs"] = {
        ...
        "local_board": rel(OUT_DIR / "DB73_local_review_board.jpg"),
    }
    MANIFEST.write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
    return {
...
def main() -> None:
    ...
    if not args.run_remote:
        print(json.dumps({"status": "ready", "message": "Use --run-remote after DB73 brief is active."}, indent=2))
        return
