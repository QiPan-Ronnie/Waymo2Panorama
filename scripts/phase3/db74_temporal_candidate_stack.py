...
OUT_DIR = ROOT / "deliverables" / "layered_target_raycaster" / "db74_temporal_candidate_stack"
REMOTE_OUT = "/content/drive/MyDrive/koi_waymo2pano_colab/results/layered_target_raycaster/db74_temporal_candidate_stack"
REMOTE_RESULT = REMOTE_OUT + "/DB74_remote_result.json"
REMOTE_SUMMARY = REMOTE_OUT + "/DB74_batch_summary.json"

LOCAL_REMOTE_RESULT = OUT_DIR / "DB74_remote_result.json"
LOCAL_SUMMARY = OUT_DIR / "DB74_batch_summary.json"
MANIFEST = OUT_DIR / "DB74_manifest.json"
BOARD = OUT_DIR / "DB74_full_review_board.jpg"
ROI_SHEET = OUT_DIR / "DB74_same_roi_comparison_sheet.jpg"
FETCH_DIR = OUT_DIR / "fetch"

FETCH_ITEMS = {
    "summary": ("DB74_batch_summary.json", 16),
    "remote_result": ("DB74_remote_result.json", 16),
    "board": ("DB74_full_review_board.jpg", 60),
    "roi_sheet": ("DB74_same_roi_comparison_sheet.jpg", 60),
    ...
    "bmw_temporal_operator": ("02a00399_a000_bmw_temporal_operator_id_map.png", 16),
    "bmw_temporal_eligible": ("02a00399_a000_bmw_temporal_eligible_mask.png", 16),
    "bmw_temporal_depth": ("02a00399_a000_bmw_temporal_ground_depth_map.png", 16),
    ...
    "clean_temporal_operator": ("0bae3b5e_a030_clean_far_temporal_operator_id_map.png", 16),
    "clean_temporal_eligible": ("0bae3b5e_a030_clean_far_temporal_eligible_mask.png", 16),
}
...
def build_local_board(summary: dict[str, Any], status: dict[str, Any]) -> None:
    ...
    draw = ImageDraw.Draw(board)
    draw.text((30, 24), "DB74 global temporal-candidate stack", fill=(242, 242, 242), font=font(27))
    ...
    lines = [
        ...
        "Scope: raw 7-camera ERP slabs + temporal raw-source labels; no warp/IPM/inpaint/generation/model.",
    ]
    y = 66
    for line in lines:
    ...
    panels = [
        ...
        ("BMW DB74 temporal-candidate RGB", FETCH_DIR / "02a00399_a000_bmw_optimized_source_owned_rgb.png"),
        ...
        ("Clean DB74 temporal-candidate RGB", FETCH_DIR / "0bae3b5e_a030_clean_far_optimized_source_owned_rgb.png"),
    ]
    ...
    board.save(OUT_DIR / "DB74_local_review_board.jpg", quality=92)
...
RAW_K = 7
TEMP_OFFSET = 7
GEOM_OFFSET = TEMP_OFFSET
ABSTAIN = 14
TEMPORAL_OFFSETS = [-4, -2, 2, 4]
CASES = [
...
VARIANTS = [
    ...
    {
        "id": "temporal_probe_t8",
        "allow_temporal": True,
        "temporal_min_frac": 0.005,
        "temporal_w": 0.06,
        "tile": 8,
        "weight_w": 0.26,
        "photo_w": 0.62,
        ...
        "risk_w": 0.42,
        "abstain_base": 2.80,
        "abstain_risk_drop": 0.60,
        "iters": 6,
    },
    {
        "id": "temporal_conservative_t16",
        "allow_temporal": True,
        "temporal_min_frac": 0.015,
        "temporal_w": 0.22,
        "tile": 16,
        "weight_w": 0.34,
        "photo_w": 0.80,
    ...
    {
        "id": "temporal_smooth_t16",
        "allow_temporal": True,
        "temporal_min_frac": 0.010,
        "temporal_w": 0.28,
        "tile": 16,
        "weight_w": 0.32,
        "photo_w": 0.75,
    ...
    {
        "id": "temporal_strict_t16",
        "allow_temporal": True,
        "temporal_min_frac": 0.020,
        "temporal_w": 0.38,
        "tile": 16,
        "weight_w": 0.42,
        "photo_w": 0.70,
...
OUT = {
    "db": "DB-74",
    "phase": "phase0_phase1_temporal_multiframe_candidate_stack",
    ...
    "scope": {
        ...
        "temporal_multiframe_source_labels": True,
        "abstain_label": True,
        "raw_camera_sourced_rgb_only": True,
        "source_derived_temporal_ground_backprojection_used": True,
        "temporal_offsets": TEMPORAL_OFFSETS,
        "temporal_labels": "raw cam0..6 plus temporal-ground cam0..6 plus abstain",
        "flow_warp": False,
...
def compose(slabs, label, valid, label0=None):
    ...
    idx = np.clip(src, 0, len(slabs) - 1)[None, ..., None]
...
def optimize_labels(slabs, weights, label0, valid, comps, cfeat, lidar_label, lidar_valid, variant):
    ...
    for idx, (x0, y0, x1, y1) in enumerate(boxes):
        ...
        for sid in range(k):
            if sid >= GEOM_OFFSET and not bool(variant.get("allow_temporal", variant.get("allow_geometry", False))):
                continue
            sv = cfeat["valid_stack"][sid][sl]
            frac = float(sv[vm].mean()) if bool(vm.any()) else 0.0
            min_frac = float(variant.get("temporal_min_frac", variant.get("geom_min_frac", 0.18))) if sid >= GEOM_OFFSET else float(variant.get("raw_min_frac", 0.18))
            if frac < min_frac:
                continue
            ...
            if sid >= GEOM_OFFSET:
                base += float(variant.get("temporal_w", variant.get("geometry_w", 0.30)))
                base += 0.45 * risk_tile[idx]
...
def lidar_tall_mask_from_ground(pts, ground):
    import cv2
    import numpy as np
    if not ground:
...
def build_temporal_candidates(log_dir, loader, anchor_timestamps, anchor_idx, frame, cam_names, raw_weights, label0, valid, base):
    import cv2
    import numpy as np
    from waymo2panorama.projection.ground_plane_layer import estimate_panorama_center_z
    from test_temporal_ground_seam import _nearest_pose, _project_temporal_ground, _read_city_poses
    import run_a1_streetview_pipeline as a1

    info = {
        "status": "not_started",
        "candidate_family": "temporal_multiframe_ground_source_derived",
        "candidate_labels": [int(i) for i in range(GEOM_OFFSET, GEOM_OFFSET + RAW_K)],
        "eligible_fraction": 0.0,
        "offsets_requested": [int(o) for o in TEMPORAL_OFFSETS],
        "offset_diagnostics": [],
        "temporal_ground": None,
    }
    try:
        anchor_ts = int(anchor_timestamps[anchor_idx])
        pts, _labels, lidar_delta_ms = a1.load_lidar_feather(log_dir, anchor_ts, max_delta_ms=75.0)
        pts = np.asarray(pts)[:, :3].astype(np.float64)
        ground, _facades = a1.fit_planes_p3(pts)
        if ground is None:
            info["status"] = "temporal_ground_fit_failed"
            return [], [], np.zeros((H, W), bool), np.zeros((H, W), bool), np.full((H, W), 1000.0, np.float32), info
        comps = structure_components(base, valid)
        conv_g = a1.build_plane_convergence(ground, [], (H, W)).astype(np.float32)
        yy = np.arange(H)[:, None]
        road_band = (yy > 410) & (yy < 790)
        boundary = source_boundary(label0, valid)
        band = cv2.dilate(boundary.astype(np.uint8), cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (35, 35))).astype(bool)
        tall = lidar_tall_mask_from_ground(pts, ground)
        plane_depth_ok = (conv_g > 3.0) & (conv_g < 45.0)
        protected = comps["lane"].astype(bool) | comps["wall_base"].astype(bool) | comps["curb"].astype(bool)
        low_structure = comps["structure_risk"] < 0.88
        eligible = valid & band & road_band & plane_depth_ok & (~tall) & low_structure

        n = np.asarray(ground["n"], dtype=np.float64).reshape(-1)
        d = float(ground["d"])
        ground_z_m = float(d / n[2]) if n.shape[0] >= 3 and abs(float(n[2])) > 1e-6 else 0.0
        if not np.isfinite(ground_z_m) or abs(ground_z_m) > 3.0:
            ground_z_m = 0.0
        center_z = estimate_panorama_center_z([frame.calibrations[c].T_ego_cam for c in cam_names])
        pose_ts, pose_mats = _read_city_poses(log_dir)
        target_pose, target_pose_ts, _target_pose_idx = _nearest_pose(pose_ts, pose_mats, anchor_ts)

        best_rgb = np.zeros((RAW_K, H, W, 3), dtype=np.uint8)
        best_weight = np.zeros((RAW_K, H, W), dtype=np.float32)
        best_offset = np.zeros((RAW_K, H, W), dtype=np.int16)
        for off in TEMPORAL_OFFSETS:
            src_idx = int(anchor_idx + off)
            diag = {"offset": int(off), "src_idx": int(src_idx)}
            if src_idx < 0 or src_idx >= len(anchor_timestamps):
                diag["status"] = "out_of_range"
                info["offset_diagnostics"].append(diag)
                continue
            src_frame = loader.load_synced_frame(anchor_timestamps[src_idx])
            src_pose, src_pose_ts, _src_pose_idx = _nearest_pose(pose_ts, pose_mats, int(src_frame.anchor_timestamp_ns))
            render = _project_temporal_ground(
                ...
                min_cam_cos=0.05,
                offset=int(off),
                target_ts_ns=anchor_ts,
                source_ts_ns=int(src_frame.anchor_timestamp_ns),
            )
            valid_render = render.valid & eligible
            for cam_i in range(RAW_K):
                m = valid_render & (render.cam_index == cam_i) & (render.weight > best_weight[cam_i])
                if bool(m.any()):
                    best_weight[cam_i][m] = render.weight[m].astype(np.float32)
                    best_rgb[cam_i][m] = np.clip(render.rgb[m], 0, 255).astype(np.uint8)
                    best_offset[cam_i][m] = int(off)
            diag.update({
                "status": "ok",
                "valid_fraction_before_eligibility": float(render.valid.mean()),
                "valid_fraction_after_eligibility": float(valid_render.mean()),
        ...
        g_slabs, g_weights = [], []
        for cam_i in range(RAW_K):
            gw = best_weight[cam_i].astype(np.float32)
            grgb = best_rgb[cam_i].astype(np.uint8)
            gw[~eligible] = 0.0
            grgb[gw <= 1e-8] = 0
            g_slabs.append(grgb)
            g_weights.append(gw)
        per_label_valid = [float((w > 1e-8).mean()) for w in g_weights]
        temporal_any = np.any(np.stack([(w > 1e-8) for w in g_weights], axis=0), axis=0)
        info.update({
            "status": "temporal_candidates_complete" if bool(temporal_any.any()) else "temporal_candidates_no_valid_pixels",
            "eligible_fraction": float(eligible.mean()),
            "temporal_any_valid_fraction": float(temporal_any.mean()),
            "temporal_candidate_valid_fraction": per_label_valid,
            "boundary_band_fraction": float(band.mean()),
            "tall_veto_fraction": float(tall.mean()),
            "protected_soft_risk_fraction": float((protected & band).mean()),
            "structure_risk_limit": 0.88,
            "ground_z_m_used": float(ground_z_m),
            "panorama_center_z_m": float(center_z),
            "temporal_ground": {
                "n": [float(v) for v in np.asarray(ground["n"]).reshape(-1)],
                "d": float(ground["d"]),
                "n_inl": int(ground.get("n_inl", 0)),
                "resid_m": float(ground.get("resid_m", 0.0)),
            },
            "lidar_delta_ms": float(lidar_delta_ms),
        })
        return g_slabs, g_weights, eligible.astype(bool), tall.astype(bool), conv_g, info
    except Exception as exc:
        info["status"] = "temporal_ground_candidates_blocked"
        info["error"] = {"type": type(exc).__name__, "message": str(exc), "trace_tail": traceback.format_exc()[-1800:]}
        return [], [], np.zeros((H, W), bool), np.zeros((H, W), bool), np.full((H, W), 1000.0, np.float32), info
...
def load_case(case_spec, workdir):
    ...
    anchor_timestamps = loader.anchor_timestamps_ns()
    anchor_ts = anchor_timestamps[anchor_idx]
    frame = loader.load_synced_frame(anchor_ts)
    slabs, weights, images, Ks, Ts, cam_names = [], [], [], [], [], []
    ...
    g_slabs, g_weights, temporal_eligible, tall_veto, ground_depth, temporal_info = build_temporal_candidates(
        log_dir, loader, anchor_timestamps, anchor_idx, frame, cam_names, weights, label0, valid, base
    )
    all_slabs = slabs + g_slabs
    all_weights = weights + g_weights
    temporal = {
    ...
    return run_name, all_slabs, all_weights, valid, label0, base, lidar, temporal
...
def build_case_boards(run_name, base, best_rgb, label0, best_label, boundary0, best_boundary, changed, abstain, comps, cfeat, summary, comp_stats):
    ...
    temporal_selected = (best_label >= GEOM_OFFSET) & (best_label < ABSTAIN)
    temporal_overlay = overlay(best_rgb, temporal_selected, (40, 255, 110), 0.70)
    risk_rgb = np.dstack([
    ...
    save_rgb(REMOTE_OUT / f"{run_name}_risk_components_board.jpg", stack_grid([
        ...
        ("temporal selected", temporal_overlay),
    ...
    save_rgb(REMOTE_OUT / f"{run_name}_route_cost_components_board.jpg", stack_grid([
        ...
        ("DB74 optimized temporal-candidate", draw_rois(best_rgb) if run_name.startswith("02a00399") else best_rgb),
        ...
        ("temporal-selected overlay", draw_rois(temporal_overlay) if run_name.startswith("02a00399") else temporal_overlay),
...
def roi_sheet_for_bmw(base, best_rgb, label0, best_label, changed, abstain, boundary0, best_boundary):
    ...
    draw.text((30, 24), "DB74 BMW marked ROI sheet", fill=(240, 240, 240))
    for r, (name, roi) in enumerate(MARKED_ROIS.items()):
        ...
        crops = [
            ...
            ("DB74 temporal-candidate", best_rgb[y0:y1, x0:x1]),
    ...
    sheet.save(REMOTE_OUT / "DB74_same_roi_comparison_sheet.jpg", quality=92)


def run_one_case(case_spec, workdir):
    import numpy as np
    t0 = time.time()
    run_name, slabs, weights, valid, label0, base, lidar, temporal = load_case(case_spec, workdir)
    ...
    operator_map = np.zeros((H, W), np.uint8)
    operator_map[(best_label >= GEOM_OFFSET) & (best_label < ABSTAIN) & valid] = 1
    save_u8(REMOTE_OUT / f"{run_name}_temporal_operator_id_map.png", operator_map * 255)
    save_u8(REMOTE_OUT / f"{run_name}_temporal_eligible_mask.png", temporal["eligible"].astype(np.uint8) * 255)
    save_u8(REMOTE_OUT / f"{run_name}_temporal_tall_veto_mask.png", temporal["tall_veto"].astype(np.uint8) * 255)
    save_u8(REMOTE_OUT / f"{run_name}_temporal_ground_depth_map.png", normalize_u8(temporal["ground_depth"], valid))
    ...
    inventory = {
        ...
        "valid_count_distribution": {str(i): int((cfeat["valid_count"] == i).sum()) for i in range(ABSTAIN + 1)},
        "candidate_valid_fraction": {str(i): float(cfeat["valid_stack"][i].mean()) for i in range(len(slabs))},
        "raw_camera_valid_fraction": {str(i): float(cfeat["valid_stack"][i].mean()) for i in range(RAW_K)},
        "temporal_candidate_valid_fraction": {str(i): float(cfeat["valid_stack"][GEOM_OFFSET + i].mean()) for i in range(max(0, len(slabs) - GEOM_OFFSET))},
        "temporal_candidate_status": temporal["info"],
        "temporal_selected_fraction": float(((best_label >= GEOM_OFFSET) & (best_label < ABSTAIN) & valid).mean()),
    ...
    claim = {
        "case": run_name,
        "raw_camera_sourced_rgb": True,
        "selected_pixels_from_raw_rotation_or_temporal_ground_slabs": True,
        "temporal_pixels_raw_camera_backprojected_on_conservative_ground": True,
        "temporal_operator_selected_fraction": float(((best_label >= GEOM_OFFSET) & (best_label < ABSTAIN) & valid).mean()),
        "abstain_pixels_kept_as_hard_select_for_visualization": True,
        "generated_mask": 0,
        "warp_vector_map": "absent",
        "operator_map": "0=raw_source_selection, 1=source_derived_temporal_ground_candidate, abstain marked separately",
        "sensor_truth_claim": "temporal raw-source candidate pending vision; not Bosch/source-faithful repair unless sidecars and vision pass",
    }
...
def build_full_board(summary):
    ...
    draw = ImageDraw.Draw(board)
    draw.text((30, 24), "DB74 - full ERP temporal raw-source candidate stack", fill=(240, 240, 240))
    lines = [
        "RGB candidates are raw-camera sourced: rotation slabs plus conservative temporal-ground backprojection labels. No inpaint/generation/model/DB32 edit.",
        f"status={summary.get('status')} classification={summary.get('claim_classification')}",
    ...
    panels = [
        ...
        ("BMW DB74 temporal-candidate", REMOTE_OUT / "02a00399_a000_bmw_optimized_source_owned_rgb.png"),
        ...
        ("clean DB74 temporal-candidate", REMOTE_OUT / "0bae3b5e_a030_clean_far_optimized_source_owned_rgb.png"),
    ...
    board.save(REMOTE_OUT / "DB74_full_review_board.jpg", quality=92)
...
try:
    ...
    classification = "temporal raw-source candidate pending vision" if bmw_ok_metric and not clean_degraded else "diagnostic/temporal raw-source candidate or rejected pending vision"
    summary = {
        "status": "DB74_phase0_phase1_complete",
        "claim_classification": classification,
        "source_faithful_repair_claim_allowed": False,
        "phase2_operator_eligibility_allowed": False,
        "temporal_candidate_result_requires_vision": True,
    ...
    (REMOTE_OUT / "DB74_batch_summary.json").write_text(json.dumps(json_safe(summary), indent=2), encoding="utf-8")
    OUT["status"] = "DB74_phase0_phase1_completed"
    OUT["summary"] = summary
except Exception as exc:
    OUT["status"] = "DB74_phase0_phase1_failed_or_blocked"
...
def remote_bash() -> str:
    ...
    return (
        ...
        "exec(compile(code, '<db74_temporal_candidate_stack_remote>', 'exec'))\n"
        "PY"
    )


def parse_json_from_log(log_tail: str) -> dict[str, Any] | None:
    if "DB74_JSON_BEGIN" not in log_tail or "DB74_JSON_END" not in log_tail:
        return None
    body = log_tail.split("DB74_JSON_BEGIN", 1)[1].split("DB74_JSON_END", 1)[0].strip()
...
def run_remote(timeout_s: int) -> dict[str, Any]:
    ...
    run_ok = bool(remote_result.get("status") == "DB74_phase0_phase1_completed")
    complete_outputs = bool((summary or {}).get("status") == "DB74_phase0_phase1_complete")
    manifest: dict[str, Any] = {
        "created_utc": datetime.now(timezone.utc).isoformat(),
        "status": "db74_temporal_candidate_stack",
        "scope": {
            ...
            "freeform_warp_or_apap_or_inpaint_used": False,
            "db32_edit": False,
            "red_promotion": False,
        },
        "runtime": {
        ...
        "drive_output_location": "results/layered_target_raycaster/db74_temporal_candidate_stack/",
        "decision": {
            "run_ok": run_ok,
        ...
        "claim_boundary": [
            "DB74 Phase0/Phase1 is a temporal raw-source candidate stack plus global label optimization.",
            "RGB candidate pixels are sourced from raw cameras either by rotation-only slabs or conservative temporal-ground backprojection labels.",
            "Abstain pixels are left as hard_select for visualization and marked by abstain_mask.",
            "No APAP/freeform flow warp, inpaint, generation, dense renderer, VGGT/Pi3/model inference, DB32 edit, or RED promotion was used.",
            "Vision review decides whether this is a temporal raw-source candidate, diagnostic/operator-eligibility evidence, presentation-only, or rejected.",
        ],
    }
    ...
    manifest["outputs"] = {
        ...
        "local_board": rel(OUT_DIR / "DB74_local_review_board.jpg"),
    }
    MANIFEST.write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")
    return {
...
def main() -> None:
    ...
    if not args.run_remote:
        print(json.dumps({"status": "ready", "message": "Use --run-remote after DB74 brief is active."}, indent=2))
        return
