...
# DB-104: ROBUST STITCHING — difficulty-aware seam escalation + robust object masks (harden Fable-5's gaps)
Status: ACTIVE (2026-06-19) — user directive: "make the stitching robust first; Fable-5 left some things unsolved." Generalizes DB-103 from the one car to the whole scene-band stitcher.
Goal/definition: for ANY input the stitcher produces a CLEAN result — a correct stitch OR a graceful degradation (NEVER a shear/ghost/smear). Make it measure-driven, not blind.
Gaps found this session (the fragility):
  (1) seam registration is a SINGLE AFFINE (STAGE-3.5 ECC) → breaks on near-object large/non-affine parallax (a309 car max_reg_px=32). Partly fixed: DB-103 `SEAM_FLOWMORPH` (affine→dense flow, gated). 
  (2) object MASKS (YOLO-seg) are incomplete on big/edge-truncated near cars — verified: front_left ragged/truncated, side_left misses the rear + window holes. SAM box-prompt is cleaner-edged but ALSO misses the rear + holes the windows by default. → need SAM+hole-fill+YOLO-union, box-constrained.
  (3) NO graceful-degrade tier when even flow can't register (the two cameras see DIFFERENT surfaces of a very close object = occlusion boundary, physically un-blendable) → currently still blends → residual shear/ghost.
Design — a difficulty-aware ESCALATION LADDER (the robustness core, ZERO scene params):
  - Tier-0 (far / small max_reg_px): ECC-affine (current).
  - Tier-1 (near, registrable overlap, max_reg_px>8): dense Farneback flow inside the body (DB-103, shipped).
  - Tier-2 (un-registrable: flow residual still high AND content disagrees): SINGLE-SOURCE the more-complete camera + route the content-DP-seam to the OBJECT EDGE; never alpha-blend un-correspondable content.
  - ROBUST MASK feeding all tiers: SAM(box-prompt) ∪ YOLO, fill internal holes (windows), keep the largest CC inside the matched box → complete object region.
Plan: (a) build + validate a robust complete object mask (SAM+holefill+union) on the a309 car [DONE - see result]; (b) add the Tier-2 graceful-degrade rung gated on (flow residual + content MAD); (c) integrate, ADDITIVE/gated, keep `_baseline_fable5/` pristine; (d) broad re-validation (the 5-scene + the near-object frames) — eyes>metrics, no new artifact, moving-car intactness.
ROBUST-MASK RESULT (a309, decisive — answers the user's "is it YOLO / would SAM help"): (1) YOLO mask on the close car is incomplete (window holes, ragged) BUT large (771k px); (2) SAM box-prompt alone is WORSE (249k, misses more) — NOT the fix; (3) ROBUST = YOLO∪SAM + binary_fill_holes + largest-CC gives a complete clean mask (793k, windows filled) — but the gain is ~all from hole-FILL, not SAM; (4) **DECISIVE: rendering a309 with `SEAM_MASK_FILL=True` (+flow) left max_reg_px at 8.64 UNCHANGED** → the mask is NOT the cause of the car shear. Combined with the earlier NEGs (object-box-depth 32→32, SAM-no-help), the residual is EXHAUSTIVELY isolated to the perspective disparity (front_left sees the car FRONT, side_left the REAR — raw frames `agent/_db103/rawframes/`), which only the dense FLOW (DB-103, 32→8.6) reduces; 8.6 is the 2-D physical floor. `SEAM_MASK_FILL` kept as a gated general tool (default OFF; helps elsewhere if a mask-hole artifact appears, not this case). Robustness core = the affine→flow ESCALATION (shipped) + graceful-degrade; extreme close-object perspective residual is accepted as physical (rare/transient).
Kill criteria: ZERO scene params; any tier must DEGRADE GRACEFULLY (a clean single-source butt-joint beats a shear); if a change regresses a currently-clean seam, gate it tighter. Physical limit (near-object perspective disparity) → accept the least-bad (clean) result, never fake-sharpen.
Required vision check: near-object seams no shear/ghost; clean seams unchanged; masks complete (no window holes / missed halves); 5-scene baseline + moving cars intact.
Output: code additive in db89 (mask step + STAGE-3.5 tiers); evidence in agent/_db104/; brief+progress+git synced.
