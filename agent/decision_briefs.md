...
**固定协议：**
  ...
  3. 候选不是逐场景超参搜索，而是同一组固定残差频带：`A + LPσ(B-A)`，σ=`8/4/2/1/0 cell`；从粗到细逐级放行。一个频带的 robust MAE 与 median RGB L2 必须在**每一个** inner fold 都不回退（零容忍）、整体改善、跨 fold 稳定，且不过度增加 Nyquist/checker 能量，否则截断。
