...
Status: **SOLVED / POS (2026-06-11, 2 /execs: 1 diag + 1 fix).** Diagnostic KILLED the label-flapping and poison hypotheses (flap <=0.21% everywhere, bestcam maps coherent) and the EMC-vs-composite A/B localised the grain to the BASE RENDER: **EDT nearest-neighbour depth is a 1-SAMPLE estimator** — on thin/sparse structures (downpipe, glass mullions, wall corners) it flips per pixel between foreground/background returns; every flip x camera baseline = sampling jump = grain (L1 was insensitive: ego-centred/plane depth). Mark-2 green bump = single-source temporal fill leaking a moving object through the LAGGED-box sightline gate (fill sources: side_left frames 4/6/8). **Fixes (both zero-threshold, in depth_field + the zone fill): (1) neighbourhood-MEDIAN depth (medianBlur 5 on the EDT field — estimator variance kill, edge-preserving); (2) TEMPORAL CONSENSUS fill — keep the 3 best independent (frame,camera) sources per pixel, fill with per-channel median (an outlier among 3 times is voted out).** Vision: mark1 downpipe coherent again, mark2 green bump GONE, mark3 wall edge clean, Porsche ZERO regression. Residual class left: native chroma fringe (source data). Tag: db91-grain-consensus-fixed.
Original report: User confirmed the Porsche overlap is SOLVED, then A/B'd the new render against the ORIGINAL L1 baseline and found 3 residuals the L1 didn't have:
...
Result summary: **CONVERGED / POS ⭐ (2026-06-10, v12; supersedes DB-88 v6 as best render).** Porsche passes every user-flagged defect at 6×/8×/14× (single A-pillar, single mirror, intact, continuous shadow, filled=0); X3 regression-free. The algorithm distilled into a six-rule EVIDENCE CALCULUS: boxes=identity-only / negative evidence untrusted within 1% border margin / ambiguous instances veto-not-assert / one-body-one-time with completeness-first + mask-measured OMC alignment / ghosts exist only >2px measured displacement / time as last resort (all-poisoned, triple-gated). KEY MECHANISM REWRITE: OMC measured du≈0 — the double A-pillar was DEPTH-PARALLAX (object at 13m painted at wall depth), not time displacement; uniform object-distance projection merges the copies. Full v5→v12 ledger in progress.md. Render `deliverables/db89_ghost_recovery/02a00399_a000_bmw_segcomposite.png`. Next: crowd/downtown generality, then fold the full stack into the 5-scene pipeline.
Earlier exploration record: **EXPLORED / MAJOR DATASET FINDING + architecture converged (2026-06-10 morning, 8 /execs).**
...
**v6–v7 addendum (2026-06-10 night, 4 more /execs with the user online):** v6 morphological mask completion (close15+dilate9) fixed the user-spotted double MIRROR, but the user then spotted the residual double A-PILLAR and correctly rejected morphology as a non-general patch. Root mechanism: the ERP holds the object at MULTIPLE camera-exposure-time positions; the body rule renders one; the other cameras' displaced copies (the GHOST ZONE) survive in the EMC base and must be replaced by the occluded background — whose only evidence source is TIME. **v7 = the principled general algorithm (designed + implemented):** (1) static ← EMC; (2) body ← one (Voronoi-dominant cam, its time), extent = mask ∪ own-time-box-ray-hit (evidence fusion: box=complete-coarse, mask=tight-lossy), uniform object-distance projection; (3) ghost zone (∪ all-camera-time box positions − body) ← temporal recovery under a TRIPLE gate (object provably departed |Δframe|≥3 ∧ padded-box-free sightline ∧ LiDAR-evidenced background depth); (4) gate fails → EMC pixel, never hard-fill; no depth overwrites. **v7 run FAILED on an infra bug, not the design** (car rendered as a stretched smear — `cam_ts` nearest-image-timestamp can land seconds away when frames are missing → `track_pose_at` interpolates the box far away → regions explode). **Successor fixes (next session, fresh budget): (a) assert |cam_ts−anchor|<60 ms per camera else skip; (b) assert per-object region width < 2× expected angular size; (c) re-run v7 unchanged.** Repo restored to v6 (best on-disk); v6 residual = the ~16 px ghost-zone A-pillar band. 9 /execs total — stopped per discipline.
