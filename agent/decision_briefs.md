...
Status: A0 NEG/MIXED → A1 built but **/code-review RETRACTED the A1 verdict (BUGGY + UNFAITHFUL, 2026-06-02)** → A1 needs a CORRECT re-do before any conclusion
A1 retraction: code-review (41 agents) confirmed critical bugs — (#1) meshgrid coord SWAP in dis_flow_align made the flow warp garbage (so the "flow starves / black-tears" A1_flow verdict was MY BUG, not the method); (#7) reduced-scale seam mask leaves coverage holes the blender fills BLACK; (#5) no spatial-propagation flow. AND faithfulness: we never implemented Surround360's novel-view synthesis (warp L·t + R·(1−t)) nor Google's global spline warp — dis_flow_align just warps toward L1. **So "Route A ceiling ≈ L1+ / geometry can't single doubling" is RETRACTED — not validated.** NEXT: fix bugs + faithfully implement Surround360 (per-azimuth flow·t/flow·(1−t) + FB-gating) + re-run + vision-judge.
