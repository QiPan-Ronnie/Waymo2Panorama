...
Result summary: DB45 is paused on DB45i executor DNS/connectivity after repeated sandboxed/approved network attempts and the latest current-endpoint `/status` precheck again failed DNS resolution before any `/exec` job. This is not a VGGT/model negative and accepts no geometry evidence. Re-run only the same DB45i `--run-remote` after `/status` reachability is independently confirmed; do not continue patch-on-patch.
