...
- Phase9 status (2026-06-04): DB45i is **blocked-or-paused on executor DNS**, with no model action. Added `scripts/phase3/db45i_vggt_calibrated_residual_extractor.py` and generated local manifest/board plus a sanitized remote-result record. The one bounded remote attempt stopped before `/status`/`/exec` because the provided tunnel hostname failed DNS resolution (`getaddrinfo failed`); therefore `model_inference_ran=false`, no `pose_enc` tensor was saved, no decoded extrinsics/intrinsics or Sim(3) residuals exist, and no target-surface residual evidence was accepted. Latest recovery rechecks after a fresh A100 executor JSON also stopped before `/status`/`/exec` in sandboxed and approved non-sandbox network attempts with the same `getaddrinfo failed` DNS error. `accepted_db45_diagnostic_evidence=false`, `accepted_db45_geometry_evidence=false`, `permission_state_changes=none`, `red_promotions=[]`. DB45 is paused on executor DNS for VGGT residuals; re-run only the same DB45i `--run-remote` when a reachable executor URL is available, and do not continue patch-on-patch. Detail archived at top of `progress.md`.
...
Result summary: DB45 is paused on DB45i executor DNS/connectivity after repeated sandboxed and approved non-sandbox attempts stopped before `/status`/`/exec`. This is not a VGGT/model negative and accepts no geometry evidence. Re-run only the same DB45i `--run-remote` after a reachable executor endpoint is available; do not continue patch-on-patch.
