...
# DB-20260602-13: LEARNED strip Band-MPI/MVSNet — ★ OVERFIT KILL-TEST DONE → surfaced a NON-learned WIN (gated LiDAR-reproject)
Status: explored — **the overfit kill-test (2026-06-02) was MIXED and DECISIVE: the self-supervised PHOTOMETRIC depth optimizer is a NEG (degenerate far-depth attractor — depth→∞ ⇒ parallax→0 ⇒ trivial zero-cost min, 92.8% pinned at the clamp), so a purely photometric/self-supervised learned depth WON'T work at these baselines. BUT the pure dense-LiDAR-depth reproject SINGLES the BMW cleanly → the win is ACCURATE DENSE DEPTH + a CROSS-VIEW GATE, not learning. ★ This led to a NON-learned deliverable `_lidar_reproject.py` (densified-LiDAR-depth reproject + cross-view gate + L1 fallback) that CLEANLY de-doubles the verified seam fraction on BMW/0bae/fbee, source-faithfully, no smear (progress.md "part 6"). So the learned route is only needed to ENLARGE the verifiable fraction, and codex's caveat stands: it must be LiDAR/metric-SUPERVISED (inherit metric depth), NOT photometric/self-supervised. The overfit kill-test thus PAID OFF (found a cheaper win + ruled out photometric self-supervision).** If scaled later: a LiDAR-supervised depth/MPI net to densify depth beyond the current kNN, still with the cross-view gate. NEEDS USER GO for the multi-day train.
Was: proposed (needs user GO — multi-day) — recommended by BOTH codex(gpt-5.5 xhigh) rounds after the 5-angle wall proof
