...
from agent.db181_multids.scene_band_source_builder_v8 import (
...
def test_builder_exports_calibration_safe_scene_band_and_mask() -> None:
    rendered = build_scene_band_renderer_source(
        ...
        export_scene_band=True,
    )
    assert '_scene_band.png"' in rendered
    assert "_scene_band_angular_mask.png" in rendered
    assert "_angular_gap_report[\"safe_band_rows\"]" in rendered
    assert '"scene_band_rows": [int(_band0), int(_band1)]' in rendered


def test_builder_does_not_export_band_when_disabled() -> None:
    rendered = build_scene_band_renderer_source(
        _source(),
        policy=policy_for_dataset("pandaset"),
        data_root="/panda",
        ...
        photometric_seam_ownership=True,
        angular_gap_fallback=True,
        export_scene_band=False,
    )
    assert '_scene_band.png"' not in rendered
    assert "_scene_band_angular_mask.png" not in rendered
