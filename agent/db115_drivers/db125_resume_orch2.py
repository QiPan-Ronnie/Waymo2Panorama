...
# ---------- RESUME: wait for GPU fill, reuse existing map ----------
t0 = time.time()
while time.time() - t0 < 14400:
    log = open("/content/_dj_db125gfill.log").read() if os.path.exists("/content/_dj_db125gfill.log") else ""
    if "GF_DONE" in log or "Traceback" in log or "AssertionError" in log:
        break
    time.sleep(30)
assert "GF_DONE" in log, "gpu fill failed: " + log[-400:]
import re as _re
_m = _re.search(r"GF_RCS .* (\d+)s \(([0-9.]+)s/frame\)", log)
mark("fill_gpu", "done %ss (%ss/frame)" % (_m.group(1), _m.group(2)) if _m else "done")
cs = json.load(open("/content/db125_cand_summary.json"))
P = cs["best_anchor"]; SC = cs["best_case"]; CDIR = cs["cand_dir"]
BAND_ANCHORS = list(range(P + 1, P + 93))
MID = P + 46
import shutil
shutil.copy("/content/db125_map/mp_a%03d_worldmap.png" % MID, "/content/db125_worldmap.png")
mark("map", "reused mp_a%03d_worldmap.png" % MID)
...
for i, aN in enumerate(BAND_ANCHORS):
    ...
    fil = cv2.imread(find1("/content/db125_fill/*/*_a%03d_segcomposite.png" % aN), cv2.IMREAD_COLOR)
    fai = cv2.imread(find1("/content/db125_fill/*/*_a%03d_faithfill_mask.png" % aN), 0)
...
'''
a.dr_launch("db125res2", JOB)
print("RESUME2_LAUNCHED")
