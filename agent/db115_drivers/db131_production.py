...
LED = {"gpu": "Blackwell96G", "K": K, "pipeline": "v10-final", "logs": {}}


def led(u8, k, v):
    LED["logs"].setdefault(u8, {})[k] = v
    json.dump(LED, open("/content/db131_ledger.json", "w"), indent=1)
    subprocess.run("cp /content/db131_ledger.json '%s/datasets/av2_1plus92_cascade_v1/db131_ledger.json'" % DRIVE, shell=True)
...
for ln in r.stdout.splitlines():
    ...
    if len(u) > 30 and not any(u.startswith(p) for p in USED):
        cands.append(u)
print("QUEUE %d candidates" % len(cands), flush=True)
...
while ci < len(cands):
    ...
    try:
        ...
        led(U8, "verdict", "OK")
        ok_count += 1
        print("PRODUCED %d logs so far (%s)" % (ok_count, U8), flush=True)
        shutil.rmtree("/content/localav2/val/" + U, ignore_errors=True)
        shutil.rmtree(root, ignore_errors=True)
        ...
        shutil.rmtree("/content/localav2/val/" + U, ignore_errors=True)
        continue
print("DB131_QUEUE_EXHAUSTED ok=%d" % ok_count, flush=True)
