...
LED = {"gpu": "Blackwell96G", "K": K, "pipeline": "v12", "machine": MACHINE_SHARD, "logs": {}}


def led(u8, k, v):
    LED["logs"].setdefault(u8, {})[k] = v
    json.dump(LED, open("/content/db132_ledger.json", "w"), indent=1)
    subprocess.run("cp /content/db132_ledger.json '%s/datasets/av2_1plus92_cascade_v1/db132_ledger_m%s.json'" % (DRIVE, MACHINE_SHARD.replace(",", "of")), shell=True)
...
while ci < len(cands):
    ...
    try:
        ...
        # ---- motion-aware window (v10-final): among clean windows pick max dmax ----
        ...
        GRIDD = [GRID_BASE, "_MHALF, _CW = %.1f, %.6f" % (R, R / 920.0)]
        ...
        for _si in range(8):
            ...
            _extra_s = json.dumps(json.loads(extra_map) + [
            ...
            mprocs.append(subprocess.Popen(["python", "/content/db125_worker.py", "ms%d" % _si, str(MID), U, _od, _extra_s],
        ...
        _extra_f = json.dumps(json.loads(extra_map) + [['WORLDBEV_LOAD = ""', 'WORLDBEV_LOAD = "%s/map/merged.npz"' % root]])
        os.makedirs(root + "/map/final", exist_ok=True)
        _flf = open(root + "/map/final.log", "w")
        mrc = subprocess.run(["python", "/content/db125_worker.py", "mf", str(MID), U, root + "/map/final", _extra_f],
...
if flux_thread is not None:
    flux_thread.join()
print("DB132_QUEUE_EXHAUSTED ok=%d" % ok_count, flush=True)
