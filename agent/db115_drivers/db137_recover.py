...
def led(u8, k, v):
    ...
    subprocess.run("cp /content/db134_ledger.json '%s/datasets/av2_1plus92_production_v14/db137_recover_ledger_m%s.json'" % (DRIVE, MACHINE_SHARD.replace(",", "of")), shell=True)
...
json.dump({"run": "run100_v14.1", "total": len(LOG100), "machine_shard": MACHINE_SHARD,
          ...
          open(_mdir + "/manifest_r2_m%s.json" % MACHINE_SHARD.replace(",", "of"), "w"), indent=1)
print("QUEUE %d candidates (machine shard %s)" % (len(cands), MACHINE_SHARD), flush=True)
for _mf in glob.glob(_mdir + "/db137_recover_ledger_m%s.json" % MACHINE_SHARD.replace(",", "of")):
    try:
