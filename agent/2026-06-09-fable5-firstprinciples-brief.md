...
- **COMPUTE / L4:** a live **L4 GPU (≈22.5 GB free)** is available and **the user has explicitly authorized you to use the runtime JSON directly** — you do NOT need to ask before each GPU run, and no secret-handling ceremony is required. One light hygiene note only (not a gate): keep the **raw token/URL out of git-committed files** — this repo pushes to a public GitHub remote, so read the endpoint from a local file/env and don't paste it into committed code/logs/boards.
