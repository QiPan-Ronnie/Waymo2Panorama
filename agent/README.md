...
As of 2026-06-05, DB58-63 closed the direct VGGT -> A1/G / DB25 post-ERP patch branch as diagnostic or presentation-only no-repair evidence. DB64 `LTR-v0 layered target-raycaster` is now running but paused after Phase4b z-visibility cause evidence: Phase0 CPU/local preflight completed, Phase1 CPU Colab Drive-data preflight confirmed the fixed BMW target and clean-control raw/calib/LiDAR data, Phase2 CPU Colab LiDAR-zbuffer produced diagnostic RGB-copy variants that remain rejected as repair, Phase3 CPU Colab emitted complete sidecar maps for the two fixed cases, Phase4a CPU/local split the high seam unknown into cause/repairability maps, and Phase4b CPU Colab produced raw projection/z-buffer cause maps under `deliverables/layered_target_raycaster/db64_ltr_v0/phase4b_z_visibility_cause/`. Phase4b confirms that BMW's extra weakness is mainly `no_target_surface_support +0.0664` and lower `multi_source_visible -0.0331`, not a z-mismatch or camera-geometry-only issue. BMW seam metrics: LiDAR support `0.4274`, multi-source visible `0.0912`, single-source visible `0.0842`, no target surface `0.5726`, no raw zbuffer support `0.1957`, z mismatch/conflict `0.0366`, source-boundary proxy `0.1224`. Latest GPT Pro review recommends the next candidate sub-scope as `DB64 Phase5a - Continuous target-surface evidence preflight` with minimal protected-mask veto, but Phase5a is not opened or authorized yet; a new decision brief/extension is required before any run. Do not call DB64 a repaired panorama, source truth, semantic layer truth, protected semantic mask, or repair permission. Further DB64 work must open a fresh continuous-surface/layer-evidence or protected object/lane/curb mask sub-scope before any layer fitting or abstain-aware renderer. HardSelect++ is the conservative control/product baseline; presentation synthesis is isolated as demo-only / not sensor truth.
