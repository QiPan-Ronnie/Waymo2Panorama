...
**The single-file full-stack algorithm:** [`scripts/phase3/db89_ghost_recovery.py`](../scripts/phase3/db89_ghost_recovery.py) — perspective→360° ERP panorama, source-faithful + evidence-gated + abstain, **zero per-scene parameters**.

**Scene band (the stitch), in `run_case()`:** virtual centre (ring-camera centroid, not ego origin) → LiDAR multi-sweep depth → photometric gain → **EMC** per-camera exposure-instant pose (SE(3)/SLERP) → YOLOv8-seg evidence + Voronoi-dominant `c_own` → **one body · one camera · one time** (+ECC-measured OMC) → view morph + DP content seam → temporal consensus → chroma-fringe YCrCb clamp. Rule zero: **never average geometry; abstain where evidence is insufficient.**

**Completing the sphere:**
- **Ground = STAGE 4 of `db89_ghost_recovery.py`** (deterministic time reprojection, real pixels, NO generation): displacement-bucketed whole-log candidate frames → two-box ego self-occlusion gate → 6-source median consensus, single nearest-to-median render → resolution-matched nadir low-pass. Per-case `ground_fill` stats + `low_coverage_warning` sentinel.
- **Sky = [`scripts/phase3/sky_fill_flux.py`](../scripts/phase3/sky_fill_flux.py)** (the ONLY generated layer): FLUX.1-Fill-dev mask-conditioned inpaint + DiT360 panorama LoRA (geometry only) + auto-prompt (sunny/dusk/overcast). Pixels outside the sky mask restored byte-exact.

**Supporting:** [`code/waymo2panorama/data_io/av2_loader.py`](../code/waymo2panorama/data_io/av2_loader.py) (AV2 ring loader). Historical G-era deliverable: [`scripts/phase3/_seamroute.py`](../scripts/phase3/_seamroute.py).

**Deliverables:** `deliverables/db89_ghost_recovery/` (5-scene v2.2 scene band) · `deliverables/complete_pano_v8/` (5-scene complete spheres). **Provenance:** scene band + ground = 100% real pixels; sky = only generated region. Full v8 chronicle in `progress.md` (DB-93, 2026-06-12); milestone commits bfbc244 → c589aa7 → 4f50ec8 → a9e3497.
...
> **DB75 current best viewable result (2026-06-05):** DB72 source-label optimizer, DB73 ground candidate labels, and DB74 temporal/multiframe candidate labels are all completed and visually rejected as seam repair. DB74 generated temporal candidates but selected none (`temporal_selected_fraction=0.0`), so continuing source-label/ground/temporal weight tuning is a local optimum. DB75 then ran the bounded full-ERP raw-source seam-band blending fallback and is the current best available viewable seam version only as `presentation_only/source_mixed_not_repair`. Output root: `deliverables/layered_target_raycaster/db75_full_erp_source_mixed_fallback/`; key files: `fetch/02a00399_a000_bmw_source_mixed_candidate.png`, `DB75_full_review_board.jpg`, `DB75_same_roi_comparison_sheet.jpg`, `DB75_vision_verdict.json`, `DB75_manifest.json`. User follow-up vision review marks that DB75 only softens the seams: the left road patch still does not connect, the right curb/sidewalk/wall-base remains stepped, and the black BMW/SUV region still shows overlap/ghosting. It uses current-frame raw slabs, alpha/source-mix/diff sidecars, and `generated_mask=0`, but pixels in the mix band are not single-source truth. It must not be claimed source-faithful repair, Bosch training-ready, geometry alignment, general method success, or RED promotion. A next technical route, if opened, needs a fresh decision brief for source-backed local alignment/operator eligibility with warp vectors/residuals, or abstain/eligibility packaging.
...
> **DB58 result (2026-06-05):** DB58 is accepted only as `db58_cpu_local_preflight_abstain_no_repair`. A CPU/local existing-artifact preflight ran under `deliverables/dit360_v2/db58_vggt_raw_camera_seam_roi/` and found that DB58 must stop before A100/VGGT/warp/composite: failed gates are `vggt_pose_coordinate_admissibility` and `target_surface_lidar_flow_support`; diagnostic/blocking gates are `raw_owner_uv_preflight` and `source_id_and_sidecar_support`. No remote/status/exec, A100, network, new VGGT inference, seamroute rerun, renderer, warp/composite, generation, source replacement, `source_id_map`, RED promotion, or permission change occurred. `A1_view_none` and `G_bmw_pano` remain diagnostic visual references only under DB58; directly repairing A1/G requires a fresh decision brief. DB41 right/lower-right remains no-evidence/abstain unless a separate fresh brief changes that evidence state.

> **DB59 result (2026-06-05):** DB59 is accepted only as `cpu_local_preflight_no_remote_no_repair`. It tested the user-intended A1/G question DB58 did not cover, but stopped before A100/VGGT because the evidence gates fail under current artifacts. Frozen ROI set: DB25 longline as primary A1/G diagnostic target, DB41 right/lower-right as negative controls only. Raw 7-camera BMW anchor 0 remains the only source truth; A1/G are diagnostic display references, not source truth or VGGT input. Failed gates are `vggt_pose_coordinate_admissibility` and `target_surface_lidar_flow_support`; blocked gate is `source_id_map_and_protected_masks`. Existing DB45f official VGGT raw-anchor owner-UV evidence already covers the frozen DB25/DB41 rows, while DB45k keeps coordinate/reflection non-admissible. No remote/status/exec, A100, network, new VGGT inference, repair, generation, source replacement, `source_id_map`, RED promotion, or permission change occurred. Chat-pasted runtime JSON was not used.

> **DB60 result (2026-06-05):** DB60 is completed as an ungated presentation-only VGGT-prior quick-look and rejected as repair. It intentionally ignored evidence gates only for visualization, using existing DB45f VGGT DB25 heatmap grids as a soft alpha prior on `A1_view_none` and `G_bmw_pano`; no A100/new VGGT/remote/generation/inpainting/source replacement ran. The A1 candidate mostly gives subtle low-frequency attenuation and leaves the seam visible; the G candidate introduces wavy sidewalk/curb distortion. Keep DB60 as diagnostic stress-test evidence only, not source-faithful/raw-camera-backed repair, not a fixed A1/G/BEST/DB32, not `source_id_map`, not RED promotion, and not Bosch training data.

> **DB61 result (2026-06-05):** DB61 is completed as a fresh-A100 ungated A1/G quick-look and rejected as repair. After the runtime source was updated through a non-repo secret file, the runner reached the Colab A100 endpoint, bootstrapped official `facebookresearch/vggt`, ran `facebook/VGGT-1B-Commercial` on raw 7-camera BMW anchor `0`, and produced DB25-only owner-UV sampled VGGT metadata plus A1/G fixed-ROI quick-look images under `deliverables/dit360_v2/db61_fresh_vggt_a1g_quicklook/`. Hard checks pass: `a100_job_submitted=true`, `new_vggt_inference=true`, `db25_only_remote_scope=true`, `uses_db45f_result_json_as_evidence=false`, and secret scan hits `0`. Vision result remains weak: A1 keeps the long seam and G keeps/gets sidewalk-curb waviness. DB61 is presentation-only diagnostic evidence, not source-faithful/raw-camera-backed repair, not a fixed A1/G/BEST/DB32, not `source_id_map`, not RED promotion, and not Bosch training data.

> **DB62 result (2026-06-05):** DB62 is completed as a VGGT point/depth/confidence-guided raw-camera source-composite diagnostic and rejected as repair. It used official VGGT `world_points`, `depth`, `depth_conf`, and `world_points_conf` to score raw-camera source candidates inside fixed DB25, then copied/blended only raw-camera ERP slab pixels into A1/G candidates under `deliverables/dit360_v2/db62_vggt_raw_source_composite/`. Hard checks pass and secret scan hits `0`; `new_vggt_inference=true`, `raw_camera_pixels_used=true`, and `vggt_point_confidence_scoring_used=true`. Vision result remains negative: the soft composite changes sparse islands and the long seam remains, while the hard selected raw crop shows blocky/vertical wall and boundary discontinuities. DB62 proves VGGT was actually used for A1/G source selection, but this direct operator is still diagnostic/presentation-only, not source-faithful, not accepted repair, not `source_id_map`, not RED promotion, and not Bosch training data.

> **DB63 result (2026-06-05):** DB63 is completed as a VGGT component-gated raw-source probe and closed as `fragmented_sparse_no_repair`. It used only existing DB62 VGGT alpha/source-label/hard-source outputs, no A100/new inference, to test whether high-confidence source-switch islands form a smaller repairable surface inside DB25. They do not: strict component gating keeps only `2` components covering `0.021025` of ROI, amplified alpha mean is `0.011252`, and A1/G ROI p95 delta remains `0.0`. Visual review shows small wall/sign/near-wall islands rather than a continuous longline seam fix. Keep DB63 as diagnostic evidence only; do not continue by simply amplifying DB62/DB63 alpha.

> **DB64/DB65 update (2026-06-05):** DB64 is paused at a Phase5a evidence endpoint after DB58-63 closed the direct VGGT->A1/G and DB25 post-ERP patch branches. Phase0-Phase5a are evidence only; Phase5a completed one approved CPU Colab `/status` plus `/exec` but failed thresholds (`aggregate_success=false`, `phase5b_allowed=false`): BMW no-surface drops only `0.0552`, visible-any `-0.0150`, visible-ge2 `-0.0068`, no-zbuffer gap worsens `+0.0417`, and longest supported component remains `0.1465`. Do not enter Phase5b layer fitting or Phase5c renderer. The current best visible seam version is DB65 under `deliverables/layered_target_raycaster/db65_visible_photometric_fallback/`: `db65_best_visible_photometric_candidate.png`, `db65_visible_photometric_fallback_board.jpg`, and manifest. It is `presentation/diagnostic photometric polish`, not source-faithful repair. DB66 inpaint was rejected by vision.

> **DB67 update (2026-06-05):** DB67 dense raw-aligned target-surface evidence audit is completed as `diagnostic/evidence-only; failed source-faithful dense-surface route`. Phase0 selected VGGT over Pi3 for the first dense backend; Phase1a hit only a missing `av2` dependency; Phase1b completed the same fixed BMW/clean A100 audit after `av2>=0.3` bootstrap. Output root: `deliverables/layered_target_raycaster/db67_dense_raw_aligned_surface_audit/phase1_vggt_dense_evidence/`. Result: `aggregate_success=false`, `phase2_renderer_allowed=false`, `clean_control_degraded=true`. BMW no-surface worsens `0.5726 -> 0.7680`, visible-any `0.1753 -> 0.0020`, visible-ge2 `0.0912 -> 0.0001`, LiDAR-agreed dense support is only `0.0129`, and longest visible component is `0.064`. Do not run DB67 Phase2/layer fitting/renderer, do not treat VGGT dense confidence as source truth, and keep DB65 as the current best visible seam result until a fresh brief produces a better presentation-only candidate.

> **DB68 update (2026-06-05):** DB68 is completed as weak `presentation/diagnostic photometric polish` but rejected as a seam solution after user visual review. It ran CPU/local only with `scripts/phase3/db68_edge_aware_photometric_polish_v2.py`, using existing DB64/DB65/DB67 artifacts and no inpaint/generation/source replacement/model/remote. Output root: `deliverables/layered_target_raycaster/db68_edge_aware_photometric_polish_v2/`; key files are `db68_best_edge_aware_candidate.png`, `db68_edge_aware_polish_board.jpg`, `db68_top_variant_roi_sheet.jpg`, and `db68_edge_aware_manifest.json`. Selected `db65_best_micro_median_r3_m0_s0.40_d5_ef0.22`: relative to DB65, seam energy improves `+9.95%`, DB25 ROI energy improves `+9.54%`, changed fraction is `0.0173`, outside-allowed changed fraction is `0.000174`, max abs delta is `5.0`, and secret scan hits `0`; however the user's marked review shows road patch, lane marking, and curb/sidewalk/wall-base geometry remain visibly misaligned. Do not continue photometric patch-on-patch as the main route. The next useful route must address seam placement or local geometry alignment, while DB67 renderer gating remains blocked.

> **DB69 update (2026-06-05):** DB69 completed the user-marked geometry seam audit plus source-label-only reroute test. Corrected Phase0 ran CPU/local with `scripts/phase3/db69_user_marked_geometry_seam_reroute_phase0.py`; output root `deliverables/layered_target_raycaster/db69_user_marked_geometry_seam_reroute/`. The audit now uses `hard_select_source_id_map` for visible source ownership; the sparse `source_id_map` was the first-run audit bug. Phase0 ROI feasibility: left road patch `RED`, lower-center road patch `RED`, center lane/wall-base `GREEN`, right curb/wall-base `GREEN`. Phase1 ran exactly one secure external `/status` plus one `/exec` with `scripts/phase3/db69_source_label_reroute_phase1.py`, output root `deliverables/layered_target_raycaster/db69_user_marked_geometry_seam_reroute/phase1_source_label_reroute/`. Best variant `lw06` lowered ROI seam energy `10.5319 -> 8.6183` (`+18.17%`) with changed fraction `0.00857`, but vision rejects it as a seam solution: the seam becomes jagged/wavy, left/lower road-patch discontinuities remain, and right wall/curb block switching remains. DB69 is a source-owned diagnostic/presentation candidate only; do not continue by tuning line weights.

> **DB70 update (2026-06-05):** DB70 is the current active route: protected local ground-plane alignment. It is opened because DB69 proved seam relocation alone is not enough and old full ground-road seamroute outputs have broad artifacts. Scope is one bounded BMW run through non-repo runtime secret: raw 7-camera ERP slabs plus LiDAR ground-plane fitting, applied only inside narrow seam/ROI masks with car/wall/tall/high-structure/out-of-FOV vetoes. No VGGT/Pi3/HF/model inference, no DiT/FLUX/3DGS/inpaint/generation, no DB32 edit, no full-frame ground-road replacement, and no source-faithful permission promotion.

> **DB71/DB72/DB73 update (2026-06-05):** DB70 completed and was rejected by metrics/vision; DB71 was opened as a presentation-only fallback but paused before any run because it is not a general algorithm. DB72 completed Phase0/Phase1 as the first real full-ERP source-owned candidate stack plus global structure-aware source-label optimizer. It is useful diagnostic/operator-eligibility evidence, but not repair. DB73 completed v0 plus Phase1b sparse geometry-gate extension; it generated source-derived ground-plane candidate sidecars but selected no geometry operator in the best candidate and remains visually rejected. The current general-route output to show is `deliverables/layered_target_raycaster/db73_geometry_candidate_stack/phase1b_sparse_geometry_gate/`, classified diagnostic/rejected, not source-faithful repair.
...
Most recent concrete route plan: `plans/2026-06-04-egsr-seam-and-route-roadmap.md`.

As of 2026-06-05, DB58-63 closed the direct VGGT -> A1/G / DB25 post-ERP patch branch as diagnostic or presentation-only no-repair evidence. DB64 `LTR-v0 layered target-raycaster` has completed Phase0-Phase5a and is paused as evidence-only, not repair; Phase5a failed its evidence thresholds (`aggregate_success=false`, `phase5b_allowed=false`) because BMW raw-visible support worsens and continuous supported components remain too short. DB65 then produced the current best visible seam version under `deliverables/layered_target_raycaster/db65_visible_photometric_fallback/`, accepted only as `presentation/diagnostic photometric polish`, not source-faithful repair. DB66 inpaint was rejected by vision for interpolation artifacts. DB67 Phase0 completed CPU/local inventory under `deliverables/layered_target_raycaster/db67_dense_raw_aligned_surface_audit/`, selected VGGT as the first dense evidence backend, deferred Pi3/Pi3X, and is paused because Phase1 now needs A100 dense inference through non-repo runtime/auth secrets. HardSelect++ remains the conservative control/product baseline; presentation synthesis is isolated as demo-only / not sensor truth.
