...
> **Current pointer (2026-06-04):** read the top of `handoff.md`, `progress.md`, `decision_briefs.md`, and `plans/2026-06-04-egsr-seam-and-route-roadmap.md` before acting. DB43/DB44 are accepted; DB45 is running. DB45e accepted only `vggt-roi-confidence-diagnostic-only` evidence, and DB45f accepted only `vggt-target-uv-sampling-diagnostic-only` evidence: official VGGT ran once on BMW anchor 0, then DB45f sampled VGGT maps at source-owner raw-camera UV pixels used by frozen ERP seam ROIs. This is diagnostic metadata only, not physical target-surface geometry evidence or repair permission; it kills VGGT confidence-only RED promotion. DB45g accepts only `vggt-official-source-decode-path-diagnostic-only`: official source documents a pose/camera decode path, and local DB45f records `pose_enc` as a prediction key, but DB45f did not store pose tensors/decoded extrinsics and the runtime source/API inspection is still blocked by executor DNS/tunnel availability. DB45g therefore has `residual_readiness=false`, no geometry evidence, and no RED promotion. DB25/DB41 remain `RED/abstain`, DB36/DB40 remain generated fake-geometry rejects.
