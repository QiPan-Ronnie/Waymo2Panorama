...
> **Current pointer (2026-06-04):** read the top of `handoff.md`, `progress.md`, `decision_briefs.md`, and `plans/2026-06-04-egsr-seam-and-route-roadmap.md` before acting. DB43/DB44 are accepted; DB45 is paused on DB45i executor DNS before model action, including the latest current-endpoint `/status` reachability precheck, which again failed DNS before any `/exec` job. DB45e/f/g/h/i respectively accept only `vggt-roi-confidence-diagnostic-only`, `vggt-target-uv-sampling-diagnostic-only`, `vggt-official-source-decode-path-diagnostic-only`, `vggt-residual-job-contract-only`, and `blocked-or-paused`; together these kill VGGT confidence-only RED promotion and define the future extractor contract, but still accept no physical target-surface geometry evidence, no residual readiness, no source-faithful repair permission, and no RED promotion. DB47 is running as the source/frame candidate-mining sidestep route; DB47a is completed as `source-selection-inventory-only`, DB47b as `source-selection-threshold-replay-only`, DB47c as `source-selection-visual-accounting-only`, and DB47d as `source-selection-exact-review-pack-only` over the 10 strict/relaxed same-log rows (`exact_review_candidate_not_final=3`, `missing_exact_hold=7`). DB47d selects no final candidate and accepts no source-faithful repair. DB49a is completed as `bosch-data-contract-inventory-only`; DB49b is completed as `sidecar-starter-pack-partial-only`, creating only DB32 partial `generated_mask`, `unknown_or_abstain_mask`, and `risk_map` sidecars from existing sky-mask/out-of-FOV/DB41 evidence; DB49c is completed as `source-id-map-feasibility-inventory-only`, finding no complete per-pixel `source_id_map` for the exact DB32 lineage and creating no map. DB49 confirms DB32 `s40` can only be packaged as a caveated handoff candidate and remains not ready for uncaveated Bosch training-data use. DB28/DB41 camera labels are ROI diagnostics, not source ownership maps; `_seamroute.py`'s internal label is only a future reproducible path candidate unless a fresh bounded brief instruments/reruns the exact lineage. DB28/DB32 remain source-sidestep/current-handoff evidence only, not original-G seam repair or a source-faithful ceiling. DB25/DB41 remain `RED/abstain`, DB41 lower-right remains zero-LiDAR abstain, DB36/DB40 remain generated fake-geometry rejects.
