...
> **Current pointer (2026-06-04):** read the top of `handoff.md`, `progress.md`, `decision_briefs.md`, and `plans/2026-06-04-egsr-seam-and-route-roadmap.md` before acting. DB43/DB44 are accepted; DB45 is paused after DB45i reached the latest A100 executor (`/status` passed and one `/exec` job was submitted) but the remote runtime failed before official VGGT inference with `ModuleNotFoundError: No module named 'vggt'`. DB45e/f/g/h/i respectively accept only diagnostic/contract/blocked evidence and still accept no physical target-surface geometry evidence, no residual readiness, no source-faithful repair permission, and no RED promotion. DB45i has no `pose_enc`, decoded cameras, Sim(3), or target-surface residual table; do not patch-on-patch by ad hoc installing/rerunning under DB45i. DB47a-d are source/frame candidate-mining review/accounting gates only; DB47d reviews the 10 strict/relaxed same-log rows (`exact_review_candidate_not_final=3`, `missing_exact_hold=7`), selects no final candidate, and accepts no source-faithful repair. DB49a is `bosch-data-contract-inventory-only`; DB49b is `sidecar-starter-pack-partial-only`; DB49c is `source-id-map-feasibility-inventory-only`; DB49d is `source-map-instrumentation-only`, adding default-off `_seamroute.py` provenance sidecar export for future exact reruns but creating no DB32 source map. DB49 confirms DB32 `s40` can only be packaged as a caveated handoff candidate and remains not ready for uncaveated Bosch training-data use. DB28/DB41 camera labels are ROI diagnostics, not source ownership maps; DB49d instrumentation is future rerun support, not a recovered owner artifact. DB28/DB32 remain source-sidestep/current-handoff evidence only, not original-G seam repair or a source-faithful ceiling. DB25/DB41 remain `RED/abstain`, DB41 lower-right remains zero-LiDAR abstain, DB36/DB40 remain generated fake-geometry rejects.
