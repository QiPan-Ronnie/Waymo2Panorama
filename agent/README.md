...
> **DB47f update (2026-06-04/05):** DB47f preflight is accepted as `fixed-universe-exact-closure-preflight-only`, and DB56 has now closed the fixed exact-asset availability gap. The DB56 run produced/fetched all `15/15` required DB47f exact compare/final assets for the fixed 8 anchors under DB28 paths. This is source-selection exact evidence only: no final panorama is selected yet, no local repair/generation/source replacement/`source_id_map`/RED promotion occurred, and DB32/G/DB41/DB25 claim boundaries remain unchanged. Next DB47 action must be a fresh visual final-candidate review/hold/reject brief over the now-available assets.
