...
> **NOW: re-running with `--flow` (DIS optical-flow align in the band — the step that makes overlaps TRULY agree before blending) + the composite fix. Vision verdict appended when done. HONEST PRIOR: our E3 history says flow STARVES on textureless mid-range, so flow may help the textured sign but not the blank wall — watch for that.** Locations — GitHub committed; local `deliverables/a1_streetview_pipeline/`; Drive `results/a1_streetview_pipeline/`.
> ---
