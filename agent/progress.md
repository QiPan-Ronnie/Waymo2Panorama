...
> **Runtime result:** attempted the one allowed Colab executor source/API inspection, but the provided Cloudflare tunnel returned HTTP `530` at `/exec`; a direct `/status` check returned the same `530`. The script now catches submit-stage HTTP errors and writes a blocked manifest/board instead of leaving a traceback. This is an executor/tunnel availability blocker, not a VGGT source/API conclusion.
