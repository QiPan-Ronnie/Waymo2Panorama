...
> **Status:** ghost FIXED (single-source align, code-reviewed, 3-anchor clean). NEXT (exploring, per user "走到尽头"): the residual = large-parallax OCCLUDING objects (BMW) still abstain → L1 hard cut; research-backed next tools = graph-cut seam routing AROUND objects (cv2.detail, full-coverage this time) + LiDAR-depth foreground-select (we have real depth; Surround360/Jump use flow-mag as a depth proxy). **Locations:** GitHub committed; Drive `results/a1_streetview_pipeline/{,0bae,fbee}`.
> ---
