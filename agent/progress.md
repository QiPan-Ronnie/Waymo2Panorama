...
> **CORRECTION: the world map raised COVERAGE% but not REAL-NESS; coverage% is not a cleanliness proxy. Retracting the "hard case SOLVED" claim (commits b90090a/27f234c). Diagnosing: added a SELFOCC gate to worldbev → re-render with self-occ ON (reject hood) → measure how much coverage is REAL road vs hood smear (`evidenceAA_worldmap_selfocc_on_off.png`). The earlier Lever-1 41% may also be partly hood-inflated — re-examine.**
> ---
