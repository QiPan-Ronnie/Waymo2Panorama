...
> **RESULT [POS — GO to A1] (vision-checked, eyes agree with numbers):** Fit GROUND + **11 facade planes** to the BMW LiDAR sweep (98,981 pts, Δ9.8 ms). Mean seam-agreement NCC (the two overlapping cams agreeing in the supported seam band) / NCC on the near doubling band:
>   - **L1 rotation-only (single-center): 0.825 / 0.822**
>   - **COARSE-PLANE DIBR (new): 0.886 / 0.884** ← best
>   - per-pixel raw LiDAR (E2-style): 0.856 / 0.841
>   So the fitted plane aligns adjacent cameras clearly better than single-center, AND beats per-pixel LiDAR. **VISION** (`deliverables/a0_plane_dibr_probe/a0_hardselect_3modes.jpg` + `a0_seam_crops.jpg`): plane-DIBR keeps geometry clean & upright (straight lines, NO global warp, NO smear); per-pixel-E2 is visibly melty/smeared at both seams (re-confirms E2 NEG); plane ≈ L1 cleanliness but cameras agree better. **→ the variable E2 missed (robust coarse-plane vs noisy per-pixel) is REAL.**
> **Honest caveats → carry into A1:** (a) at these specific seams L1's doubling is only moderate, so the win is mainly the NCC/agreement gain + the no-smear-vs-E2 contrast, NOT yet a dramatic visible un-doubling — A1 must target the WORST near-doubling seam, measure seam-disparity reduction directly, and verify far field stays byte-clean (relative_warp ≈ 0). (b) one tiny stray white sliver from a bad convergence pixel — clean in A1.
> **VERDICT: H1 premise SUPPORTED → GO to A1** (build the full plane-DIBR ERP + graph-cut seam routing + measure disparity reduction + far-field relative_warp). Driver `scripts/phase3/run_a0_plane_dibr_probe.py`. Locations — **GitHub** committed (script + figs + this entry); **local** `deliverables/a0_plane_dibr_probe/`; **Drive** `results/a0_plane_dibr_probe/`.
> ---
