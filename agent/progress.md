...
> **NEXT (the genuine non-gen frontier = the user's point 1 "lay the ground ONCE in 3D, not per-frame"): Lever-2 = world-anchored robust multi-view FUSION — build ONE world ground map from all frames (each cell from its agreeing low-spread subset, moving-gated), render every frame from it = coherent + denoised. (The naive worldbev failed = all-frame median + flat ground; robust version untested.) Highway still only 41% clean after Lever-1, so Lever-2 fusion + accurate geometry is needed to push higher.**
> ---
