...
> ### 2026-06-25 ◆ DB-109 SELF-CHECK (user: "you can't say NO real ground — Fable-5/we had 20%+ real"): the user is right — ~23% genuine real EXISTS (self-occ ON); my "0%" was a short-window artifact
> **CORRECTING my own "0% / no real ground" over-statement: that was the WORLD-MAP self-occ-on (short 33-frame window 285-317 → no over-the-trunk 20-28m displacement views). The PER-FRAME whole-log candidates (disp 5-58m) DO find the non-self-occluded over-the-trunk views: highway a300 cap clean(spread<=14) with self-occ ON = **23%** (eye-verified full-res `evidenceBB_selfocc_on_vs_off_perframe.png`: genuine real near-field road + plate in the deep-blind centre, NOT hood). That 23% = the user's remembered "20%+" = what Fable-5 / the shipped good frames (which use self-occ ON by default) actually render. So genuine non-generative real ground EXISTS (~23% highway, more on textured/good scenes).**
> **The precise truth (no more conflation): cap = ~23% genuine real (near-field, self-occ ON) + the deep-blind CENTRE (directly below/behind ego, always self-occluded) which is generation-only. My error was conflating "deep-blind-centre = 0% real" with "the whole cap = 0% real". The self-occ-OFF Lever-1 (41%) and world-map (84%) ADDED hood smear on top of the genuine 23% — retract those extra % as fake. CORRECT non-gen = self-occ ON (genuine ~23% + honest hole); the good frames (bmw/crowd/clean, self-occ ON) are VALID. Redoing highway FLUX-refine on the self-occ-ON genuine-real base (not the hood base).**
> ---

> ### 2026-06-25 ◆ DB-109 ⚠️ HONEST CORRECTION — the world-map "84% / SOLVED" was an OVER-CLAIM (user caught it at full-res); deep nadir is HOOD-reflection smear, not real road
> **User looked at `GOODFRAME_hw_v4_worldmap_flux.png` at FULL RESOLUTION (not my brightened crops) and was right: the deep nadir is still a smooth featureless SMEAR (no real road texture/lane-lines), with black-square artifacts + white reflection smears. I had been fooled 3× by brightened crops + coverage numbers ([[feedback-vision-not-just-metrics]] — eyes over metrics, ALWAYS full-res).**
> **ROOT (the user's own suspicion, likely correct): "the car hood is smeared into the ground". The world map ran with self-occ OFF (Lever-1) + robust averaging → the HOOD-skimming grazing views (probe: ~100% of deep-nadir candidates are self-occluded = ray skims own hood) get averaged in → smooth grey hood-reflection smear, NOT real road. So the "84% coverage" is INFLATED by hood; the genuine deep-nadir real road is still absent (physical grazing-blind). Black squares = fusion/FLUX artifacts; white smears = reflection.**
> **CORRECTION: the world map raised COVERAGE% but not REAL-NESS; coverage% is not a cleanliness proxy. Retracting the "hard case SOLVED" claim (commits b90090a/27f234c).**
> **★ DEFINITIVE DIAGNOSTIC (`evidenceAA_worldmap_selfocc_on_off.png`, eye-verified FULL-RES): world map with SELFOCC=ON (reject hood-skimming views) → highway a300 nadir cap onok% = **0.0%** (was 84% with self-occ OFF). ⇒ 100% of the deep-nadir "coverage" was HOOD sky-reflection smear; the genuine non-self-occluded real road in the deep nadir cap = ZERO. The user was right at every step ("hood smeared into the ground").**
> **DURABLE TRUTH (physical, now rigorously proven — supersedes any "non-gen can recover the deep nadir"): the deep inner nadir (directly below/behind the ego) is ALWAYS self-occluded by the ego's own hood/trunk (front-pod rig) → no clean real-road view exists in ANY frame. Non-generative is at the PHYSICAL FLOOR there (not "unfinished"). The funnel's "geometry-blind=0%" was about FOV-hit; the SELF-OCC wall is real for the deep nadir. CORRECT ARCHITECTURE: (1) reject hood (SELFOCC ON) → genuine real NEAR-FIELD (directly-seen road below the horizon) + honest deep-nadir HOLE; (2) the deep-nadir hole MUST be GENERATED (no real data) — and for temporal coherence that is the DOWNSTREAM VIDEO model (Cosmos), not single-frame FLUX/DiT360 (which blur/flicker). The faithful-base/Lever-1/world-map "coverage" of the deep nadir was all hood; the genuine non-gen contribution is the near-field only.**
> **LESSON (4th over-claim caught by the user): NEVER trust coverage% or brightened crops — eyeball EVERY result at full native resolution before any "clean/solved" claim. [[feedback-vision-not-just-metrics]].**
> ---

> ### 2026-06-25 ◆ DB-109 TWO-TRACK BUILD (user locked direction): Track-B FLUX-refine fixes blur (WIN); Track-A 3D world-map ("lay once") built + validating
> **Track-B WIN (`evidenceV_fluxrefine_vs_blend.png`, `GOODFRAME_hw_v3_fluxrefine.png`): the blur was FLUX synthesizing smooth from a BLANK hole + the alpha-blend averaging. FIX = don't inpaint blank; feed FLUX the REAL (quilty) reprojection as the init + low-strength img2img (FluxImg2ImgPipeline, FLUX.1-dev, strength=0.45) → it CLEANS the quilt + KEEPS real lane-lines/curb/texture, SHARP not smooth. strength 0.65 over-hallucinates (gravel from the "aggregate" prompt). Track-B recipe locked: FLUX img2img refine the real @ ~0.45, guidance 3.5, drop the "aggregate" prompt word. (FLUX-fill mechanism = diffusion + prompt + surrounding-image conditioning, a dedicated inpainter; DiT360 = FLUX panorama LoRA via inversion. Both prompt-driven; FLUX-fill respects surroundings more.)**
> **Track-A BUILD (the user's point-1 "lay the ground ONCE in 3D", db89 `worldbev` improved, 3 edits): moving-box gate + per-frame PHOTOMETRIC normalization (kills the exposure drift that broke naive accumulation) + robust tight-cluster FUSION (mean of agreeing samples, not pick-one).**
> **★ Track-A RESULT = BREAKTHROUGH (`evidenceW_worldmap_3frames.png`, `evidenceX_worldmap_vs_perframe.png`): highway nadir real coverage 22% (baseline) → 41% (Lever-1) → **84-88% (world map)**; world-map ok_pct 91.5% (vs naive worldbev's 74%). The photo-norm + robust fusion + whole-window accumulation OVERCAME the exposure-drift wall. The 3 frames (a299/300/301) from the SAME map are STRUCTURALLY CONSISTENT (temporal coherence by construction — swirl killed). ⇒ the user's "lay once in 3D" is VALIDATED and strong.**
> **Residual defects (polish): deep-nadir BLUE Fresnel tint (grazing sky-reflection baked into the fused map; truth-ring white-balance `evidenceY_worldmap_wb.png` only half-fixes) + mild waviness (grazing misregistration). NEXT: feed the 84% coherent world-map real into Track-B FLUX img2img refine (low strength) → clean the tint+waviness + fill the residual 16% = the two tracks COMBINED (high coverage + clean + faithful + coherent).** (user: "you may be in a local optimum"): candidate-realness probe + FLUX+real blend + Lever-1 self-occ test
> **User reframed: the ground should be laid ONCE in 3D world (not redrawn per-frame at the ERP pole) — confirmed as the core local-optimum. And asked to trace the ACTUAL pipeline: for bad frame a300, which candidates B/C/D/E/F feed each ground point, and are their captures REAL?**
> **Probe (`evidenceP_probe_candidate_realness.png` + `probe_a300.json`, new `GROUND_MODE="probe"` in db89): for 6 bad ground points, each is IN-VIEW of 28-41 candidate (frame,cam) pairs — NOT "never seen". The back-projected crops show REAL road (some with real lane-markings: yellow line, white arrows). BUT every view is LOW-GRAZING (7-15°) AND clear(non-occluded)=0-1 per point (mostly self-occlusion = ray skims the source ego's own hood/trunk) AND they DISAGREE (different stretch/exposure → the quilt). ⇒ the answer to "is it never captured?": NO — captured many times as real road, but always grazing + self-occ-flagged + inconsistent. "Seen poorly + inconsistently", not "never seen".**
> **FLUX-fill datapoint: SOTA inpaint, CLEANER than DiT360 on the hard nadir (smooth plausible asphalt, no splotches/red blobs) — `evidenceO`. Still prompt-dependent + single-frame (no temporal coherence) + generic (no real lane-lines).**
> **FLUX + REAL confidence-blend (user idea, `evidenceQ_fluxreal_blend.png`, `GOODFRAME_hw_fluxreal_blend.png`): blend real×alpha + flux×(1-alpha), alpha=1 for spread<=14 → 0 at spread>=30. ARCHITECTURALLY RIGHT + general: real where trusted + clean-fill where not. On good scenes (bmw 97%) → ~all real. On highway the deep nadir is mostly FLUX because the real there is genuinely low-confidence (probe-proven). Modest win on highway; great on good scenes.**
> **⇒ Two unexplored NON-GENERATIVE levers (the real frontier): (1) refine the self-occ gate; (2) multi-view robust FUSION + accurate ground geometry. Both pure geometry.**
> **LEVER-1 RESULT (`evidenceR_selfocc_on_vs_off.png`, `SELFOCC` flag): turning the self-occ gate OFF raised highway a300 CLEAN (spread<=14) nadir coverage 23% → 41% (nearly 2×), any=72% unchanged. Objective evidence it's REAL ROAD not hood: the recovered views AGREE (raise spread<=14) — hood sky-reflection (per-frame-varying sky) would DISAGREE; visual shows continuous road + lane-line, not bright/blue hood smear. ⇒ the shipped self-occ two-box gate is OVER-CONSERVATIVE, leaving ~18% clean real coverage on the table on hard frames.**
> **Blend v2 (`GOODFRAME_hw_v2_selfocc_fluxreal.png`, `evidenceS_blend_v1_vs_v2.png`): self-occ-off 41% real + FLUX confidence-blend = MORE faithful highway (more real road/lane-line, still clean) than v1 (23% real). Modest but real gain in the right direction (more real, less generic FLUX).**
> **CAUTION (memory v3j): self-occ-OFF was tuned to stop downtown hood pollution.**
> **Lever-1 GENERALITY ✅ COMPLETE (verified, `evidenceT` bmw/clean + `evidenceU_lever1_downtown.png`): self-occ ON vs OFF clean(s<=14) coverage UNCHANGED + visually IDENTICAL on bmw 20/20, clean 28/28, downtown 22/22, dt70 22/22 — NO hood pollution, NO regression even on downtown (the memory v3j feared case: real yellow curb-line + sidewalk preserved, no bright/blue hood smear). The gate ONLY over-rejects on STARVED hard frames (highway +18% real). ⇒ DURABLE: the shipped self-occ two-box is OVER-CONSERVATIVE; it can be safely RELAXED/removed to recover real road on hard frames with zero good-scene cost. A clean non-generative win — confirms the user's "non-generative not exhausted".**
> **LEVER-2 PROBE = NEGATIVE (important): wide window (92 frames) + selfocc-off → highway a300 clean(s<=14) = 16%, LOWER than Lever-1's 41% (bucketed+selfocc-off). MORE views HURTS — larger time span = exposure/lighting DRIFT (memory's "±15s auto-exposure drift → lavender wash") → same ground disagrees across time → higher spread → less clean. ⇒ naive "accumulate more views" does NOT work; the shipped BUCKETED candidate design (3 time-NEAREST per displacement bucket) already optimally avoids drift. Lever-1 (relax self-occ on the bucketed set) = 41% = the per-frame non-generative BEST.**
> **NON-GENERATIVE CEILING (honest, the answer to "is non-gen exhausted"): NOT exhausted — Lever-1 recovered 22→41% on hard highway (verified safe). But the hard-frame ceiling is ~41% clean: beyond it needs robust photometric-normalized fusion (untested, uncertain) to beat exposure drift + the 7-15° grazing physics. PRACTICAL BEST per the evidence = Lever-1 faithful base (97% good scenes / 41% hard) + FLUX+real confidence-blend (real where trusted + clean-fill the rest). Good scenes ≈ fully real; hard scenes = mostly real + clean fill. Robust world-fusion (per-frame exposure-normalize + tight-cluster) is the one remaining unexplored lever — a bigger build, worth co-deciding with the user.**
> ---
...
> ### 2026-06-24 ◆ DB-109 Track1: DiT360 RUNS on A100 (360-native nadir fill) — v1 worked but had VAE-tiling block artifacts; v2 re-run (tiling off + hard composite)
> **DiT360 v1 (`dit360_a300_nadir.png` + `_panel.jpg`): the pipeline WORKS — FLUX.1-dev + DiT360 LoRA load offline from Drive cache (weight_name=`adapter_model.safetensors` fix), invert + personalize-attn regen, model-load 40s + gen 185s, preserve 71.5% / generate 28.5%. Upper scene preserved; the nadir HOLE was generated 360-natively (no ERP-pole distortion — unlike SDXL). BUT the generated nadir has square BLOCK artifacts (VAE-tiling decode seams; `vae_tiling=true`) + the preserved region was softly altered (preserve_psnr 26).**
> **v2 fixes (re-running): `--disable-vae-tiling` (A100 80GB has the memory → kills the block seams) + a HARD COMPOSITE post-step (keep the faithful base EXACT, paste DiT360 only inside the hole) → `dit360_a300_composited.png`. This is the proper "faithful base + 360-native generative fill". Then eyeball: clean asphalt in the hole? lanes continuous? no pole artifact?**
> ---

> ### 2026-06-24 ◆ DB-109 scene-coverage SPECTRUM (answers "how much can good scenes restore") + DiT360 hit a fixable offline-LoRA error
> **5-scene faithful real% at spread14 (`evidenceK_scene_coverage_survey.png`, eye-verified): bmw 96.8% (full clean asphalt+lanes) > crowd 64.3% (mostly real road, some cars) > clean 35.2% (partial) > highway 22.0% (dusk) > downtown 0.0% (ego idling at a light → no ego displacement → no candidate frames). ⇒ the faithful method's CEILING is HIGH (≈97%); coverage tracks scene conditions (brightness + ego motion + traffic), NOT a method failure. Good scenes already give near-perfect ground; the generative fill is only needed for the hard low-coverage frames. (downtown 0% = a candidate-window/idle edge case worth a separate look — fixed window may need an idle fallback.)**
> **DiT360 (A100) ran but errored: `load_lora_weights(...)` in HF offline mode needs an explicit `weight_name` (`ValueError: When using the offline mode, you must specify a weight_name`). Fix: read the cached LoRA `.safetensors` filename + sed it into the runner (offline-safe), OR use the now-provided HF token online. Re-running.**
> ---

> ### 2026-06-24 ◆ DB-109 2-track results: Track2 confirms faithful coverage is CONDITION-DEPENDENT (bmw 96.8% vs dusk-hw 22%); Track1 DiT360 launched on A100 (all cached, offline)
> **Track2 combine (`evidenceJ_combine_bmw_hw_plate_inpaint.png`): faithful real% at spread14 = bmw a046 **96.8%** (bright daytime → NEAR-COMPLETE clean ground, lane-lines + crosswalk), highway a300 **22.0%** (dusk + traffic = the hard case). ⇒ the faithful method is NOT "broken / not dug to the end" — it is CONDITION-DEPENDENT: good scenes are already near-perfect (bmw IS a good frame), only the hard dusk/traffic frames are holey. The combine (faithful base + Fable-5 NS-inpaint) on highway fills the hole but SMEARY (the 白团); on bmw it is moot. ⇒ deliverable strategy: faithful-alone for good scenes + generative fill (DiT360) for the hard holes.**
> **Track2-ext (user asked): the 22% is the HARD highway — survey how much the faithful method restores on OTHER scenes. Launched `db109_scene_survey.py` on L4 (clean/crowd/downtown a046 @ spread14) → 5-scene coverage table `evidenceK` (with bmw 96.8% + highway 22%). User also provided an HF token → stored non-repo `~/.waymo2panorama/runtime/hf_token.txt` (NEVER commit) → unblocks FLUX-fill / gated models on the A100 (DiT360 already runs offline).**
> **Track1 DiT360 (A100): EVERYTHING is cached on Drive — `external/DiT360` repo (pa_src) + `cache/huggingface/hub/models--black-forest-labs--FLUX.1-dev` + `...--Insta360-Research--DiT360-Panorama-Image-Generation` LoRA. Run OFFLINE (HF_HOME=Drive cache, HF_HUB_OFFLINE=1) → no token/gating. Launched `db109_dit360_a100.py` on highway a300 (the hard 22% case): init = faithful base, mask = 255-faithfill (white preserve / black generate), tau=50, 2048×1024 native ERP, FLUX.1-dev + DiT360 LoRA. DiT360 is 360-native → no pole distortion. ~20-30 min (FLUX load + invert + gen). Output → `dit360_a300_nadir.png` + panel.**
> ---

> ### 2026-06-24 ◆ DB-109 → 2 TRACKS (user co-decided, gave an A100): Track1 = A100 generative (DiT360/FLUX-fill/Cosmos); Track2 = principle dig (faithful + Fable-5 combine). Git pushed `532aa05`.
> **User split the work after seeing the faithful base: (1) A100 generative fill — try DiT360 / FLUX-fill / Cosmos; (2) keep digging the PRINCIPLE — the bmw frame's ground looked good, so why is bmw (bright daytime) full but highway (dusk) holey, and can we COMBINE this session's faithful base with last session's Fable-5? "this road maybe isn't dug to the end". No video yet — get a few GOOD frames first.**
> **Dual-machine: `active_url.json` (L4) + `active_url_a100.json` (A100 80GB), both reachable; `ColabClient` with `.url/.token` override targets each (drivers read the per-machine json).**
> **Git: committed + pushed the whole DB-109 architecture work to main (`532aa05`, 56 files: db89 gated flags + progress + decision_briefs + SUMMARY.md + evidence A-G). Scrubbed a dead-tunnel hostname from an old progress line first (public repo).**
> **Track2 launched (L4, `db109_combine.py`): 2x2 = bmw a046 / highway a300 × plate (pure faithful) / inpaint (faithful base + Fable-5 NS-fill of the holes) → real% + eyeball → `evidenceJ`. (Earlier r3 finding: bmw coherent coverage was 100% = bright scenes near-full; this quantifies bmw-vs-highway at spread14 + tests whether NS-inpaint plausibly fills the dusk holes.)**
> **Track1 probe (A100): 81 GB free; torch 2.11/diffusers 0.38; HF_TOKEN UNSET → FLUX.1-Fill-dev is gated/blocked WITHOUT a token; BUT Drive has `db96_fluxfill`, `cache/huggingface`, and EXTENSIVE prior DiT360 infra (`dit360_seam_completion/oracle/outpaint_v2/...`). No Cosmos cache.**
> **Track1 RECIPE FOUND (`run_koi_outpaint_colab.sh` + `run_dit360_mask_batch.py` / `run_dit360_trimap_clamp.py`): DiT360 = `RFPanoInversionParallelFluxPipeline` (FLUX.1-dev + LoRA `Insta360-Research/DiT360-Panorama-Image-Generation`), run OFFLINE via `HF_HOME=/content/hf_local` + `HF_HUB_OFFLINE=1` (uses cached FLUX → no token/gating), `--dit360-root /content/DiT360`, native ERP 2048×1024, tau personalize-attn. MASK CONVENTION = white/255 PRESERVES source, black/0 GENERATES (so the nadir fill mask = INVERT of faithfill_mask). DiT360 is 360-NATIVE → no ERP-pole distortion (the thing that broke vanilla SDXL). This is the right Track-1 tool. NEXT: probe2 to locate the DiT360 repo + FLUX cache on Drive, then a setup+run driver on the A100 (init=faithful base a300, mask=inverted faithfill).**
> ---

> ### 2026-06-24 ◆ DB-109 A /loop — CONSOLIDATED (SUMMARY.md); faithful-base video re-attached through a tunnel 502/530 blip (Drive MCP confirmed runtime alive + job running)
> **Tunnel hiccup handled cleanly: the 33-frame faithvideo job (d6f91e8d) submitted fine but the local poll hit a transient cloudflare 502→530. Drive MCP (bypasses the tunnel) confirmed the runtime is ALIVE and the job RUNNING (fv14_a285/286/287 segs landing on Drive ~2 min/frame, active_url.json fresh @22:24). Did NOT resubmit (avoid double-GPU); re-attached with a TOLERANT poll/fetch (`db109_fv_attach.py`, rides out tunnel blips) → builds `faithbase_full_h264.mp4` + nadir mp4 + montage + `evidenceH_oldquilt_vs_faithbase.png` on completion.**
> **CONSOLIDATION: wrote `deliverables/db109_ground_rootcause/SUMMARY.md` — one-page root-cause→architecture→faithful-base→generative-findings→next-codecide, with the evidence{A..H} index. State: the faithful half is SOLVED + clean (spread14 base + verified faithfill_mask = Cosmos-ready); the generative fill is the downstream Cosmos/DiT360 stage (co-decide: model + GPU + DB-94 contract). Recommendation: route the faithfill_mask holes to Cosmos (gives video coherence that no per-frame 2D inpaint can).**
> ---

> ### 2026-06-24 ◆ DB-109 A /loop — BEV-domain SDXL inpaint ALSO fails (large hole left black); generative fill CONFIRMED = downstream Cosmos/DiT360 (A100), not L4-vanilla. Converging on the faithful-base deliverable.
> **BEV-domain inpaint (`evidenceG_bevfill_a300.png` + `evidenceG_bevfill_bright_2x.png`): the BEV faithful raster is GOOD (mostly real top-down asphalt + yellow lane-line, only ~14% holes — far better coverage than the ERP cap's inner 22% because the 24 m BEV tile includes well-seen mid-range road), BUT SDXL left the main occlusion hole BLACK (stock SDXL degenerates on a large hard-edged mask). ⇒ vanilla SDXL inpaint FAILS in BOTH domains: ERP = pole-distorted streaks, BEV = large-hole degenerate.**
> **CONCLUSION (recorded, honest): the generative fill is genuinely the DOWNSTREAM stage — a 360/video-aware model (Cosmos / DiT360, A100-class), NOT cheaply solvable on the L4 with stock inpainting. This is a co-decide for the user (model + GPU + the DB-94 Cosmos contract). The BEV finding is a SILVER LINING: the world-anchored BEV is the right SUBSTRATE (clean top-down asphalt, temporally coherent by construction, only ~14% holes) — a world-BEV + proper generative hole-fill, then ERP reprojection, is the clean coherent path (future DB-14).**
> **Converging (autonomous): rendering the faithful-base (spread14) + faithfill_mask over the loop2 window a285-317 (33 frames) = the concrete Cosmos-ready deliverable + a before/after vs the loop2 quilt; then a consolidated summary for the user.**
> ---

> ### 2026-06-24 ◆ DB-109 A /loop — vanilla SDXL ERP-inpaint = NEGATIVE (streaky/pole-distorted); BEV-domain inpaint (the right fix) launched
> **SDXL ERP-inpaint on the a300 faithfill_mask (`evidenceF_base_vs_genfill_a300.png`): the fill came out STREAKY + pole-distorted, NOT clean asphalt — a vanilla 2D inpainter fights the ERP nadir-pole geometry (and is per-frame → would re-swirl). CLEAN NEGATIVE: do not pursue naive ERP inpaint. (Local cp1252 print crash on the diffusers log was cosmetic; remote job exit 0, files fetched.)**
> **Fix (launched): inpaint in the UNDISTORTED top-down BEV domain (flat asphalt = SDXL-friendly, no pole distortion), then reproject to ERP. Added a gated `bevraster`/`bevmask` export to db89's `bev` branch (~:1323, fires under FAITH_MASK). Testing a300: render GROUND_MODE=bev + spread14 → SDXL-inpaint the BEV raster → is the BEV-domain fill CLEAN? Output `evidenceG_bevfill_a300.png`.**
> ---

> ### 2026-06-24 ◆ DB-109 A /loop — faithfill_mask VERIFIED; L4 generative feasibility checked; SDXL-inpaint end-to-end DEMO launched
> **faithfill_mask (a298-303 @ spread14, `evidenceE_faithbase_mask_overlay.png`): the mask = the deep-nadir abstain fan + center cap (fill_frac 0.51-0.57 of the nadir half); the upper near-ground road + lane-lines are KEPT (black) — the mask does NOT eat faithful pixels. faithful-base + mask = the Cosmos-ready artifact, verified.**
> **L4 generative feasibility (queried): 22.5 GB free, diffusers 0.38 / torch 2.11 / transformers / accelerate ALL present, but NO FLUX/DiT360 weights cached anywhere (Drive + HF). ⇒ FLUX-fill (24 GB) = OOM on L4; SDXL-inpaint (~7 GB) = FITS. Proper 360-aware + temporally-coherent generation (DiT360 [FLUX-based] / Cosmos video) needs A100 + is the downstream DB-14 / DB-94 stage.**
> **Launched an illustrative SDXL-inpaint DEMO on a300 (single frame, spatial only) so the user can SEE the end-to-end faithful-base + generative-fill ground. HONEST CAVEAT recorded in the artifact label: vanilla 2D inpaint is NOT ERP-pole-aware nor temporally coherent — it informs the generative-stage choice, it is NOT a shipped solution. Output: `evidenceF_base_vs_genfill_a300.png`.**
> ---

> ### 2026-06-24 ◆ DB-109 A /loop (user resting, autonomous) — faithful-base gate LOCKED at SPREAD_MAX=14; spread18 reintroduces quilt-creep; faithfill_mask export added
> **Gate A/B (a298-303 × spread 14 vs 18, `evidenceD_faithbase_14_vs_18.png` + `evidenceD_grid_14_18_x_a300_a302_2x.png`):** 14 = clean road + lane-lines, the mediocre high-spread fan abstains to smooth plate (meanLap 12.7, plateFrac 0.51); 18 buys coverage but the radial QUILT strips creep back (meanLap 14.0 = quilt NOT clean road, plateFrac 0.43). Per-frame Laplacian decreases SMOOTHLY a298→a303 in both (14.0→11.7 / 14.8→13.3) = tracks the scene, no random flicker → the faithful base is temporally COHERENT.
> **DECISION (autonomous, recorded): lock SPREAD_MAX=14 — precision (clean real pixels) over recall (coverage), because the abstain region IS the generative-fill region; a returning quilt at 18 would feed BAD pixels into the generative conditioning. Cap ≈ 22% faithful-real + 78% honest hole for the downstream coherent generative fill.**
> **Added gated `FAITH_MASK` flag (db89 ~:64 + ~:1658) → exports `{run}_faithfill_mask.png` (cap abstain ∪ fg-occ = the generative region). NEXT: render a298-303 @ spread14 + FAITH_MASK to VERIFY the mask = exactly the abstain fan (not the clean road), produce the faithful-base+mask artifact; then assess a generative-fill prototype (DiT360/FLUX) on one mask.**
> ---

> ### 2026-06-24 ◆ DB-109 Evidence-B+C + DIRECTION LOCKED (user chose A): the 格子 is NOT selection-fixable; the nadir ground is only ~6–22% faithfully-recoverable per traffic frame → ARCHITECTURE = faithful spread-gated base + generative fill
> **Evidence-B (`COHERENT_PICK="agree"`, argmin-to-median over the egod-sweet slots, a300):** mean-abs pixel change vs sweet = ONLY **2.66/255**; Laplacian 16.0→16.4 (NOT smoother); quilt visually unchanged (`evidenceB_sweet_vs_agree.png`, `evidenceB_crop_*`). ⇒ the 格子 is NOT caused by the source PICK — the selection layer is EXHAUSTED. (New gated flag `COHERENT_PICK`, default `"sweet"` = shipped behaviour unchanged.)
> **What the 格子 actually IS (hi-res crop):** radial grazing-STRETCH strips + GENUINE source disagreement (spread 15–30) in the red high-spread fan — NO single source is clean there, so no pick can help.
> **Evidence-C (SPREAD_MAX sweep 8/14/30, a300, `evidenceC_*`):** faithful real-render coverage = **6.6% @≤8, 22% @≤14, ~92% @≤30**. At ≤14 the road + white lane-lines render CLEAN (quilt gone), the rest = honest plate. ⇒ **HARD PHYSICAL FACT: on a traffic frame only ~6–22% of the nadir cap is faithfully recoverable; "full + clean" is IMPOSSIBLE from pure reprojection** (blend / world-accum fight physics: low ceiling + blur risk).
> **DIRECTION LOCKED (user co-decided A): faithful pristine base (spread-gated, clean, deterministic-coherent) + GENERATIVE fill (DiT/Cosmos = DB-14) for the honest holes.** Resolves ②quilt (abstain the mediocre) + ①center-cap + ③scene-band boundary (generative) + swirl (deterministic + pristine). Matches the user's own "Cosmos regenerates appearance" framing + the GENERAL north-star. **CAVEAT: the generative fill MUST be temporally coherent (video model) or the filled hole re-swirls.** The old "fill the cap to 100% via reprojection" goal is RETIRED.
> **NEXT (building): pick the spread gate (14 vs 18 across the a298–303 strip = gate A/B + temporal-coherence montage), then export faithful-base + abstain-mask as the clean deliverable / generative input.**
> ---

> ### 2026-06-24 ◆ DB-109 Evidence-A (user re-engaged on the loop2 video: 车机盖子 + 地面格子化 + scene-band 边界失真) — the 格子/tiling ROOT-CAUSED to per-pixel source-LABEL fragmentation + borderline spread; user's "旧版偶尔有还原好的地面" QUANTIFIED; direction = best-agreement DETERMINISTIC pick (NOT single-frame blend, NOT a world grid)
> **Method: 2× 1-frame diag renders on a300 highway, `GROUND_MODE="diag"` (added a per-cap-px winning-source-frame label dump at db89 ~:1567), label=which frame won + spread=source agreement.** Evidence in `deliverables/db109_ground_rootcause/evidenceA_{evAcoh,evAold}_seg_label_spread.png` (3 layers: seg / label / spread).
> **evAcoh (current B-coherent, COHERENT egod-sweet):** coverage 100%, **27 unique source frames quilt the cap**, **spread_med 19.8** (borderline vs the 30 cutoff). The visible 格子 = (1) the label's ~27 arc-BANDS (egod=22m iso-distance arcs) mis-registered at band edges + (2) the spread>30 RED radial streaks punched to plate + (3) per-frame grazing stretch.
> **evAold (Fable-5 path: bucketed cand + argmin-to-median + MOVING on):** coverage **only 5.8%** (a300 has a lead truck → moving-gate kills the forward ground), but where it writes, **spread_med 1.6 = pristine agreement**.
> **=> user's "旧版偶尔有还原好的帧" is EXACTLY the OLD 5.8% pristine patches (spread 1.6 = genuinely restored road); the other 94% was NS-inpaint=swirl. NEW buys 100% coverage by accepting spread→30 borderline sources = uniformly mediocre quilt, but no swirl. SAME fidelity↔smoothness axis, opposite ends. Info IS there (geometry-blind still 0) — it's a SELECTION problem.**
> **HONEST WALK-BACK: my first-pass "salt-pepper label" guess was WRONG — the label is large arc-BANDS, not high-freq speckle (middle layer). The fragmentation is real but its geometry is arc-bands + spread-streaks.**
> **Root cause (proven): the quilt = COHERENT picks the egod-SWEET single source (spread 19.8) NOT the BEST-AGREEING source. KEY: in a FIXED window, argmin-to-median is ALSO a deterministic function of the world point (fixed candidates → fixed median → fixed winner) → can get best-agreement (kills 格子) AND temporal determinism (kills swirl) AT ONCE, WITHOUT a world grid (dodges B-grid's discretisation loss → consistent with why B-coherence beat B-grid).**
> **CAVEAT (must eye-test, not assume): prior loop-r1 found "argmin baseline = blurred". But that argmin'd over egod-NEAR slots (grazing-diverse 5-28m → flat). COHERENT-v2 argmins over the egod-SWEET slots (uniform ~22m grazing) → different composition; EYE-test required.**
> **NEXT (building): `COHERENT_PICK="agree"` flag (argmin-to-median over the sweet slots) → render a300, eyeball vs evAcoh. If 格子 gone w/o blur → neighbours (coherence) + scenes; THEN center-cap①(plate/generative) + scene-band boundary③ separately (one fix at a time).**
> ---

> ### 2026-06-24 ◆ DB-109 /loop r3 — bmw coherent generality PASS (real road + lanes, no car-hallucination); clean/crowd UNVERIFIED (L4 cloudflare tunnel went 530/down mid-fetch); runtime process alive but tunnel dead
> **5-scene generality (one coherent frame each): bmw a046 = coverage 100%, eye-verified (`cohbmw_a046_seg_nadir.png`): real asphalt + white lane-lines/crosswalk fill the cap, car in the scene band, NO car-body smear — coherent GENERALISES to bmw (a bright daytime scene). clean/crowd jobs returned done exit 0 but produced NO segcomposite/manifest, then the tunnel went 530.**
> **INFRA HALT: the L4 cloudflare tunnel is DOWN (530 ×3, persistent).** Drive's latest `active_url.json` (read via the Drive MCP) = SAME url `[dead-tunnel-host]` + same token, uptime 9908s → the runtime PROCESS was alive (still writing Drive) but the cloudflared QUICK TUNNEL died WITHOUT rotating the URL (no new endpoint to self-heal to). Cannot run jobs until the tunnel/runtime is restarted (user re-runs the Colab setup cell on wake). Local + Drive endpoints both = the dead URL.
> **Loop state (all committed): highway continuous-frame coherent validated at 12 + 33 frames (coverage 100%, montage consistent, fidelity > baseline); bmw generality PASS. PENDING when L4 is back: re-verify clean/crowd, then the full highway-video deliverable. A periodic tunnel-recovery check is scheduled.**
> **Wake-check 2 (CORRECTION): the runtime is ALIVE — `list_recent_files` bypassed search's index-lag and shows newest active_url.json @14:12, uptime 5.1h, `active_jobs:2` (= clean/crowd still running). But the cloudflared tunnel is DEAD (DNS getaddrinfo-fail ×3) and the URL did NOT rotate → runtime is alive-but-unreachable, no new endpoint to self-heal to. Drive MCP bypasses the tunnel (can fetch job outputs once written) but clean/crowd haven't written segcomposite yet. Full-video deliverable needs the tunnel (new /exec) → waiting for the user to restart cloudflared. Re-checking in 30 min.**
> ---
...
> **Round 2 launched:** 33 continuous frames a285-317 coherent → montage + mp4 (toward the full highway window 225-317 + stronger continuous-frame evidence). ~55 min.
> ---

> ### 2026-06-24 ◆ DB-109 /loop round 1 — 12-frame coherent (a303-314): coverage 100% real road, montage CONSISTENT, slit relatively STABLE (not baseline's swirl); baseline A/B rendering for the decisive proof
> **loop1: 12 continuous coherent frames a303-314 in ONE job (21 min, ALL coverage 100%, residual_inpaint low ~52k).** Eye (`loop1_nadir_montage.png`): 12 nadirs are real asphalt + lane-lines, the car moves smoothly, the road pattern is CONSISTENT frame-to-frame — no per-frame random swirl. Slit-scan (`loop1_slit_center.png`, center column × 12 frames): the nadir band is relatively STABLE (smooth grey road + mild vertical texture, NOT the violent baseline swirl); scene-band horizon stable.
> **So continuous-frame coherent looks real + coherent.** To PROVE the swirl is gone (not just "looks ok"), rendering BASELINE (per-anchor argmin, MOVING off) on the SAME 12 frames → slit A/B (`loopb_slit_center.png` vs `loop1_slit_center.png`). Remaining toward "perfect": mild slit-nadir vertical texture (possible per-anchor `gn_glob` tone wobble or pick-boundary jumps) + dusk-dark scene + ERP-pole radial. Evidence: `loop1_{nadir_montage,slit_center}.png` + `loop1_{full,nadir}.mp4`.
> ---

> ### 2026-06-24 ◆ DB-109 B-coherence — AUTONOMOUS /loop STARTED (user resting, L4 left on): render CONTINUOUS frames, iterate toward a "near-perfect ground outpainting"
> **User invoked /loop: "做一段连续帧, 看连续帧加持下能否得到比较完美的地面 outpainting, 向这个方向靠拢. 我准备休息了, L4 一直开着, 你保持跟进."** Mode = autonomous loop, each tick: render a continuous coherent window → eyeball (swirl + perfection: coverage / texture / tone-consistency / radial-pole / car-body) → diagnose the worst remaining defect → one improvement → commit → next tick. GROUND_MODE="fill" + COHERENT=True + MOVING_GATE=False + COHERENT_WIN fixed so neighbours share candidates. Discipline unchanged: write file → Read-verify → eyeball every image → progress+commit; never commit url/token. Round 1 = a303-a314 (12 frames) coherent → nadir montage + mp4 to judge swirl + perfection.
> ---

> ### 2026-06-24 ◆ DB-109 B-coherence baseline A/B — coherent's COVERAGE + TEXTURE clearly beat baseline (egod-sweet > argmin); but 3-frame montage is NOT a sensitive swirl test (honest walk-back) — strict swirl proof needs a continuous slit-scan / video
> **Ran baseline (nomove: MOVING off, COHERENT off = per-anchor argmin) on the SAME a308/309/310 for an A/B montage.** Eye (`nom_temporal_montage.png` vs `coh_temporal_montage.png`): **coherent's nadir is clearly RICHER — more real asphalt + radial lane-line texture; baseline is darker/flatter/less textured.** So the egod-sweet deterministic pick also picks a SHARPER source than argmin-spread (argmin pulls toward the multi-source median = softer). **HONEST: the 3-frame montage does NOT decisively show the swirl difference** — both look statically consistent across 3 frames (3 frames too few + ego motion shifts the whole nadir; swirl is a temporal high-freq jitter needing a continuous slit-scan or rendered video). My earlier "3-frame looks coherent" was over-optimistic as a swirl proof.
> **State:** B-coherence's COVERAGE + texture/clarity win is SOLID (real road fills the cap, beats baseline + worldbev); the TEMPORAL-coherence claim is plausible but NOT yet strictly proven. Decisive swirl test = render a continuous window (e.g. a300-a317) coherent vs baseline → slit-scan (same azimuth × frames) or video. Evidence: `nom_*`, `coh_*`.
> **NEXT (user co-decide):** (a) strict swirl proof (continuous slit-scan / short video, heavier); (b) 5-scene regression first; (c) pause + digest.
> ---

> ### 2026-06-24 ◆ DB-109 B-coherence IMPLEMENTED + 3-frame test — coverage RESTORED (real road fills the cap, vs worldbev's 6.2%) + nadir looks temporally COHERENT across a308/309/310
> **Implemented `COHERENT` (gated fill variant, default off, 5 small edits): KEEP the per-pixel cap reprojection (the 81.9% MOVING_GATE=False path) but make source selection WORLD-DETERMINISTIC — FIXED-window candidates (`COHERENT_WIN`, not anchor-relative) + rank by |egod − 22 m sweet-spot| (a deterministic world-point function, NOT per-anchor argmin-spread) + render slot-0 + nvalid≥2 guard. Use with MOVING_GATE=False. Ran a308/309/310, window 225-317.**
> **RESULT — coverage RESTORED + looks coherent:** all 3 frames `coverage 100% filled, cand_frames 84, low_coverage_warning=FALSE` (vs worldbev 6.2% + warning). Eye (`coh_a309_seg_nadir.png`): the cap fills with REAL asphalt + lane-lines + texture (vs worldbev's gray); car stays in the scene band, NO car-body smear. Temporal montage (`coh_temporal_montage.png`, a308/309/310 stacked): the nadir road pattern is CONSISTENT across the 3 frames (radial lane-line structure tracks smoothly frame-to-frame, not the random per-anchor swirl).
> **So B-coherence does BOTH: keeps the per-pixel high coverage AND adds temporal coherence via world-deterministic source selection — without the world-grid's discretisation/accumulation loss.** Honest: 3 frames is a FIRST LOOK; the decisive proof is a baseline (COHERENT off, same MOVING off) vs coherent slit-scan over more frames, then 5-scene + the full 93-frame video. Evidence: `coh_a30{8,9,0}_seg*.png`, `coh_temporal_montage.png`. Gated, default off; pristine backups intact.
> **NEXT:** baseline-vs-coherent swirl comparison (decisive), then 5-scene regression + the full video.
> ---

> ### 2026-06-24 ◆ DB-109 B1 wide-window — "narrow window" hypothesis WRONG (wide 225-317 made cap onok WORSE 6.2%→3.6%); root = naive grid accumulation FAILS on the trajectory-line band = exactly the cap; world map's 74% ok is the EASY side-road
> **Wide window (225-317, 93 frames): world map = 74.2% ok (even higher) but cap onok DROPPED to 3.6% (narrow 28-frame was 6.2%). My "too narrow / missing 20-28 m sources" diagnosis was WRONG — more frames made the cap WORSE (honest correction).** Coverage map (`wbev_a309_wide_worldcov.png`, eye-verified): a horizontal black CAPSULE along the ego trajectory = the cap's region (ego's own car over many frames + the trajectory-line near-ground only seen grazing). The 74% green is the side/far road; a309's CAP (its blind nadir = the trajectory band) lands on this black capsule.
> **Real finding: naive grid accumulation BUILDS a nice road map but FAILS on exactly the band the cap needs.** Two compounding causes: (1) more frames → more transient-car samples + more grazing-angle/exposure diversity per cell → spread inflated → not-ok (onok FALLS as frames grow, 6.2→3.6); (2) flat-Z grid + 6 cm discretisation loses the per-pixel grazing precision. **CONTRAST: single-frame nomove got 81.9% on the SAME a309 cap** — per-pixel reprojection to bucketed cand frames (disp 5-58 m, time-nearest 3/bucket) keeps it; the all-frame grid spread-median throws it away. So a309's cap IS fillable (81.9% proven); the naive world-grid is the WRONG VEHICLE.
> **B needs a redesign — two paths:** **(B-grid)** robust per-cell fusion: per-cell LiDAR-Z (not flat) + reject transient-car/grazing outliers + pick a consistent time-nearest subset (mirror single-frame's bucketing) → recover the trajectory band. **(B-coherence)** DROP the grid: keep single-frame's per-pixel reprojection (81.9% coverage) but add cross-frame consistency — every WORLD ground point picks ONE global source used by ALL target frames → temporal coherence WITHOUT the discretisation/accumulation coverage loss. B-coherence likely better (keeps the 81.9%, only adds coherence; no spread-pollution).
> **Honest status:** "physical wall disproved" (Stage-1) STANDS; but B's naive accumulation does NOT auto-solve the cap — the cap = the trajectory band, the hardest part, and simple spread-median over many frames pollutes it. The world-BEV branch (`GROUND_MODE="worldbev"`) is committed as a working accumulator but the cap-fill needs the redesign. Evidence: `wbev_a309_wide_{worldmap,worldcov}.png` + the narrow `wbev_a309_*`.
> **NEXT (user co-decide):** B-grid (robust fusion) vs B-coherence (per-pixel reprojection + world-anchored single-source selection).
> ---

> ### 2026-06-24 ◆ DB-109 B (world-BEV mosaic) — B1 FIRST RUN: accumulation WORKS (world map = 71% real road over the window); cap-sampling 6.2% bug = WINDOW TOO NARROW (no 20-28m sources); wide-window re-run in flight
> **Implemented `GROUND_MODE="worldbev"` (additive new branch + `WORLDBEV_WIN` flag): build ONE world-fixed BEV ground map over a FIXED anchor window, accumulate ALL window frames (spread-dominant, NO moving box-gate = Stage-1c folds in, nvalid≥2 guard), each target anchor samples the SAME map → temporal coherence by construction. Ran a309, window 290-317 (28 frames). Payload now ~108 KB (gzip-inject still fine).**
> **PROVEN — the accumulation CONCEPT works:** the world map (795×602 @ 6 cm/px) = **71% ok cells = a real road mosaic** (eye-verified `wbev_a309_worldmap.png`: real asphalt + lane-line diagonals; black holes = the cars' positions, correctly held by spread/nvalid). vs single-frame a309 5.6% real → accumulation recovers the road as hoped.
> **BUG (not physics): the cap render is only 6.2% coverage.** Diagnostic `WBEVCAP`: cap ground points are **100% in-grid but only 6.2% land on ok cells**. Root: window 290-317 = only ~10 m ego motion, so a309's nadir (the trajectory line) has NO 20-28 m-distant source frames (the grazing sweet spot that clears the hood) — every ego's own nadir is a hole in this NARROW window. The world map's 71% ok is the near/side road; the cap wants the trajectory-line nadir whose sources are 20-28 m away and absent. (Single-frame nomove got 81.9% precisely because it uses disp 5-58 m WHOLE-LOG candidates = HAS those far frames.)
> **FIX: widen the window to cover ±20-28 m sources** — re-running WORLDBEV_WIN=(225,317) (highway video window; a309 has ~84 frames / 31 m back-window → covers the sweet spot). If cap onok% jumps, B1 is confirmed and the next step is the a309/310/311 temporal-coherence test (same map, 3 frames, swirl gone?).
> **Evidence:** `wbev_a309_{worldmap,worldcov,seg}.{png,npy}`. `GROUND_MODE="worldbev"` in db89 (additive, gated; pristine backups intact).
> ---

> ### 2026-06-23 ◆ DB-109 Stage-1c — precise gate: scale=1.0 only recovers a309 to 33.6% (it's box+grazing geometry, not just the 1.3x inflation); SPREAD-DOMINANT is the fix; single-frame ceiling = the traffic itself (→ B)
> **Added `MOVING_SCALE` flag (default 1.3); ran a309 + crowd funnel at scale=1.0 + fetched seg renders (eye-verified: no car-hallucination, real asphalt+lanes).** a309 real 5.6%(1.3) → **33.6%(1.0)** → 81.9%(off); crowd 67.7%(1.0, gate3=0). So scale=1.0 helps but is FAR from off's 81.9% — the over-aggression is NOT just the 1.3x inflation: even a 1.0x box tested along the WHOLE grazing ray drops ~66% of a309's good sources. **Reason (first-principles): the moving box is a 3D AABB (car + surrounding void); a near-grazing ray passes through the box's VOID (under/beside the car) and samples the GROUND behind it, yet the box-intersection test marks it blocked.**
> **The spread gate ALREADY separates ground (multi-source agree, spread≤30) from car-body (disagree, spread>30) — a309 off = 81.9% real + 18.1% spread-held car. So the moving box-gate is REDUNDANT on multi-source points and only mis-blocks them. Precise fix = SPREAD-DOMINANT: soften the moving hard-gate, let spread judge, guard single-source points (require nvalid≥2 to render REAL) against the single-source-car-body case.**
> **Single-frame CEILING = the traffic itself:** a309's blind ground that THIS frame's cars genuinely block is unrecoverable in THIS frame; the root recovery is **B (world-BEV whole-log accumulation — other frames have the cars elsewhere).** So Stage-1c (A) = make the per-frame gate precise (improves every frame + feeds B cleaner sources); B = accumulation (the real coverage solution). They STACK.
> **Evidence (`deliverables/db109_ground_rootcause/`):** `hwytight_a309_*`, `crowdtight_a045_*` (funnel counts/map + seg render). Flag `MOVING_SCALE` in db89 (default 1.3 = shipped).
> **NEXT:** implement the spread-dominant gate (gated flag, needs go-ahead — changes shipped fill behavior) + 5-scene regression; then B (world-BEV).
> ---

> ### 2026-06-23 ◆ DB-109 Stage-1b RESULT — the moving-box gate is OVER-AGGRESSIVE: a309 real 5.6% → 81.9% when off; the "94%" = ~76% mis-blocked REAL ground + 18% genuine car (held by spread)
> **Isolation (new `MOVING_GATE` flag, default True = shipped; driver injects False): re-ran a309 funnel with MOVING_GATE=False.** real **5.6% → 81.9%**. The 94.4% gate3 mass splits into **76.3% that becomes spread≤30 REAL** (consistent GROUND sources the moving-box gate wrongly blocked) + **18.1% spread>30** (genuine car-body sources, correctly held by the spread gate). **Eye-verified:** `hwynomove_a309` nadir render = real asphalt + lane lines filling the cap (vs baseline's GRAY plate); funnel map = 81.9% GREEN ground + 18.1% YELLOW concentrated at the car's azimuth (spread gate cleanly isolates the car; no ground-painted-car hallucination visible).
> **Root of the over-aggression (`gseg_blocked`, 1.3x-inflated moving box tested along the WHOLE grazing ray):** a long grazing source ray (egod up to 28 m) that merely PASSES NEAR a 1.3x car box ANYWHERE along its length is dropped, even though the ground point itself is visible. a309's nearby traffic (left sedan + front truck) thus blanket-blocks ~76% of otherwise-good ground sources.
> **DECISIVE: a309's low real-share is NOT a physics wall NOR a world-BEV necessity — it's a fixable single-frame gate bug.** Tightening the moving gate (precise box; block only when the car is actually BETWEEN source and ground point, not 1.3x + whole-ray) recovers ~76% real ground per-frame. **CAUTION: do NOT ship MOVING_GATE=False** (it leans on the spread gate to catch cars → a consistently-multi-seen car could pass with low spread → ground-painted car). Ship = a PRECISE moving gate, spread as backstop.
> **Two SEPARATE problems now cleanly named:** (1) swirl/temporal incoherence = per-anchor independent rebuild (DB-109 root, needs world-BEV); (2) low real-coverage on traffic frames = over-aggressive moving gate (Stage-1b, a per-frame fix). Both real, independent, both improve the ground; they STACK.
> **Evidence (`deliverables/db109_ground_rootcause/`):** `hwynomove_a309_{funnel_counts.json,funnel_map.png,funnel_map_nadir.png,seg.png,seg_nadir.png}` + `highway_a309_seg*` (baseline). Flag `MOVING_GATE` in db89 (default True, diagnostic).
> **NEXT (user co-decide):** (A) implement a PRECISE moving gate (per-frame real-coverage win across all scenes); (B) world-BEV mosaic (temporal coherence + whole-log accumulation). They stack.
> ---

> ### 2026-06-23 ◆ DB-109 Stage-1 RESULT — gate-funnel DISPROVES the "physical wall"; a309's 94% "no-source" = MOVING-BOX occlusion, NOT geometry-blind (Finding 5 CORRECTED)
> **Ran `GROUND_MODE="funnel"` (diagnostic branch, additive, no main-path change) on bmw a044 + clean a046 + highway a309 + a260 (L4). Each blind nadir pixel classified by the HIGHEST gate it reaches: N1 FOV-hit → N2 ~self-occ → N3 egod∈[5,28] → N4 ~moving-box → N5 spread≤30. eye+code+data verified; funnel self-checks: clean gate5=91.3% == progress "clean 91.3% real"; a309 gate5=5.6% == progress "a309 5.6% real".**
> **DECISIVE: gate0 (geometry-blind = no cam EVER saw it) = 0.0% on ALL FOUR frames.** Every nadir blind pixel IS seen by ≥1 camera. The "physical wall / 94% never-seen geometry dead-lock" (Finding 5, 2026-06-22) is WRONG — a misattribution. The user's instinct ("driving 5-58 m, the ground behind us must get photographed") was RIGHT.
> **What the 94% actually is (per-frame, eyeballed funnel maps):** a309 = **94.4% gate3 MOVING-BOX occluded** (viz: nadir almost entirely BLUE) — the source-frame back-projection ray passes through a moving-object box (1.3x-inflated, tested along the whole grazing ray) → abstain; only 5.6% real. a260 (SAME highway log, a low-traffic moment) = **99.0% real** (viz: all GREEN) + 1% spread. bmw a044 = **58.6% gate4 spread>30** (sources disagree, grazing outer ring) + 41.4% real. clean a046 = 91.3% real + 8.7% spread.
> **The two REAL bottlenecks are both "the algorithm ABSTAINS", NOT "the info doesn't exist":** (1) moving-box occlusion (a309) — but traffic is INSTANTANEOUS; across the whole log each ground patch has frames where no car blocks it; (2) spread>30 multi-source disagreement (bmw) — grazing outer ring, likely an alignment problem curable in the metric domain.
> **Implication (STRENGTHENS world-BEV Stage-2):** a309's single-frame 5.6% real is throttled by THAT frame's transient traffic blocking 94% of source rays; a whole-log world-BEV mosaic (accumulate ALL 93 frames, each patch keeps its car-free views) should recover real ground FAR above 5.6%. "a309 = geometry dead-lock, generation-only" is OVERTURNED. The swirl root (per-anchor independent rebuild, entry below) STANDS; the "physical-blind floor" does NOT.
> **Evidence (`deliverables/db109_ground_rootcause/`):** {bmw_a044,clean_a046,highway_a309,highway_a260}_funnel_{cls.npy,counts.json,map.png,map_nadir.png}. Diagnostic branch `GROUND_MODE="funnel"` in db89 (additive; pristine backups: git ae0c823 + `db89_ghost_recovery_PRE_DB109.py.bak` + `_baseline_fable5/`). Infra fix: db89 inject payload (~135 KB b64) exceeded Linux MAX_ARG_STRLEN (128 KB) → `[Errno 7]` → gzip the payload (→44 KB) + remote `gunzip | python -`.
> **NEXT:** Stage-1b — is the moving-box gate OVER-aggressive (1.3x + whole-ray) vs genuinely blocked? + a back-projection overlay (eyeball a309 blind points on the source frames). Then Stage-2 world-BEV mosaic, now strongly justified.
> ---

> ### 2026-06-23 ◆ DB-109 OPENED — ground swirl ROOT-CAUSED to per-anchor independent rebuild (wrong representation domain); world-BEV-mosaic route co-decided; L4 live
> **New thinking-leader session (user `/using-superpowers`).** User REFRAMED the task: NOT "pick a nadir filler" (DB-108's trichotomy) but "find the底层 root cause of the ground problem". Did a full eye+code root-cause pass on `ground_video_v1` (the Fable-5 93-frame videos).
> **EYE (local frame-extraction `agent/_db108_video_debug/`, every image vision-checked):** the nadir "boils" — radial swirl from the ERP south-pole, per-frame writhing. Severity bmw > highway ≈ crowd > clean, INVERSELY tracking real-reproj share (clean 91% real = most stable; bmw/highway fabrication-dominant = worst). slit-scan (same azimuth × 93 frames) shows vertical step-jitter = temporal incoherence, NOT a single-frame defect.
> **CODE (db89 STAGE-4, data-flow confirmed):** `run_case` renders ONE anchor; the video = 93 INDEPENDENT run_case calls → ground rebuilt FROM SCRATCH per frame. STAGE-4 on each frame's ERP south-pole disc: per-pixel `argmin` source pick (:1438) + per-frame global tone gain (:1459) + per-frame plate/inpaint (:1484), with NO cross-frame constraint — the code itself admits `No cross-anchor fusion` (:1471; DB-99's gray plate just swapped flickering inpaint for a non-flickering gray to HIDE the symptom, never touched the cause). **ROOT = wrong representation domain: the ground is world-static but solved as a per-anchor ERP disc.**
> **Two-distance clarification (user caught my sloppiness — corrected an overclaim):** `disp` 5–58 m (:1158, frame↔frame, picks candidate frames) and `egod` 5–28 m (:1356, ground-point↔source-car, picks usable source per pixel) are DIFFERENT measurements — not a contradiction (58 = 28 + 30 reach, :1154). egod's 28 m ceiling is self-admittedly "wrong geometry (point distance not ray clearance)" (:1372) → may over-reject. So "94% no-source" CONFLATES geometry-blind vs rule-rejected — must be SPLIT by diagnostic, NOT asserted as a physical wall. (Honest: my earlier "physical wall" framing was too strong.)
> **DECISION (user co-decided):** opened **DB-109** (root-cause route), SUPERSEDES DB-108's filler trichotomy. Stage-1 = gate-funnel diagnostic (bmw a044 + clean control) splitting no-source into geometry-blind (N1=0) vs rule-rejected (N1>0,N5=0). Stage-2 = no-network world-frame BEV ground mosaic (accumulate 93 frames×7 cams into one world raster + coverage map; each anchor samples it → coherence by construction; upgrades DB-102 per-anchor local BEV to a cross-log world map). Pure geometry, L4, no network/A100.
> **Infra:** L4 live (22.5 GB free); token written to NON-repo `~/.waymo2panorama/runtime/active_url.json` (never committed). Frame A = bmw a044 (worst radial+jitter; local scan `agent/_db108_video_debug/bmw_pick_worst.py`).
> ---

> ### 2026-06-22 ◆ AUDIT (user-led) — nadir "all gray" REGRESSION root-caused; ground_video_v1 "real ground" = db89 fill + NS-inpaint; DB-99 plate = regression for koi; COMBO verified
> **HANDOFF CONTEXT (read this first if you are a fresh agent):** user audited the current panorama and found the nadir = entirely GRAY ("no real ground left"). This entry + brief **DB-108** = the full state of the ground problem. GPU = Colab **L4** via `~/.waymo2panorama/runtime/active_url.json` (NON-repo, never commit url/token). All evidence in `deliverables/db105_nearfield_geometry/`. Drivers in `tmp/` (non-repo) inject db89's remote_py via string-replace. Reply to the user in Chinese. VERIFY every remote result by reading the output file; vision-check every image.
> **Finding 1 — what `ground_video_v1` actually is:** the 2026-06-19 video the user remembers as "real ground" was rendered by `db89_ghost_recovery.py` `GROUND_MODE="fill"` with **NS-inpaint** fill (`deliverables/ground_video_v1/RENDER_FRAMES.md` §4 + this file's DB-99 records). Its "real ground" = a SMALL real-reproj skeleton + NS-inpaint EXTENDING the real edges into the blind cap. Verified by pulling the actual a309 video frame from Drive → `gv1_a309_original.png`: has ground-feel but smeared/白团, identical in kind to the inpaint baseline I re-ran.
> **Finding 2 — the gray regression:** DB-99 (implemented AFTER 6-19) REPLACED NS-inpaint with the gray DC `plate` (this file's 2026-06-18 ROUND-2 record; db89 STAGE-4 ~L1482-1484). So the current default nadir = flat gray. For the koi need (plausible; Cosmos regenerates the single frame downstream), DB-99 is a **REGRESSION** — its only justification was per-frame video swirl-flicker, which is MOOT once Cosmos regenerates per-frame. Product POV: ground-feel (even blurry) > honest gray.
> **Finding 3 — real-reproj share is TEXTURE-GATED:** measured per same-frame run. a309 (bare highway asphalt + 4-6° grazing specular) = real coverage **5.6%** (current code + inpaint) → ~94% is inpaint; older `_baseline_fable5` = **16.9%** on the same frame; clean a046 (city: tire-marks/lines/manholes) = **91.3% real-write (spread≤30) / 100% has-source** (diag-measured; `clean_a046_REAL_vs_INPAINT_spread.png` — red=inpaint is only 8.7%, sitting exactly on the near-nadir edge arcs the user circled). So clean = real-dominant, a309 = inpaint-dominant: SAME algorithm, scene decides the real/fabricated ratio.
> **Finding 5 — ROOT CAUSE of why some blind ground is recoverable and some is not (a309 diag VERIFIED, `a309_REAL_vs_INPAINT_diag.png`):** a309 cap = **94.4% NO-SOURCE** (no camera EVER saw it) + 5.6% real + **0% disagree**. The 0% disagree is the key: a309's loss is **GEOMETRY (no source), NOT appearance (spread)** — earlier "bare-asphalt grazing-specular spread" was NOT a309's main cause. WHY: a blind nadir point sits ON the ego's OWN trajectory line (it's directly under the virtual centre), so every OTHER frame views it from dead-ahead / dead-behind → ego self-occludes it (hood 0-9 m / trunk 0-20 m) + steepest grazing. The ONLY escape is a LATERAL trajectory (turns/intersections/lane-changes) that puts the point to the SIDE of some frame → a side camera sees it unoccluded → real. clean (city, curved) = lateral motion → 100% has-source; a309 (highway, straight) = zero lateral → geometry dead-lock → 94% never seen → no amount of TIME (whole-log) or LiDAR recovers it. **LiDAR cannot help**: it gives geometry/position, NOT appearance; the blind point's colour/texture must come from a camera that actually saw it, and none did. ⇒ "real ground" feasibility is **SCENE-DEPENDENT physics, not an algorithm bug**: city ~91% recoverable, straight-highway ~5.6%. Direction implication: pursue REAL (C) on city scenes; straight-highway nadir is real-impossible → only plausible fill (A inpaint / B generative). This also re-frames the 5.6%-vs-16.9% OPEN: a309 is geometry-blind regardless; the baseline's higher number is mostly its egoproj-union counting ego-under-body cells as "filled", not more genuine recovery. "Looks real" (user, on video) and "94% fabricated" (measurement) are the SAME fact: NS-inpaint extends real edges so it reads as real road. The user's eye was right all along (real skeleton + inpaint-blurred filler; the "糊" the user noticed IS the inpaint).
> **Finding 4 — COMBO verified (recommended quick restore):** DB-106 boundary (keeps the near car) + swap gray `plate` → `cv2.inpaint(comp, resid_m, 8, cv2.INPAINT_NS)` = recovers the video's ground-feel AND keeps the near car intact (the best of both, which had NEVER been combined before — baseline had inpaint-feel but ate the car; current keeps the car but is gray). Ran on a309. NOT YET固化 to db89 — runs via injection (`tmp/_combo_a309.py` swaps the plate line). Images: `combo_a309_DB106_plus_inpaint.png`, `a309_THREE_WAY_nadir.png`, `a309_GV1_vs_NOW_vs_COMBO.png`.
> **OPEN (not root-caused):** real skeleton 5.6% (current+inpaint) vs 16.9% (baseline_fable5) on the SAME frame & SAME candidate set — DB-98 (LiDAR-ground marching) and/or DB-106 (egoproj-drop) may have ALSO shrunk the real skeleton, not just the filler. Needs a one-variable rollback to confirm which step and how much. → DB-108 direction (C).
> **Evidence (`deliverables/db105_nearfield_geometry/`):** `baseline_a309_videoEra.png` (video-era inpaint), `db106_a309_FILL.png` (current gray), `combo_a309_DB106_plus_inpaint.png` (combo), `gv1_a309_original.png` (the actual video frame), `a309_BASELINE_vs_NOW_nadir.png`, `a309_THREE_WAY_nadir.png`, `a309_GV1_vs_NOW_vs_COMBO.png`, clean diag (`clean_a046_*`, in progress).
> **NEXT = user's call (brief DB-108):** (A)固化 combo / (B) inpaint→generative / (C) chase the 5.6%-vs-16.9% skeleton loss first.
> ---

> ### 2026-06-21 ◆ DB-95 SCOPING — Waymo data is E2ED camera-only (no LiDAR/boxes); full-stack generality NOT testable here, but the no-LiDAR degradation IS
> **Checked the Waymo data on Drive (`data/waymo_e2ed`):** it is Waymo's **End-to-End Driving** set (`E2EDFrame` proto, 8 cameras, 1.6 GB tfrecord, `waymo_open_dataset` pkg not installed) — **camera-only: NO LiDAR, NO 3D boxes, NO ego pose in the extracted meta.** Camera calibration IS complete (K + T_ego_cam 4×4 + distortion, cam height 1.445 m, in `frame0_extracted/frame_meta.json`, 8 cams FRONT/FRONT_LEFT/FRONT_RIGHT/SIDE_LEFT/SIDE_RIGHT/REAR/REAR_LEFT/REAR_RIGHT). `waymo_loader.py` is a SKELETON (`_load_calibrations`/`_index_images` = NotImplementedError stubs). Prior Waymo work was L1 only (`l1_waymo_*.png`, no depth).
> **Implication for the north-star (DB-95):** db89's FULL stack depends on LiDAR (depth_field/ground/photometric) + tracks (moving objects) → it CANNOT run on this Waymo data → full-stack cross-rig generality is NOT testable on E2ED. BUT what the north-star most cares about — **"graceful degradation WITHOUT LiDAR"** — IS testable: re-centring (c*=ring centroid, needs only extrinsics) + no-LiDAR fallback (plane depth, identity gains) on a DIFFERENT rig (Waymo 8-cam) → does the Fable-5 re-centring breakthrough generalise to another rig?
> **Roadmap (a real next战线 — needs user go-ahead, NOT a one-loop job):** (1) fill `waymo_loader` stubs from `frame_meta` (calib) + extracted jpgs; (2) factor a no-LiDAR core out of db89's AV2-coupled `load_all`/`run_case` (or a standalone re-centring sphere-projection probe); (3) render a Waymo re-centred panorama, compare ego-origin vs centroid seams on the 8-cam rig. Honest limit: E2ED has no LiDAR, so depth/ground/object generality stays UNVERIFIED here — a data limitation to state, not a method failure. (To verify those, need a LiDAR-bearing second dataset, e.g. Waymo Perception or nuScenes.)
> ---

> ### 2026-06-21 ◆ DB-107 IMPLEMENTED — fill radial artifact (user's defect #2): bevdirect = metric-consistent select + DIRECT ERP sample
> **User authorized fixing defect #2. Implemented `GROUND_MODE='bevdirect'` (additive, default still fill).** Reuse bev's METRIC-domain consistent source choice (`bchosen`, no per-pixel jump → no radial) but render by DIRECT ERP sampling of that source per cap pixel (no source→raster→ERP resample → keep texture, not bev's soft). 3 edits in STAGE-4 (bev-branch condition + the bevdirect direct-sample block + the cap override). db89_completed on a309 + downtown, coverage 100% on downtown.
> **Result (eyes-verified, `downtown_road_speckle_zoom.png`, `a309_FILL_vs_BEVDIRECT_vs_BEV.png`):** bevdirect's road is MORE CONSISTENT than fill (per-pixel speckle reduced) AND keeps lane-line/texture density (NOT bev's softness). It unifies fill's texture + bev's coherence as designed; no new artifact, car still uncovered (DB-106). HONEST: improvement is MODEST on a309 (4.1% cov, mostly blind→flat) and downtown a30 (fill's speckle there is mild); the radial defect is WORST on high-coverage grazing frames (the early highway full-cap with radial lane-line shards) — that's where bevdirect's full value would show.
> **Status:** clean unification of the 3 ground modes (fill=texture / bev=coherence / bevdirect=both). Making it the ground DEFAULT is a fill-vs-bev policy choice (DB-94 Cosmos contract).
> **VALIDATED on the radial-WORST frame (highway a260, `a260_radial_zoom.png`) — HONEST FINDING:** bevdirect's road IS more consistent (between-lane speckle reduced) with lane-lines/crosswalk kept. BUT a260's prominent "radial arcs" are mostly the REAL CROSSWALK's ERP-pole projection (NOT artifact — both modes render them), and fill's true per-pixel speckle is ALREADY largely suppressed by DB-89's EMC + agreement gate. So bevdirect's gain is a MODEST consistency polish, not a heavy-defect removal. ⇒ **fill radial is already not a big problem in current db89; the user's defect #2 was largely handled back in DB-89.** bevdirect remains a cleaner OPTIONAL ground mode (texture + coherence), worth keeping but not a dramatic fix. Honest close on defect #2.
> ---

> ### 2026-06-21 ◆ fill radial-artifact (user's defect #2) — first-principles diagnosis + unified-solve plan (NOT implemented; a USER co-decide, gated on fill-vs-bev policy)
> **Diagnosis (DB-102-consistent, STAGE-4 source-select confirmed):** fill's radial/speckle = ERP near-nadir is a coordinate singularity + fill picks the ground source PER-PIXEL (chosen_g per cap pixel from bucketed whole-log candidates) → neighbouring cap pixels draw DIFFERENT source cams / candidate frames → radial colour discontinuity. bev (GROUND_MODE='bev') already fixes this via metric-domain CONSISTENT source choice (bchosen on a uniform BEV raster) but routes source→raster→ERP (2 resamples) → SOFT.
> **Unified-solve plan (take both strengths):** metric-domain CONSISTENT source choice (kill the per-pixel jump → no radial) + DIRECT ERP sampling of that chosen source (no raster round-trip → keep fill's texture density, not bev's softness). I.e. reuse bev's `bchosen` selection but render by sampling the source straight into each cap ERP pixel. Medium-high STAGE-4 refactor.
> **Gating (why NOT autonomously done):** whether to invest depends on the fill-vs-bev mainline, which is itself gated on the DB-94 Cosmos contract — if Cosmos regenerates ground appearance (masked-360 training), neither radial nor soft matters (even middle-only+mask suffices). So this is a USER co-decide, not an autonomous bet. Recorded ready-to-implement.
> ---

> ### 2026-06-21 ◆ DB-106 verified on the FILL original algorithm too + regression PASS (user steered back to db89's fill, not bev)
> **User correctly flagged I'd been demoing bev (GROUND_MODE='bev', DB-102 opt-in) while db89's DEFAULT/original is GROUND_MODE='fill' — the ground_video_v1 deliverables are fill.** DB-105/106 edits are in shared db89 code, so both routes benefit. Re-ran a309 on the FILL original algorithm (+ single-source + DB-106): db89_completed, whole-log search active (cand_frames=33, disp 5.2-55.8 m), CAR NO LONGER COVERED (DB-106 works on fill identically). `deliverables/db105_nearfield_geometry/a309_FILL_vs_BEV_ZOOM.png`.
> **Regression PASS (DB-106, eyes-verified):** downtown dusk + bmw bev — deep-coloured real content (dark cars) PRESERVED (the comp<12 edge case did NOT trigger on real vehicles), near objects kept, ground still 100% filled. DB-106 is safe + general; the "object-mask explicit protection" hardening is not urgent (theoretical edge case only).
> **HONEST fill-vs-bev state (a309, both with DB-105+106):** car not covered in BOTH. Ground itself: FILL = real texture but the ERP-pole RADIAL artifacts remain (user's defect #2, which DB-106 does NOT touch — bev was the workaround); BEV = no radial artifacts, smooth, but soft + local texture-joins + honest nadir blind spot. So #1 "ground eats the car" = SOLVED both routes; #2 "radial/speckle" = still open on fill (bev sidesteps it at the cost of softness). Deliverable fill-vs-bev = a policy choice gated on the DB-94 Cosmos contract.
> **OPEN next (if fill is the chosen mainline):** fix fill's radial pole artifact AT THE SOURCE (per-pixel argmin across the ERP pole singularity → metric-domain reproject like bev but KEEP fill's texture density), rather than switching to bev's soft fill.
> ---

> ### 2026-06-21 ◆ DB-106 (USER-FOUND) — ground outpainting was OVERWRITING the scene band's real content; boundary fixed + validated
> **User caught it (I had WRONGLY called the v9 car "complete" off a thumbnail — a real eyes-beat-metrics failure on my part):** zooming the v9 bev sphere, the near car's LOWER BODY was eaten by ground (dark contact-shadow + grey road), while middle-only (no ground) renders the WHOLE car. User's first-principles diagnosis (correct): the nadir/ground layer and the scene-band main subject have NO clean boundary — ground outpainting overwrites real content the scene band already rendered ("中间主体在 middle-only 拼得好,一开地面 outpainting 就被影响").
> **Root cause (code-confirmed):** ground's fill region `blackg = (comp black) | egoproj` (L1085) and `resid_m` unioned `egoproj` (L1443). A NEAR car's lower body has rays that PASS THROUGH the ego box (egoproj=True) even though comp there is the REAL car; unioning egoproj pulled the real car lower body into the ground fill zone → footprint-shadow (plate×0.55, L1459) + bev/plate painted OVER it. The boundary was GEOMETRIC (egoproj/footprint), never "is the scene band already real here".
> **Fix (DB-106, 2 lines):** `blackg` and `resid_m` = ONLY `comp` truly-black (scene band rendered nothing). Dropped the unconditional egoproj union. egoproj's genuine blind region (under-hood/under-ego) is comp-black and STILL filled; comp-nonblack near-car body is now PRESERVED. Principle (user's): ground fills ONLY scene-band holes, NEVER overwrites real content.
> **VALIDATED (L4, eyes-verified, `deliverables/db105_nearfield_geometry/db106_car_BEFORE_vs_AFTER.png`):** a309 bev — BEFORE = car lower body eaten; AFTER = WHOLE car back (roof/windows/doors/both wheels/lower body complete), surrounding bev ground still clean, ground still fills the genuine nadir blind spot (coverage 12.6%→4.1%; the drop = removing the erroneous over-car fill). db89_completed.
> **Impact:** fixes fill AND bev (shared boundary); benefits ANY near object / near road the scene band rendered. The "complete panorama" had a real, general defect — the faithful mosaic now actually KEEPS its near-field content under ground outpainting. NOT gated (it only ever REMOVES over-paint = strictly more conservative); pristine core in `_baseline_fable5/`.
> **NEXT:** quick regression (clean/bmw ground — fix only removes over-paint, expect no harm); then re-confirm the complete deliverable with DB-105 single-source + DB-106 boundary together.
> ---

> ### 2026-06-21 ◆ DB-105 LOOP SYNTHESIS — near-field STITCHING solved; the faithful mosaic is now COMPLETE; remaining frontier = generation (gated on contract)
> **This autonomous /loop's achievement (user's frontier #1, near-object seam):** the near-car seam — declared an "8.6 px physical limit" by DB-104 — was REFRAMED + SOLVED. Root (deeper than DB-103/104): db89's c_own selection ranks by COMPLETENESS, which on a very close car picks the grazing-SLIVER camera as primary and morphs the complete car INTO it = the shear. Fix = dominant-coverage flip + single-source (`SEAM_SINGLE_SOURCE`, gated). 5-scene validation: a309 FIXED, crowd/bmw cross-camera morphs PRESERVED, clean BYTE-IDENTICAL, downtown clean → surgical + safe. Commits 0be1e34..5526726.
> **First-principles global status:** every FAITHFUL hard edge of perspective→ERP is now handled — re-centre (c*), photometric, async-shutter, moving-object, near-ground BEV domain, AND now the near-object seam. Middle band solid; near objects single-source cleanly. **The faithful mosaic is COMPLETE.**
> **Ground frontier — faithful CEILING reached (physics, not a TODO):** 0-3 m nadir = sensor-agnostic blind spot (generation only); 3-10 m grazing = BEV renders it but SOFT — grazing resolution is capped at the source's stretched sliver, and multi-frame fusion only denoises ~√N, it CANNOT break the grazing resolution ceiling. So "make the ground sharp" REQUIRES generation, gated on the DB-94 Cosmos contract (plausible-vs-faithful + does Cosmos ingest masks) — a USER decision, not autonomously resolvable.
> **TWO pivot decisions for the user (the actual high-leverage next steps):** (1) flip `SEAM_SINGLE_SOURCE` default ON — 5-scene-validated, surgical+safe, one-line revertible; (2) the DB-94 Cosmos contract — confirm centre + mask semantics, then decide ground = bev-soft+mask (let Cosmos generate) vs in-house plausible fill. Both need the user; autonomous high-leverage work on the near-field branch is SATURATED.
> ---

> ### 2026-06-21 ◆ DB-105 single-source 5-SCENE VALIDATION COMPLETE — surgical + safe
> **bmw + downtown SS=True (morph + full-pano eyeballed):** bmw -> 1 morph PRESERVED (front_left/side_left 6.46 @ ecc 0.889 = a genuine cross-camera object, dominant-flip correctly did NOT fire); downtown -> view_morph [] (no straddle morph); both panos clean (white SUV / dusk city intact, no truncation/artifact).
> **5-SCENE SCORECARD for SEAM_SINGLE_SOURCE:** a309 highway = FIXED (near-car 8.64 -> gone, car complete); crowd a50 = NO degradation (ecc restored 0.913, RAM intact); clean a30 = BYTE-IDENTICAL (0/2.1M px diff); bmw = cross-camera morph preserved + clean; downtown = clean. ⇒ the dominant-coverage flip is SURGICAL (fixes single-cam-dominant near-objects ONLY) + SAFE (zero off-target side-effect, no regression on any scene).
> **SHIP RECOMMENDATION (for the user):** strong evidence to flip SEAM_SINGLE_SOURCE default ON — but it changes a Fable-5 CORE default (the c_own source selection), so per the SEAM_FLOWMORPH precedent (shipped only with user go-ahead) it is LEFT DEFAULT OFF pending the user's one-line OK. Fully gated + one-line revertible (`SEAM_SINGLE_SOURCE=False`); pristine core in `_baseline_fable5/`.
> **NEXT:** disocclusion-edge polish (a309 residual viewpoint-join) is physical + low-leverage; the higher-value frontier is the GROUND blind spot, whose CORE (generation) is gated on the DB-94 Cosmos contract (user coordination). Faithful contract-independent ground work = BEV multi-frame temporal super-res on the 3-10 m grazing band (likely marginal) + precise nadir-mask output for Cosmos. Probing BEV multi-frame super-res next.
> ---
...
> **Result (L4, eyes-verified):** a309 SS=True -> the car's 8.64 px morph GONE (front_left/side_left job removed) while OTHER morphs are PRESERVED (side_left/rear_left 1.22 kept — vs the over-eager first cut that zeroed ALL morphs); crowd a50 SS=True -> morph pairing + ecc_cc RESTORED to baseline (front_right/side_right 6.89 @ ecc 0.913, was 6.98 @ 0.377) = NO degradation. Both panos eyeballed (`A309_ssfinal.png`, `CROWD_ssfinal.png`): a309 car complete + no shear, crowd RAM complete. ⇒ single-source is now SURGICAL: fixes the single-cam-dominant near-object, leaves true cross-camera objects on the morph path. Evidence also `crowd_a050_AB.jpg`.
> **NEXT (this loop):** clean/bmw/downtown SS=True regression (no dominant near-object -> expect ~baseline) + SS=False 5-scene byte-identical confirm; then disocclusion-edge photometric polish; then pivot to the GROUND blind spot (the one place that needs generation).
> ---

> ### 2026-06-21 ◆ DB-105 step-3b IMPLEMENTED + VALIDATED on a309 (L4) — the REAL root was c_own picking the SLIVER camera; area-gated single-source removes the morph shear
> **Implemented `SEAM_SINGLE_SOURCE` (gated, default OFF) in db89 + ran a309 baseline vs single-source A/B (GROUND_MODE=off, scene-band focus). Self-driving /loop, user authorized autonomous work.**
> **DEEPER ROOT than DB-103/104:** db89's pass-3 c_own choice ranks by COMPLETENESS ("mask not touching the image border") FIRST. On a very close car this MISFIRES: the grazing SLIVER (front_left, 149 LiDAR pts) sits fully in-frame = "complete", while the camera that sees the WHOLE car (side_left, 1610 pts) has it touching the border = "incomplete". So c_own became the SLIVER cam and side_left the secondary; STAGE-3.5 then morphed the complete car INTO the sliver -> the 8.64 px shear. DB-103's flow-morph and DB-104's exhaustive isolation BOTH treated the symptom; the cause is c_own inversion. (cam_pair proof: baseline morph job = ['ring_front_left','ring_side_left'] i.e. c_own=front_left=the sliver.)
> **Fix (3 edits, all under `SEAM_SINGLE_SOURCE`, default OFF, pristine core in _baseline_fable5/):** (1) c_own ranked by visible MASK AREA first (flips to side_left, the camera that sees the object MOST); (2) a sliver secondary (mask < 0.4x c_own area) KEEPS its disocclusion patch (the leading edge c_own can't see) but (3) SKIPS the view-morph (fusing complete-into-sliver IS the shear).
> **RESULT (a309, `deliverables/db105_nearfield_geometry/a309_seam_AB_v2.jpg`):** `view_morph` 8.64 px -> `[]` (no morph); the car's shear is GONE, car renders COMPLETE (side_left body + front_left's leading-edge disocclusion, 5146 secondary px). v1 (drop secondary entirely) TRUNCATED the nose; v2 (keep disocclusion, drop only the morph) keeps it whole. Residual: a FAINT viewpoint-join at the front_left/side_left disocclusion boundary (physical — two cams image the nose from different angles) — but its FORM (a faint luminance seam) is far better than the morph's structural shear. **baseline (SS=False) byte-equivalent** (a309 re-run still 8.64 with 3 morph jobs) -> the change is fully gated, shipped path untouched.
> **NEXT (this loop):** (a) 5-scene regression — confirm SS=False byte-identical on all 5 + SS=True doesn't harm clean/bmw/downtown/crowd; (b) generality — crowd a50 (the other near-object morph case, 8-10 px); (c) disocclusion-edge photometric polish (gains/feather on the join); (d) THEN pivot to the GROUND blind spot (the one place that truly needs generation). Diagnostic drivers `tmp/_db105_ss*.py` (non-repo).
> ---

> ### 2026-06-21 ◆ DB-105 step-1+2 RESULT (L4, a309) — near-car seam is NOT disocclusion; LiDAR-coverage-gated SINGLE-SOURCE is the faithful, no-generation fix
> **Ran the diagnostic on the GPU, eyes-verified every image.** Target = the user's a309 silver sedan: dist **4.18 m**, REGULAR_VEHICLE 4.53×1.74 m, straddling the front_left/side_left seam.
> **Step-1 (LiDAR coverage, `deliverables/db105_nearfield_geometry/overlay_*.jpg`):** one anchor-sweep, box-interior points (NOT remove_dyn'd) = **1742 LiDAR points on the car**; projected into each ring cam: **ring_side_left 1610, ring_front_left 149**, rear_left 111, all others 0. Overlays eyeballed: side_left's points **blanket the WHOLE car** (roof/doors/windows/both wheels — calibration tight); front_left sees only a **tiny front-corner sliver**. ⇒ the car is NOT "front vs rear across two cams" (DB-104's framing) — it is **almost entirely inside ONE camera (side_left)**; front_left only grazes its leading edge.
> **Step-2 (ERP single-source splat, `step2_erp_singlesource.jpg`):** splatting the 1742 car points to the virtual-centre ERP, coloured per camera — **side_left single-source = a COMPLETE car across the full az span; front_left = a sliver at the front edge only**. side_left alone reconstructs the whole car; front_left is redundant.
> **DECISIVE conclusion:** the 8.6 px near-car residual is **NOT disocclusion and needs NO generation** — LiDAR densely covers the car (1742 pts), calibration is tight, and ONE camera sees it whole. The view-morph's error: STAGE-3.5 tries to **FUSE** the complete side_left car with front_left's useless 149-pt grazing sliver → that fusion IS the seam. Faithful fix = **LiDAR-coverage-gated SINGLE-SOURCE**: use per-camera visible-LiDAR-point count (1610 vs 149) as the objective trigger to single-source the more-complete camera and SKIP the morph. = exactly DB-104's deferred Tier-2 "graceful single-source degrade", now with a real failing case + an objective trigger. (DB-103's flow-morph 32→8.6 was *rescuing a fusion that shouldn't happen*; single-source removes the seam's CAUSE, not its symptom.)
> **Reframes the near-field map (important):** the two routes have DIFFERENT physical last-miles — STITCHING (near object) = **geometry single-source, FAITHFUL, no generation**; GROUND (0–3 m nadir) = the ONLY place that genuinely needs **generation (plausible)**. So the user's "plausible-not-faithful" intuition is RIGHT for the ground blind spot but UNNECESSARY for the near-object seam — the two were wrongly lumped together.
> **Step-3a (independent dense-reproject check, `step3_dense_singlesource.jpg`, does NOT touch shipped core):** side_left single-source dense reproject (nearest-neighbour LiDAR depth) of the car → renders as ONE complete body with NO fusion seam, BUT shows horizontal LiDAR-scanline striping (nearest-neighbour depth across sparse scan lines). ⇒ single-source removes the seam CAUSE; the implementation must densify depth along the (rigid) car surface (scanline interpolation / per-facet plane), not nearest-neighbour.
> **Step-3b (proposed — touches the Fable-5 view-morph core → needs go-ahead + 5-scene byte-regression):** implement LiDAR-coverage-gated single-source in run_case's c_own / STAGE-3.5 (dominant-camera by visible-LiDAR-point count → single-source it, gate the morph OFF) WITH car-surface depth densification, render a309, verify car complete + seam gone + clean frames byte-identical across 5 scenes. Diagnostic drivers `tmp/_db105_diag*.py` (non-repo). Evidence committed under `deliverables/db105_nearfield_geometry/`.
> ---

> ### 2026-06-21 ◇ DB-105 OPENED — new thinking-leader re-derives the NEAR-FIELD problem (both routes) from first principles + literature; opens the unified-solve brief
> **Mandate (user `/goal`):** re-think as leader; mine frontier literature; the ground-outpainting and the stitching routes "still have problems — everything CLOSE to the AV2 cameras goes wrong"; figure out how to solve; ~1h of reasoning. (Also: user floated "plausible-not-faithful — Cosmos will DiT-fill anyway" as a candidate, NOT a settled decision.)
> **First-principles unification (the core of this hour):** "closer = worse" because small Z simultaneously inflates FIVE quantities — parallax `d_px≈(W/2π)arctan(b/Z)`, occlusion severity (adjacent cams see DIFFERENT surfaces of a near object), grazing angle (near-ground = stretched low-res slivers), ERP-pole stretch, sparse-depth error × b_perp. These split into REPRESENTATION problems {parallax-rep, ERP-pole, single-depth-field} — ALREADY half-cured by DB-102 BEV metric domain — and PHYSICAL-OBSERVATION problems {occlusion boundary, grazing/blind-spot} that NO representation can fix. After stripping representation, the residual on BOTH routes is pure observation insufficiency: stitching = occlusion boundary (8.6 px floor, DB-104), ground = grazing low-res + 0–3 m true blind spot (DB-98/102). For observation insufficiency there are EXACTLY three responses: (A) TIME / whole-log (already used), (B) GEOMETRY (reconstruct→reproject→z-buffer, leaves disocclusion holes), (C) GENERATION (plausible, not truth — the ONLY filler of true-blind/disocclusion). A fails on true blind spot; B fails on disocclusion holes; C is plausible-not-faithful.
> **Literature mined (frontier, 2024–2026):** the modern large-parallax answer = B+C. **PIS3R** (arXiv 2508.04236, 2025-12) — VGGT 3D-reconstruct two images → reproject to a reference view → **point-conditioned diffusion** fills holes; **MagicRoad** (2507.23340) — planar Gaussian surfels + segmentation-guided **video inpaint** for road, HSV color-consistency; **OmniStitch** (ACM MM'24) depth-aware omni stitching; **UDIS++ / PTRS** parallax-tolerant seam-driven (visually-pleasing > geometric-exact); **DVI / DiffuEraser / flow-guided diffusion** temporally-consistent video inpaint (smooth-noise-progression). KEY: PIS3R unifies "faithful geometry + conditioned generation" — and we are ASSET-COMPLEMENTARY (we HAVE LiDAR/calib/poses/time; it estimates all from 2 images). Also: below-vehicle rendering (production surround-view) confirms the 0–3 m nadir is filled ONLY by historical frames + ego-motion (= our whole-log search) — no sensor sees under the car.
> **Vision pass (eyes, this hour):** `agent/_db102/crops/highway_fullcap_fill_top_bev_bot.png` — FILL near-cap = radial lane-line shards + speckle + black under-car wedge = a visual disaster ("looks-broken"); BEV near-cap = clean uniform grey but almost EMPTY (most of the nearest ring is the physical blind spot) = "honest but contentless". → for a "plausible" bar, fill FAILS (artifacts), bev is INSUFFICIENT (empty); the real want is a clean plausible texture on bev's correct geometry. `_db103/RESULT_board.png` confirms flow seam-fix (car nose intact) but middle-only.
> **Karpathy/Tesla cross-check (user-supplied Douyin):** verified the 3 tricks (virtual-camera rectification; BEV-transformer fusion; video-module feature-queue 27ms/1m + spatial RNN for occlusion). Decisive difference = OUTPUT SPACE: Tesla outputs abstract BEV (hallucination = feature, recall-from-memory of a truly-seen object), we output faithful RGB (fabricated salient geometry = bug that conflicts with Cosmos' point cloud). Mapping: rectification→residual-extrinsic optimisation (calib BA was NEG; needs JOINT not isolated); temporal→we already do it (whole-log); unified early-fusion net→that IS Cosmos, not our layer. Net: Karpathy BACKS the two-layer split + "blind-spot/occlusion completion belongs in the learned generative layer, not pixel-hard-fill".
> **Decision:** opened **DB-105** (decision_briefs.md) as the one ACTIVE brief — the unified near-field solve, step 1 = the faithful, contract-INDEPENDENT, variable-isolating geometry test (DB-104's explicitly-deferred "per-pixel car-LiDAR-depth reproject + z-buffer", NOT the box-single-depth it already NEG'd). Kill→if geometry can't beat 8.6 px / fill, the residual is pure disocclusion → pivot to C(generation)+the DB-94 contract experiment (two first-frames into Cosmos = the real total-switch for the plausible-vs-faithful question). Diagnostic only, ≤2 scenes, no shipped-pipeline change.
> **Infra:** L4 runtime confirmed live (22.04 GB free, this hour, status Read-verified, token in non-repo `~/.waymo2panorama/runtime/`). Drive housekeeping: MyDrive root had **11176 stray `active_url (NNNN).json`** (framework duplicate-write bug) → background deletion submitted (numbered copies only, maxdepth-1, runtime/ untouched).
> ---
