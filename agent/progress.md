...
> **VERDICT: A0 = NEG/MIXED → do NOT proceed to A1 as planned. PIVOT** (per DB-11 outer-loop): coarse-plane full re-render is not viable; options = confine plane-DIBR to flat-facade seam regions + HARD L1 fallback on non-planar/near-object/near-ground pixels (won't fix the BMW, only avoids making it worse), OR plane-sweep/MPI (DB-07, per-pixel measured depth), OR concede geometry-only can't single the cut car → diffusion/3DGS-with-real-reference (DB-02/03). Bring to user. Driver `scripts/phase3/run_a0_plane_dibr_probe.py`. Locations — GitHub committed; local `deliverables/a0_plane_dibr_probe/`; Drive `results/a0_plane_dibr_probe/`.
> ---
