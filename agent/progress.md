...
> **Step-3 (proposed — touches the Fable-5 view-morph core → needs go-ahead + 5-scene byte-regression):** implement LiDAR-coverage-gated single-source in run_case's c_own / STAGE-3.5 (when one camera's visible-LiDAR-point count dominates the straddling object, single-source it and gate the morph OFF), render a309, verify car complete + seam gone + clean frames byte-identical across 5 scenes. Diagnostic drivers `tmp/_db105_diag*.py` (non-repo). Evidence committed under `deliverables/db105_nearfield_geometry/`.
> ---
