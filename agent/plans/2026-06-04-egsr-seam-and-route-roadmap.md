...
DB45 is **paused** by executor DNS/tunnel availability after DB45i repeatedly stopped before `/status`/`/exec`; this is not a VGGT/model negative and accepts no geometry evidence. DB45g is completed as **vggt-official-source-decode-path-diagnostic-only**: official source documents a pose/camera decode path, but DB45f did not store pose tensors/decoded extrinsics and runtime source/API inspection remains blocked by executor DNS/tunnel availability. DB45h is completed as **vggt-residual-job-contract-only**: it defines the future extractor contract but accepts no geometry evidence. DB45i is the first calibrated residual extractor but accepts only a paused connectivity record: no pose tensor, decoded camera, Sim(3), LiDAR/raw residual, model inference, geometry evidence, permission change, or RED promotion exists yet. The next VGGT action should be only the same DB45i `--run-remote` extractor after a reachable executor endpoint is available. Owner-camera confidence and target-ROI owner-UV model confidence are both now known insufficient for promotion. DB47 is now running as the source/frame candidate-mining sidestep route. DB46/DB48 remain side branches for presentation-only or center-preserve experiments and should not jump ahead unless the user explicitly switches priority for meeting/demo needs.
...
- DB45i VGGT calibrated residual extractor: opened but blocked-or-paused on executor DNS before `/status`/`/exec`; a fresh A100 executor recheck still failed DNS before `/status`/`/exec`; local extractor contract/manifest/board exist; no model action, no pose/decode/Sim(3)/residual evidence, no permission change, no RED promotion. Re-run only the same DB45i `--run-remote` when executor connectivity is restored.
