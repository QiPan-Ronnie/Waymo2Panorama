...
- DB45i VGGT calibrated residual extractor: opened but blocked-or-paused on executor DNS before `/status`/`/exec`; local extractor contract/manifest/board exist; no model action, no pose/decode/Sim(3)/residual evidence, no permission change, no RED promotion. Re-run only the same DB45i `--run-remote` when executor connectivity is restored.
