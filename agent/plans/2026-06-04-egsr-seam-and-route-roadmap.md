...
DB58 VGGT-assisted raw-camera-backed seam ROI repair feasibility is now opened in `agent/decision_briefs.md` as the **primary prepared next attempt**, but it has not run. It reframes the remaining seam problem as a Google/Meta-style evidence gap: VGGT may help only if it provides target-surface geometry evidence that can be checked against source owner/UV mapping, rig/LiDAR/flow, and protected structure masks. DB58 defaults to the DB25 longline ROI `[850, 420, 1650, 720]` on `02a00399-3857-444e-8db3-a8f58489c394` / anchor `0`, while DB41 right/lower-right stays a negative control/abstain boundary. The intended output location is `deliverables/dit360_v2/db58_vggt_raw_camera_seam_roi/`. It allows no prompt-only DiT/FLUX repair, no generated ground/curb/lane/right-line pixels, no source replacement, no RED promotion, and no training-ready claim. If raw-owner/UV, VGGT coordinate/reflection, target-surface evidence, or fake-geometry gates fail, DB58 must stop as abstain/no-repair and record the result in `progress.md`.
