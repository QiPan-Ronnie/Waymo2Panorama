...
- DB45g VGGT pose/pointmap metric-residual readiness gate: opened and paused on executor/tunnel availability (HTTP 530 earlier; latest hostname DNS `NXDOMAIN` / `getaddrinfo failed` before `/exec`); source/API inspection has not started; no inference.
