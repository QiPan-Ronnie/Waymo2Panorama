...
DB45 is **not closed** by these readiness/diagnostic passes. DB45g is completed as **vggt-official-source-decode-path-diagnostic-only**: official source documents a pose/camera decode path, but DB45f did not store pose tensors/decoded extrinsics and runtime source/API inspection remains blocked by executor DNS/tunnel availability. DB45h is completed as **vggt-residual-job-contract-only**: it defines the future extractor contract but accepts no geometry evidence. DB45i is opened as the first calibrated residual extractor but is **blocked-or-paused on executor DNS before `/status`/`/exec`**, so it accepts only a paused connectivity record: no pose tensor, decoded camera, Sim(3), LiDAR/raw residual, model inference, geometry evidence, permission change, or RED promotion exists yet. A fresh A100 executor recheck still failed DNS before `/status`/`/exec` in both sandboxed and approved non-sandbox network attempts. The next VGGT action should be only the same DB45i `--run-remote` extractor after a reachable executor endpoint is available. Owner-camera confidence and target-ROI owner-UV model confidence are both now known insufficient for promotion. DB46/DB48 remain side branches for presentation-only or center-preserve experiments and should not jump ahead unless the user explicitly switches priority for meeting/demo needs.
