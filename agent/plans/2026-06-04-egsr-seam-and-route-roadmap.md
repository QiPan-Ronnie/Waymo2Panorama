...
DB45 is **paused** after DB45j accepted `vggt-calibrated-residual-diagnostic-only` evidence. DB45g is completed as **vggt-official-source-decode-path-diagnostic-only** and DB45h as **vggt-residual-job-contract-only**. DB45j restored official VGGT setup/checkpoint readiness, then DB45i ran one official inference and saved `pose_enc`, decoded cameras, preprocessing mapping, Sim(3), and DB25/DB41 residual tables. It still accepts no geometry evidence because `sim3_contract_thresholds_pass=false` with `reflection_detected=true`, ROI raw/LiDAR residuals remain large, and every ROI keeps `permission_promotion_allowed=false`; DB41 lower-right remains known LiDAR support `0.000`. The next VGGT action should not be patch-on-patch; it needs a fresh bounded reflection/coordinate evidence brief if pursued at all. Owner-camera confidence, target-ROI owner-UV model confidence, and calibrated residual diagnostics are all now insufficient for RED promotion under current gates. DB47 is now running as the source/frame candidate-mining sidestep route. DB46/DB48 remain side branches for presentation-only or center-preserve experiments and should not jump ahead unless the user explicitly switches priority for meeting/demo needs.
