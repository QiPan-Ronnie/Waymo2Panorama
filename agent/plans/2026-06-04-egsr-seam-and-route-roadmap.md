...
DB45 is now paused after DB45i reached the latest A100 executor but failed before official VGGT inference because the remote runtime could not import `vggt`. Its phase0 evidence-only control/registry pass is completed: outputs are under `deliverables/dit360_v2/db45_geometry_evidence_audit/`, with script `scripts/phase3/db45_geometry_evidence_audit.py`. DB45 v0 fixed 8 controls, registered reusable evidence sources, verified A100 live/env/cache state only, and kept all permission states unchanged (`GREEN=1`, `YELLOW=2`, `RED=5`; no RED promotion; no foundation-model confidence claimed).
...
DB45 is **paused** after DB45i reached the latest A100 executor (`/status` passed and one `/exec` job was submitted) but the remote runtime failed before official VGGT inference with `ModuleNotFoundError: No module named 'vggt'`; this is not a VGGT/model negative and accepts no geometry evidence. DB45g is completed as **vggt-official-source-decode-path-diagnostic-only**: official source documents a pose/camera decode path, but DB45f did not store pose tensors/decoded extrinsics. DB45h is completed as **vggt-residual-job-contract-only**: it defines the future extractor contract but accepts no geometry evidence. DB45i is the first calibrated residual extractor but currently accepts only a paused runtime/import blocker record: no pose tensor, decoded camera, Sim(3), LiDAR/raw residual, model inference, geometry evidence, permission change, or RED promotion exists yet. The next VGGT action should not be an ad hoc DB45i patch-on-patch; it needs a fresh bounded runtime-bootstrap/cache-restore plus residual-retry brief or explicitly scoped setup replay. Owner-camera confidence and target-ROI owner-UV model confidence are both now known insufficient for promotion. DB47 is now running as the source/frame candidate-mining sidestep route. DB46/DB48 remain side branches for presentation-only or center-preserve experiments and should not jump ahead unless the user explicitly switches priority for meeting/demo needs.
...
These are roadmap items, not direct execution commands. DB43 and DB44 are completed and accepted; DB45 is paused after DB45i reached the latest executor but hit a remote runtime `vggt` import blocker, and DB47 is the current running source/frame candidate-mining sidestep brief. Any remaining item must become or remain a live decision brief before work starts.
...
- DB45i VGGT calibrated residual extractor: opened and latest A100 endpoint reached `/status` plus one `/exec` job, but the remote runtime failed before official VGGT inference with `ModuleNotFoundError: No module named 'vggt'`; local extractor contract/manifest/board exist; no model inference, no pose/decode/Sim(3)/residual evidence, no permission change, no RED promotion. Do not ad hoc install/rerun under DB45i; next VGGT residual attempt needs a fresh bounded runtime-bootstrap/cache-restore plus residual-retry brief or explicitly scoped setup replay.
