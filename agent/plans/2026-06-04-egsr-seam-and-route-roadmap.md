...
DB45 is **not closed** by these readiness/diagnostic passes. DB45g is now opened as the next bounded readiness question: can VGGT pose/pointmap outputs be officially decoded and calibrated against the known camera rig/LiDAR well enough to justify a future residual job? DB45g is source/API inspection only: no model inference, no model load, no render/repair/source replacement, no RED promotion. Its runtime source/API inspection has not actually started: one provided executor tunnel returned HTTP `530`, and a later provided tunnel hostname failed DNS resolution (`NXDOMAIN` / `getaddrinfo failed`) before `/exec` submission. This is executor/tunnel availability only, not a VGGT/API negative. DB45g now accepts only `vggt-official-source-decode-path-diagnostic-only`: official source documents a pose/camera decode path, but DB45f did not store pose tensors/decoded extrinsics and runtime readiness remains false. Owner-camera confidence and target-ROI owner-UV model confidence are both now known insufficient for promotion. DB46/DB48 remain side branches for presentation-only or center-preserve experiments and should not jump ahead unless the user explicitly switches priority for meeting/demo needs.
...
- DB45g VGGT pose/pointmap metric-residual readiness gate: accepts official-source decode-path diagnostic-only; runtime source/API inspection is still paused on executor/tunnel availability (HTTP 530 earlier; latest hostname DNS `NXDOMAIN` / `getaddrinfo failed` before `/exec`); DB45f has pose keys but no stored pose tensor/decoded extrinsics; no inference, no geometry evidence, no RED promotion.
