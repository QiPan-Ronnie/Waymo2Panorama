...
## 4.5 移植实测：三个 rig 的带内覆盖率

`ELEV_DEG = 35` 是在 AV2 上定的。各 rig 的**垂直 FOV 不同**，同一个带宽下真实内容占比差别很大
（实测，取每个 rig 最窄的那路相机）：

| 数据集 | 最小垂直半 FOV | ±35° | ±30° | ±25° | ±20° | ±15° |
|---|---|---|---|---|---|---|
| Argoverse 2 | 24.6° | **70%** | 82% | 94% | 100% | 100% |
| nuScenes | 19.5° | **59%** | 69% | 81% | 95% | 100% |
| Waymo Percep | 12.1° | **27%** | 32% | 38% | 47% | 57% |

⚠️ **Waymo Perception 在 ±35° 带内只有 27% 是真实像素**，另外 73% 是黑的
（158° 方位空洞 + 最窄相机仅 ±12.1° 垂直 FOV）。这比"车后 130° 洞"的估计严重得多。

这不是缺陷 —— v15 契约本来就写明「sky-top 和 nadir 是黑的，相机物理上看不到」。
但 **27% 这个数字 koi 没见过**，属于量产前必须让他看到的信息（B7 眼验时一并报）。

若要提高密度，唯一手段是按 rig 收窄带宽，代价是各源 ERP 几何不再一致 —— 
**这是数据语义决策，不由我们单方面决定**。

### 去畸变的 FOV 代价（已知取舍，选了安全的一侧）

Waymo 去畸变用 `getOptimalNewCameraMatrix(alpha=0)`，即裁掉所有无效边缘像素。
代价：方位覆盖 230° → **202°**（洞从 130° 变 158°）。
选它是因为 `alpha=1` 会把无效区留成黑边，而那些黑边会被当作"真实像素"采样进 KEEP，
直接违反 I3。要两者兼得需把去畸变有效性掩膜一路带进 `written`，属未做项。
