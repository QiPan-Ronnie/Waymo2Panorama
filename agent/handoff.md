...
**Updated**: 2026-05-28 (Latest: temporal ego-motion ground seam probe was tested after DiT360 v14, region-coherent seam v3a/v3b, and DiT-as-oracle source selection. It adds real temporal evidence but is still NEG as a seam solver: one ground plane inserts dragged strips and cannot handle vehicles/facades/poles. Moving/changing the source seam and one-plane temporal replacement are now near exhausted. Risk-gated local Y repair remains the stable POS optional polish.)
...
**Latest temporal evidence test**: `scripts/phase3/test_temporal_ground_seam.py` used nearby 20Hz AV2 frames and ego poses to synthesize seam-band evidence on a local ground plane. This is a real new-information route, not another seam cost tweak, but the one-plane assumption fails visually: BMW/fbee/0bae repairs insert dragged rectangular strips, smear pedestrians/cars/facades, and reduce source-fidelity NCC from ~1.0 to 0.8583/0.8015/0.7588. Evidence in `deliverables/temporal_ground_seam/three_anchor_v1/`. Conclusion: temporal may still matter, but only with layered/object/depth reasoning; one-plane temporal replacement is not the solver.
