...
**Updated**: 2026-05-27 (Latest: Stage B DiT360 v9 multi-seed check finished on A100. r008/r010 tau5 with seed 1/2 did not rescue seam completion; evidence gating can suppress raw DiT360 halo hallucination while preserving non-candidate pixels exactly, but it collapses the result back toward hard_select and still does not repair seam geometry. Verdict is NEG for main method / usable only as qualitative baseline. Prior: no-DL DP seam-routing v2 NEG / weak MIXED; seam-local ECC MIXED/weak NEG; seam-root-cause investigation; Xihan Waymo E2ED L1+L2 HDR color shift solved; hard_hdr_of NCC +25.3% but visually risky on near-field large objects.)
