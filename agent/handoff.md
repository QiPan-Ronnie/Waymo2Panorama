...
>
> DB43 Source-Faithfulness Gate and DB44 EGSR dispatcher are accepted. DB45 Geometry foundation evidence audit is still running. Completed DB45 substeps: DB45a current-runtime VGGT no-go; DB45b permission calibration; DB45c HF Commercial access/schema gate; DB45d official VGGT setup/load smoke; DB45e official VGGT frozen-ROI confidence probe; DB45f official VGGT target-ROI owner-UV sampling gate. DB45e accepts only `vggt-roi-confidence-diagnostic-only`, and DB45f accepts only `vggt-target-uv-sampling-diagnostic-only`: official VGGT ran once on BMW anchor 0 and produced real `depth_conf` / `world_points_conf`, then DB45f sampled those maps at source-owner raw-camera UV pixels used by the frozen ERP seam ROIs. This is stronger diagnostic metadata than owner-camera summaries, but still not target-surface geometry evidence, not metric ego truth, and not source-faithful repair permission. It kills VGGT confidence-only RED promotion. DB45g accepts only `vggt-official-source-decode-path-diagnostic-only`: official source documents a pose/camera decode path, and local DB45f records `pose_enc` as a prediction key, but DB45f did not store pose tensors/decoded extrinsics and the runtime source/API inspection is still blocked by executor DNS/tunnel availability. DB45g therefore has `residual_readiness=false`, no accepted geometry evidence, no permission-state change, and no RED promotion. DB25/DB41 remain `RED/abstain`; DB41 lower-right remains zero-LiDAR abstain; DB36/DB40 remain generated fake-geometry rejects.
>
> Next source-faithful DB45 work must open a fresh bounded brief and bring true target-surface evidence beyond VGGT confidence (track consistency, LiDAR/raw residuals, calibrated pointmap checks, or equivalent). Owner-camera and target-ROI owner-UV VGGT confidence alone are now known insufficient. Do not reopen prompt-only DiT/FLUX ground/curb/lane/right-line repair, and do not continue any direction after kill criteria without archiving it in `progress.md`.
