...
**Updated**: 2026-05-28 (Latest: risk-gated local Y seam repair tested on three AV2 anchors. It keeps hard_select geometry and only adjusts low-structure-risk seam luminance, reducing mean seam ΔY by 10-19% with no visible new ghost/warp in the crop review. Prior: source-evidence seam confidence map v1 POS as metadata; DiT360 v9 multi-seed NEG as main solver; no-DL DP seam-routing v2 NEG / weak MIXED; seam-local ECC MIXED/weak NEG; seam-root-cause investigation; Xihan Waymo E2ED L1+L2 HDR color shift solved.)
...
**6 layers of "no-depth / no-DL" seam fixes tested plus one DiT360 generative seam-completion path tested. Seam-local alignment is safer than full OF, and DP seam-routing is a clean hard-select seam ablation, but neither beats L1 hard_select. DiT360 can fill masked seam strips; post-compose and evidence-gated composition can force exact non-candidate preservation, but the generated strip either changes too little to fix geometry or hallucinates vertical/block artifacts when given enough freedom. New source-evidence seam confidence maps are POS as diagnostics, and risk-gated local Y repair is POS as conservative seam-color polish: it reduces luminance seams without claiming to solve impossible geometry.**
...
| 7 | Stage B — DiT360 masked seam completion + post/evidence-gated compose | ❌ NEG as main solver. Non-candidate pixels can be preserved exactly, but safe gates revert toward hard_select; wider/free edits hallucinate artifacts; multi-seed does not rescue |
...
| 9 | L2 — risk-gated local Y seam repair | ✅ POS as conservative polish. Mean seam ΔY -10% to -19% on 3 anchors; no new ghost/warp; does not fix geometry |
...
**Latest optional repair**: `scripts/phase3/seam_risk_gated_color_repair.py` uses the confidence map to adjust only Y-channel luminance near low-structure-risk seam regions. It keeps hard_select camera assignment, leaves chroma untouched, and makes no geometry/warp/DL changes. On three anchors it reduces mean seam ΔY by 10.17%, 18.73%, and 18.93% with changes limited to ~3.1-3.6% of pixels and no visible new artifact in the crop review. Treat this as an optional local L2 seam-color polish; expand to 11 fresh anchors before making it a default.

**Latest DiT360 refinement**: the first r040/tau20 DiT360 run was visual NEG. Follow-up r004/r008/r012/r020 seam masks plus invalid-region outpainting showed that hard post-compose is mandatory: it restores all white-mask pixels exactly. v7 small-mask/tau sweep (r006/r008/r010/r012/r014, tau 3/5/8) found r008/tau5 visually most plausible but still weak. v8 evidence-gated composition then downweighted DiT edits near strong source edges and large raw-vs-source changes; it reduced suspicious halo/soft-block artifacts, but also made the output nearly identical to L1 `hard_select` and did not repair lane/road seam geometry. v9 multi-seed check (r008/r010 tau5, seed 1/2) did not find a better seed. Treat DiT360 as a constrained qualitative baseline or low-risk texture repair experiment, not a Bosch training-data solver.
...
**4 paths from here**:
- **Source-evidence confidence maps + gated color repair** — current best forward direction. Keep L1 `hard_select` as the panorama, output seam risk/confidence metadata for Bosch filtering/loss weighting, and optionally apply low-risk local Y seam repair. Evidence in `deliverables/seam_confidence_map/three_anchor_v1/` and `deliverables/seam_risk_gated_color_repair/three_anchor_v1/`.
- **DiT360 as constrained qualitative baseline** — A100 BMW r040 run succeeded after HF auth, `torchao` upgrade, and VAE tiling. Follow-up outpaint + small seam masks showed raw DiT edits still rewrite evidence, while post-compose restores non-mask pixels exactly. v8 evidence gating suppresses risky edits near lane/building/car edges, but then the result largely reverts to hard_select and still does not solve geometry. v9 multi-seed does not rescue it. Keep it as a paper discussion/baseline or low-risk texture repair ablation, not as a Bosch training-data solver.
