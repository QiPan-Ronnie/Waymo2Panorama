...
**Updated**: 2026-05-28 (Latest: DiT360 v10 adaptive masks + fidelity-budget + low-frequency harmonization were tested on the BMW anchor. They sharpen the diagnosis but do not change the top-level recommendation: raw/loose DiT looks smoother only by rewriting evidence; low-frequency-only DiT is safer but only a color/harmony ablation, not geometry repair. Risk-gated local Y repair is now expanded to fresh11: 11/11 anchors improve mean seam dY, mean reduction 18.19%, no visible new warp/ghost in compact review.)
...
**6 layers of "no-depth / no-DL" seam fixes tested plus the DiT360 generative seam-completion path pushed through fixed masks, adaptive masks, post/soft/evidence compose, multi-seed, fidelity-budget compose, and low-frequency-only harmonization. Seam-local alignment is safer than full OF, and DP seam-routing is a clean hard-select seam ablation, but neither beats L1 hard_select. DiT360 can fill masked seam strips, but the visually smoother variants rely on nontrivial source rewriting; safe variants revert toward hard_select or only change low-frequency color. New source-evidence seam confidence maps are POS as diagnostics, and risk-gated local Y repair is POS as conservative seam-color polish: it reduces luminance seams without claiming to solve impossible geometry.**
...
**Latest DiT360 refinement**: the first r040/tau20 DiT360 run was visual NEG. Follow-up r004/r008/r012/r020 seam masks plus invalid-region outpainting showed that hard post-compose restores all white-mask pixels exactly but can reintroduce hard boundaries. v7 small-mask/tau sweep found r008/tau5 visually most plausible but still weak; v8 evidence gating reduced suspicious edits but reverted toward hard_select; v9 multi-seed did not rescue it. v10 added source-risk adaptive masks (`adaptive_lowstruct_r006`, `adaptive_color_r008_guardstruct`, `adaptive_expand_histruct_r024`), fidelity-budget compose, and low-frequency-only harmonization. Raw/adaptive outputs still need preserve MAE around 4 to look smooth, and those edits visibly rewrite evidence. Loose budget versions expose the same knob: budget2 is safer but still altered; budget4 is close to raw and brings hallucination back. Low-frequency-only DiT is the safest learned variant (edge-region output MAE 0.13-1.41 vs raw 16.99-21.48), but it only harmonizes color/illumination and does not repair geometry. Treat DiT360 as a paper discussion/baseline or low-frequency color prior, not a Bosch training-data solver.
...
- **DiT360 as constrained qualitative baseline** — A100 BMW r040 run succeeded after HF auth, `torchao` upgrade, and VAE tiling. Fixed masks, adaptive masks, post/soft/evidence/fidelity compose, multi-seed, and low-frequency-only harmonization were tested. The useful variants are qualitative/color-prior ablations; none is source-faithful geometry repair. Evidence in `deliverables/dit360_seam_completion/runs_v10_*`.
