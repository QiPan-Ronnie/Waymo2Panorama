...
>
> DB43 Source-Faithfulness Gate and DB44 EGSR dispatcher are accepted. DB45 Geometry foundation evidence audit is paused on DB45i executor DNS before `/status`/`/exec`: completed DB45 substeps DB45a-i accept only no-go/permission/readiness/diagnostic/contract/blocked evidence, kill VGGT confidence-only RED promotion, and define the residual extractor contract, but still accept no target-surface geometry evidence, no metric ego truth, no source-faithful repair permission, no permission-state change, and no RED promotion. The latest fresh A100 endpoint recovery recheck again stopped at `URLError getaddrinfo failed` before `/status` or `/exec`; no remote job, model inference, pose/decode/Sim(3), residual table, or geometry evidence exists. DB45i may be rerun only as the same bounded `--run-remote` extractor after a reachable executor endpoint is independently confirmed: it must save pose tensors/decoded extrinsics, align VGGT camera centers to the Waymo rig by Sim(3), and compare target-surface samples to LiDAR/raw residuals before any geometry claim. DB47 source/frame candidate mining is now running as the CPU/local sidestep route; DB47a accepted only `source-selection-inventory-only`, DB47b accepted only `source-selection-threshold-replay-only`, and DB47c accepted only `source-selection-visual-accounting-only` evidence. DB47c reviews all 22 DB47b rows using existing DB28/DB31 visuals: 3 strict rows have exact same-log review assets, 4 strict rows are montage-only holds, 3 relaxed same-log rows are holds, and all 12 rejected/diagnostic rows remain rejected or diagnostic. DB47c selects no final candidate and performs no full scan, repair, generation, source replacement, permission change, or RED promotion. DB28/DB32 remain source-sidestep/current-handoff evidence only, not original-G seam repair and not a source-faithful ceiling. DB25/DB41 remain `RED/abstain`; DB41 lower-right remains zero-LiDAR abstain; DB36/DB40 remain generated fake-geometry rejects.
>
> Next DB45 work must wait for independently confirmed `/status` reachability or a fresh bounded evidence brief; do not continue DB45i patch-on-patch. Next DB47 work, if opened, must be a fresh bounded exact same-log review for strict/relaxed rows or a separately briefed fixed-universe full scan; do not choose a final candidate from DB47b/DB47c metrics or montage-only evidence. Do not reopen prompt-only DiT/FLUX ground/curb/lane/right-line repair, and do not continue any direction after kill criteria without archiving it in `progress.md`.
