...
>
> DB43 Source-Faithfulness Gate and DB44 EGSR dispatcher are accepted. DB45 Geometry foundation evidence audit is still running. Completed DB45 substeps: DB45a current-runtime VGGT no-go; DB45b permission calibration; DB45c HF Commercial access/schema gate; DB45d official VGGT setup/load smoke; DB45e official VGGT frozen-ROI confidence probe; DB45f official VGGT target-ROI owner-UV sampling gate; DB45g official-source pose decode diagnostic; DB45h calibrated residual job contract; DB45i calibrated residual extractor is opened but blocked-or-paused on executor DNS before `/status`/`/exec`, including the latest fresh-A100 recovery recheck. DB45e accepts only `vggt-roi-confidence-diagnostic-only`; DB45f accepts only `vggt-target-uv-sampling-diagnostic-only`; DB45g accepts only `vggt-official-source-decode-path-diagnostic-only`; DB45h accepts only `vggt-residual-job-contract-only`; DB45i currently accepts only `blocked-or-paused` evidence. Together these kill VGGT confidence-only RED promotion and define the residual extractor contract, but still accept no target-surface geometry evidence, no metric ego truth, no source-faithful repair permission, no permission-state change, and no RED promotion. DB45i may be rerun only as the same bounded `--run-remote` extractor after a reachable executor endpoint is available: it must save pose tensors/decoded extrinsics, align VGGT camera centers to the Waymo rig by Sim(3), and compare target-surface samples to LiDAR/raw residuals before any geometry claim. DB25/DB41 remain `RED/abstain`; DB41 lower-right remains zero-LiDAR abstain; DB36/DB40 remain generated fake-geometry rejects.
>
> Next source-faithful DB45 work must stay inside the active DB45i brief while the executor blocker is the only issue, or open a fresh bounded brief for a genuinely new evidence source. It must bring true target-surface evidence beyond VGGT confidence (track consistency, LiDAR/raw residuals, calibrated pointmap checks, or equivalent). Owner-camera and target-ROI owner-UV VGGT confidence alone are now known insufficient. Do not reopen prompt-only DiT/FLUX ground/curb/lane/right-line repair, and do not continue any direction after kill criteria without archiving it in `progress.md`.
