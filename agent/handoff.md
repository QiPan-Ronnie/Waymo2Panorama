...
**Updated**: 2026-05-28 (Latest: source-evidence seam confidence map v1 tested on three AV2 anchors. It does not stitch better by itself, but it cleanly separates broad color-risk seam regions from sparse high-structure geometry conflicts and is promising as Bosch filtering/loss-weighting metadata. Prior: DiT360 v9 multi-seed NEG as main solver; no-DL DP seam-routing v2 NEG / weak MIXED; seam-local ECC MIXED/weak NEG; seam-root-cause investigation; Xihan Waymo E2ED L1+L2 HDR color shift solved; hard_hdr_of NCC +25.3% but visually risky on near-field large objects.)
...
**6 layers of "no-depth / no-DL" seam fixes tested plus one DiT360 generative seam-completion path tested. Seam-local alignment is safer than full OF, and DP seam-routing is a clean hard-select seam ablation, but neither beats L1 hard_select. DiT360 can fill masked seam strips; post-compose and evidence-gated composition can force exact non-candidate preservation, but the generated strip either changes too little to fix geometry or hallucinates vertical/block artifacts when given enough freedom. New source-evidence seam confidence maps are POS as diagnostics: they do not claim to repair impossible geometry, but they expose where L1 hard_select is low-risk vs high-risk.**
...
**4 paths from here**:
- **Source-evidence confidence maps** — current best forward direction. Keep L1 `hard_select` as the panorama, output seam risk/confidence metadata for Bosch filtering/loss weighting, and use the map to gate only low-risk color repair. Evidence in `deliverables/seam_confidence_map/three_anchor_v1/`.
