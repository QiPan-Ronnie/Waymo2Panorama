...
**Updated**: 2026-05-27 (Latest: Stage B DiT360 outpaint + tiny seam-mask post-compose tested on A100. Post-compose forces non-mask fidelity, but tiny masks do not repair seam geometry and wider masks hallucinate vertical/block artifacts, so verdict is MIXED / weak NEG for Bosch training data. Prior: no-DL DP seam-routing v2 NEG / weak MIXED; seam-local ECC MIXED/weak NEG; seam-root-cause investigation; Xihan Waymo E2ED L1+L2 HDR color shift solved; hard_hdr_of NCC +25.3% but visually risky on near-field large objects.)
...
| 7 | Stage B — DiT360 masked seam completion + post-compose | ⚠️ MIXED / weak NEG. Non-mask pixels can be preserved exactly, but tiny masks do not fix geometry and wider masks hallucinate artifacts |

**Latest DiT360 refinement**: the first r040/tau20 DiT360 run was visual NEG. Follow-up r004/r008/r012/r020 seam masks plus invalid-region outpainting showed that hard post-compose is mandatory: it restores all white-mask pixels exactly. Even with this constraint, r004/r008 mostly leaves L1 `hard_select` unchanged, while r012/r020 introduces visible vertical/block artifacts near SUV/building seams. Treat DiT360 as a constrained qualitative baseline, not a Bosch training-data solver yet.
...
**4 paths from here**:
- **DiT360 as constrained qualitative baseline** — A100 BMW r040 run succeeded after HF auth, `torchao` upgrade, and VAE tiling. Follow-up outpaint + small seam masks showed raw DiT edits still rewrite evidence, while post-compose restores non-mask pixels exactly. Remaining issue is the generated strip itself: r004/r008 is too narrow to repair geometry; r012/r020 creates visible vertical/block artifacts. Keep it as a paper discussion/baseline, not as a Bosch training-data solver yet.
