...
>
> Next DB45 work, if any, must bring new official-source convention evidence in a fresh bounded brief; do not continue VGGT residual patch-on-patch, rerun inference, or reinterpret large residuals as permission. Next DB47 work should pause unless a fresh brief opens a fixed-universe full scan; do not promote missing-exact holds or choose candidates from metrics/montage alone. Next DB49 work, if opened, should run a bounded exact-lineage rerun using the DB49d sidecar support to build a real `source_id_map`/full masks from source-ownership evidence, or package a human-facing handoff while keeping missing fields explicit; do not narratively fill `source_id_map`, `unknown_or_abstain_mask`, or `risk_map`. Do not reopen prompt-only DiT/FLUX ground/curb/lane/right-line repair, and do not continue any direction after kill criteria without archiving it in `progress.md`.
