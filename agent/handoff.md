...
>
> DB43 Source-Faithfulness Gate and DB44 EGSR dispatcher are accepted. DB45 Geometry foundation evidence audit is paused on DB45i executor DNS before `/status`/`/exec`: completed DB45 substeps DB45a-i accept only no-go/permission/readiness/diagnostic/contract/blocked evidence, kill VGGT confidence-only RED promotion, and define the residual extractor contract, but still accept no target-surface geometry evidence, no metric ego truth, no source-faithful repair permission, no permission-state change, and no RED promotion. The latest goal-continuation reachability precheck again failed DNS on `GET /status` in both sandboxed and approved non-sandbox network; no `/exec` request, remote job, model inference, pose/decode/Sim(3), residual table, or geometry evidence exists. DB45i may be rerun only as the same bounded `--run-remote` extractor after a reachable executor endpoint is independently confirmed: it must save pose tensors/decoded extrinsics, align VGGT camera centers to the Waymo rig by Sim(3), and compare target-surface samples to LiDAR/raw residuals before any geometry claim. DB47 source/frame candidate mining is running as the CPU/local sidestep route; DB47a accepted only `source-selection-inventory-only`, DB47b accepted only `source-selection-threshold-replay-only`, DB47c accepted only `source-selection-visual-accounting-only`, and DB47d accepted only `source-selection-exact-review-pack-only` evidence. DB47d reviews the 10 strict/relaxed same-log rows: only `a105`, `a200`, and `a204` have exact DB28 compare evidence, while the other 7 rows remain missing-exact holds. DB47d selects no final candidate and performs no full scan, repair, generation, source replacement, permission change, or RED promotion. DB49a is completed as `bosch-data-contract-inventory-only`; DB49b is completed as `sidecar-starter-pack-partial-only`, creating only partial DB32 `generated_mask`, `unknown_or_abstain_mask`, and `risk_map` sidecars from existing sky-mask/out-of-FOV/DB41 evidence; DB49c is completed as `source-id-map-feasibility-inventory-only`, finding no complete per-pixel `source_id_map` artifact for the exact DB32 lineage and creating no map; DB49d is completed as `source-map-instrumentation-only`, adding default-off `_seamroute.py` provenance sidecar export for future exact reruns but creating no DB32 map. DB49c records DB28/DB41 camera labels as ROI diagnostic/count evidence only, DB34 source preservation as not an owner map, and DB49d sidecar support as future reproducible support only, not a DB32 owner artifact until an exact lineage rerun validates it. DB49 creates no repaired image, no generated pixels beyond existing DB32 sky caveats, no permission-state change, and no RED promotion; DB32 `s40` remains not uncaveated Bosch training data. DB28/DB32 remain source-sidestep/current-handoff evidence only, not original-G seam repair and not a source-faithful ceiling. DB25/DB41 remain `RED/abstain`; DB41 lower-right remains zero-LiDAR abstain; DB36/DB40 remain generated fake-geometry rejects.
>
> Next DB45 work must wait for independently confirmed `/status` reachability or a fresh bounded evidence brief; do not continue DB45i patch-on-patch. Next DB47 work, if opened, must be a stricter final-candidate review or a separately briefed fixed-universe full scan; do not choose a final candidate from DB47b/c/d metrics, montage-only evidence, or review-pack labels alone. Next DB49 work, if opened, must either run a bounded exact-lineage rerun using the DB49d sidecar support to build a real `source_id_map`/full masks from source-ownership evidence or package a human-facing handoff while keeping missing fields explicit; do not narratively fill `source_id_map`, `unknown_or_abstain_mask`, or `risk_map`. Do not reopen prompt-only DiT/FLUX ground/curb/lane/right-line repair, and do not continue any direction after kill criteria without archiving it in `progress.md`.
