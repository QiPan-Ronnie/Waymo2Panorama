...
>
> DB58 is now opened in `agent/decision_briefs.md` as the **primary prepared next attempt**, but it has not run. Its question is whether VGGT can provide enough target-surface geometry evidence to repair one fixed curved seam ROI by raw-camera-backed local warp/composite, not by prompt inpainting. Default target is the DB25 longline ROI `[850, 420, 1650, 720]` on `02a00399-3857-444e-8db3-a8f58489c394` / anchor `0`; DB41 right/lower-right remains a negative control/abstain boundary unless a separate brief brings new evidence. DB58 must first recover or prove per-pixel raw owner/UV evidence, then gate VGGT pose/depth/point output against rig/LiDAR/flow and protected lane/curb/object/building-edge masks. If any gate fails, DB58 stops as abstain/no-repair. No DiT/FLUX generation, source replacement, RED promotion, or training-ready claim is allowed under DB58.
>
