...
**Updated**: 2026-05-28 (Latest: same-frame raw ground-plane seam layer was tested after temporal ego-motion ground replacement, DiT360 v14, region-coherent seam v3a/v3b, and DiT-as-oracle source selection. It targets the user-flagged left2 -> 3 road/lane mismatch but is still NEG: one road plane creates rectangular pasted blocks around vertical structure. Moving/changing the source seam and one-plane replacement are now near exhausted. Risk-gated local Y repair remains the stable POS optional polish.)
