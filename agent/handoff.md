...
**Updated**: 2026-05-28 (Latest: region-coherent seam v3a/v3b and DiT-as-oracle source selection were tested after DiT360 v12/v13. They do not beat L1 hard_select. Moving/changing the source seam is now near exhausted: safe variants revert toward hard_select, aggressive variants create source-swap blocks or DiT-like geometry drift. Risk-gated local Y repair remains the stable POS optional polish.)
...
**6 layers of "no-depth / no-DL" seam fixes tested plus the DiT360 generative seam-completion path pushed through fixed masks, adaptive masks, post/soft/evidence compose, multi-seed, fidelity-budget compose, low-frequency-only harmonization, Poisson/gradient-domain gated compose, and DiT-as-oracle source selection. Seam-local alignment is safer than full OF, and DP seam-routing is a clean hard-select seam ablation, but neither beats L1 hard_select. Region-coherent/component-only seam repair is safer than DP routing but mostly reverts to hard_select. DiT360 can fill masked seam strips, but the visually smoother variants rely on nontrivial source rewriting; using DiT only as a source-selection oracle still creates blocky source swaps. New source-evidence seam confidence maps are POS as diagnostics, and risk-gated local Y repair is POS as conservative seam-color polish: it reduces luminance seams without claiming to solve impossible geometry.**
