...
>
> DB43 Source-Faithfulness Gate and DB44 EGSR dispatcher are accepted. DB45 Geometry foundation evidence audit is paused on DB45i executor DNS before `/status`/`/exec`: completed DB45 substeps DB45a-i accept only no-go/permission/readiness/diagnostic/contract/blocked evidence, kill VGGT confidence-only RED promotion, and define the residual extractor contract, but still accept no target-surface geometry evidence, no metric ego truth, no source-faithful repair permission, no permission-state change, and no RED promotion. DB45i may be rerun only as the same bounded `--run-remote` extractor after a reachable executor endpoint is available: it must save pose tensors/decoded extrinsics, align VGGT camera centers to the Waymo rig by Sim(3), and compare target-surface samples to LiDAR/raw residuals before any geometry claim. DB47 source/frame candidate mining is now running as the CPU/local sidestep route; DB47a accepted only `source-selection-inventory-only` evidence over existing DB27/DB28/DB31/DB34/DB38/DB42/DB43 artifacts. It records 36 existing candidate records and confirms DB28/DB32 are source-sidestep/current-handoff evidence only, not original-G seam repair and not a source-faithful ceiling. DB25/DB41 remain `RED/abstain`; DB41 lower-right remains zero-LiDAR abstain; DB36/DB40 remain generated fake-geometry rejects.
>
> Next DB45 work must wait for a reachable executor or a fresh bounded evidence brief; do not continue DB45i patch-on-patch. Next DB47 work, if opened, must define a fixed candidate universe before scanning and must report total scanned, strict/relaxed accepted, reject-by-reason, abstain distribution, and both wins/failures. Do not reopen prompt-only DiT/FLUX ground/curb/lane/right-line repair, and do not continue any direction after kill criteria without archiving it in `progress.md`.
