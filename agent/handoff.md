...
> ## ⭐⭐⭐⭐⭐⭐ 2026-06-09 EVENING — FABLE 5 SESSION RESULT (supersedes all below): the virtual centre was the wall; DB-80/81/82 all POS; new best base = `cen_depth + B1`
> **Read `agent/2026-06-09-fable5-firstprinciples-analysis.md` + the three 2026-06-09 progress.md entries FIRST.** The first-principles audit found the ERP virtual centre had been pinned to the **ego origin** (1.8–2.2 m from the cameras, mostly height) since L1, and **every** depth-aware "wall" number inherited that amplification (`err ∝ b_perp`; ring-centroid b_perp ≈ 0.12 m vs ego ≈ 1.7 m).
> **DB-80 (POS, 5 AV2 scenes, CPU):** same depth, same protocol, centre moved to the ring centroid → depth-aware render-back p90 falls **18–96×** (84→4.7, 147→5.8, 38.8→0.4, 11.7→0.1, 67.9→1.6 cam px); depth tolerance relaxes ~20× (~98 % of near field tolerates >1 m depth error). The one fired kill bucket (BMW curbwall 64 px) is 100 % occlusion-aliased far-layer test points (protocol artifact, also inflates DB-79's numbers); visible-surface pairs = 0.59 px. Vision: yellow lane lines continuous across former seams, no new artifacts. **DB-79's "depth cannot reopen the seam" is RE-SCOPED — it was a property of the wrong centre.**
> **DB-81 (P1 POS / P2 honest-NEG):** LiDAR-correspondence per-camera colour gains cut cross-camera colour steps **58–88 %** on 4/5 scenes (dusk highway: collage → unified; no tone drift). Radial-CA search returned k≈0 → AV2 images have NO lateral CA; the purple fringe is **source-data ISP shadow chroma** (present in the native ring_side_right image; not CA, not JPEG, not our pipeline).
> **DB-82 (POS):** 5 logs × 3 anchors × 3 variants — structurally identical stats, no occlusion leaks / moving-object shred; **no-LiDAR ablation (plane-only depth) ≈ full LiDAR at panorama scale** → the north-star graceful-degradation claim now has an on-disk A/B. Known limit: B1 gains amplify clipped-sky cyan on dusk scenes (saturated pixels violate the multiplicative model; sky belongs to outpaint anyway).
> **Decision board for the user:** `deliverables/fable5_2026_06_09_summary/OLD_vs_NEW_5scenes.jpg`. **Next queue:** B1-P3 saturation-aware tone (optional), DB-78 thin-band flow on the new base, sky-outpaint integration (needs FLUX env — likely A100), multi-anchor → Waymo DATA step, 3DGS re-eval at ≤0.3 m extrapolation, **confirm with Xinhan: point-cloud-video first-frame centre = ring centroid at camera height**. Commits b8f577d → 3f9e1fc.
>
> ## ⭐⭐⭐⭐⭐ 2026-06-09 READ FIRST — FABLE 5 FIRST-PRINCIPLES HANDOFF + DB-79 DONE + downstream = COSMOS (superseded by the entry above)
> **A fresh first-principles pass is being handed to Fable 5.** Read `agent/2026-06-09-fable5-firstprinciples-brief.md` FIRST — it frames our conclusions as *challengeable hypotheses* (deliberately, to avoid anchoring) and gives the full arc + mandate.
> **DB-79 COMPLETE (leader-audited, verdict CONFIRMED on disk + by eye + parallax math):** the old "12 m surface wall" was a measurement artifact — fair LiDAR-only layered hold-out recovers **surface depth to 3.8–7.5 cm**; BUT depth-aware render-back to the ~1.5 m off-trajectory virtual centre still gives **55–88 px at curb/wall** ⇒ **depth reopens DENSIFICATION on surfaces, NOT the SEAM**; near-field curb/wall abstain stays the honest ceiling. Surfaces being cm-clean is a POSITIVE for any B-layer refiner. (`deliverables/db79_fair_metric_wall/`, progress.md top.)
> **Downstream (provisional — don't over-commit):** the fundamental consumer is **Bosch's world-model needs**; *currently* that looks like a **Cosmos-style 360 video diffusion pipeline** (via Xinhan, `cosmos-transfer2.5-pcd`; first-frame + point-cloud + text). Treat the specific consumer as not-yet-fixed — but it already hints the A/B fork may matter less than "produce a first-frame the world-model can use". **The CORE invariant is unchanged: the perspective→panorama algorithm.** See brief Part 3.
> **Compute:** a live **L4 GPU (~22.5 GB free)** is available — **user authorizes direct use of the runtime JSON** (no per-run permission, no secret ceremony); only hygiene = keep the raw token out of git-committed files. **Subagents:** only for clerical doc work, NOT for thinking (token-control). One active brief; brief-before-experiment; vision mandatory.
>
> ## ⭐⭐⭐⭐ 2026-06-06 — DEEP RETROSPECTIVE + DIRECTION RESET
> Read **`agent/2026-06-06-deep-retrospective.md`** FIRST (19-agent workflow `wf_789ffbb7-1a7`, every claim Read-verified at HEAD `87cc16b`). It RESOLVES the contradiction the banners below created (source-faithful LOCK vs "set aside source-faithful" vs DB-78 re-adopt) — that oscillation was the **verified root cause of the churn**.
> **User decision (2026-06-06): LAYER BOTH.** A = source-faithful multi-center mosaic + provenance/risk/abstain = the **canonical training-safe FLOOR** (current mosaic + DB-78 flow + sky-outpaint ≈ done under A). B = look-good seam-dissolve = a **SEPARATE, labeled presentation layer** on top (`generated_mask`, `loss_weight≈0`), **never mixed into training truth**; salient-geometry hallucination stays BANNED even under look-good (DB36/DB40 + World-in-World).
> **Key correction:** the "3 geometry walls" are **~1.5 + 1 mislabeled** — DB-77B p90 15.4/12.8m is partly an NN-fill metric artifact (scored across occlusion edges); DB-76a false-GREEN 0.373/0.223 is rotation-only (no-depth baseline); only **EXP-B UniDepth** is a clean confirm → wall is REAL at occlusion SILHOUETTES (Lemma A) but OVER-credited on smooth SURFACES. We may have partly **mis-killed the depth route on surfaces on a confounded number**.
> **Leader self-correction:** "go 3DGS, seam dissolves" was TOO OPTIMISTIC — per-scene AND feed-forward GS COLLAPSE at the off-trajectory virtual-centre (same Lemma-A wall from the render side; ExtraGS/EUVS/ConFixGS). 3DGS's only unique gift = canonical DYNAMIC-ACTOR render (kills the moving-object ghost). Caught a false keystone: NeuRAD's AV2 loader uses 7 ring ONLY, drops the stereo pair.
> **NEXT = DB-79** (active brief in `decision_briefs.md`): fair-metric wall settlement (layered/edge-aware hold-out depth + camera-native z-buffer + depth-aware LOO), **measurement-only, CPU/L4, NO A100 (user said hold it)** — kill-or-cleanly-reopen the depth route on surfaces. Parallel (no compute): resolve the 5 Bosch format questions + B-consumer (world-model vs demo) + is OFFLINE per-log reconstruction an acceptable general deliverable (user: "not sure", defer to Bosch). One active brief; secrets non-repo/env only, scan 0; route remote results to a non-repo file + Read-verify (fabrication caveat).
>
> ## ⭐⭐⭐ 2026-06-06 LATEST (AUTONOMOUS run — see `progress.md` top for live state)
> User set an autonomous goal (explore the plausible-renderer line to its limit; use opus subagents to avoid local optima; A100-40GB live). Chain: DB-76a (source-faithful single-centre = physical wall, 4 batteries) → DB-77B (hand-built IBR tears; densify p90 15m at edges, renderer bugs NOT the cause) → **DB-77C** Phase 0 base = **A1_view_none** (hard_select mosaic + sky/out-of-FOV outpaint=generated); Phase 1 Poisson safe-seam harmonize gate-passes but FAINT (safe band 1.6%). **opus strategy audit reframed the problem:** we fought in the GEOMETRY domain but the task is IMAGE-SEMANTIC (PLAUSIBLE + no hallucination); the ONLY untested path = a STRONG-CONDITIONED single-step refiner on the aligned mosaic (vs DiT360 pure-generative = invents cars). Running **EXP-A** (Difix `nvidia/difix` zero-shot seam-band refine + YOLO object-gate, A100) and **EXP-B** (UniDepthV2 dense metric depth → re-feed IBR; hold-out residual p90 12-15m→<2m? = the decisive geometry-wall test). End deliverable = **3-tier graceful degradation** (verifiable→Poisson / single-source-misalign→learned-refine / no-geometry→abstain) + object gate + provenance. One active brief; A100 secret non-repo only, scan 0.
>
> ## ⭐⭐ 2026-06-06 — DB-76a COMPLETE (all 4 batteries) → DB-77B/C
