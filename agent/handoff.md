...
> **NEW downstream reality (Xinhan, 2026-06-09):** our panorama is the **first-frame conditioning input to a Cosmos-style 360 video diffusion model** (`cosmos-transfer2.5-pcd`) that also ingests a point-cloud video + text. This may collapse the A/B fork — the right target might be "Cosmos-conditioning-optimal" (honest partial-360 + masked holes), not "perfect 2D seam". See the brief Part 3.
> **Compute:** a live **L4 GPU (~22.5 GB free)** is available; endpoint is a SECRET (non-repo/env only — NEVER in any repo file). **Subagents:** only for clerical doc work, NOT for thinking (token-control, user constraint). One active brief; brief-before-experiment; vision mandatory.
>
