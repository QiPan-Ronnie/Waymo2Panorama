...
>
> DB43 Source-Faithfulness Gate and DB44 EGSR dispatcher are accepted. DB45 Geometry foundation evidence audit is still running. Completed DB45 substeps: DB45a current-runtime VGGT no-go; DB45b permission calibration; DB45c HF Commercial access/schema gate; DB45d official VGGT setup/load smoke; DB45e official VGGT frozen-ROI confidence probe. DB45e accepts only `vggt-roi-confidence-diagnostic-only`: official VGGT ran once on BMW anchor 0 and produced real `depth_conf` / `world_points_conf`, but the current evidence pack has camera-owner summaries rather than pixel-exact target-surface mapping, so DB25/DB41 remain `RED/abstain`, DB41 lower-right remains zero-LiDAR abstain, DB36/DB40 remain generated fake-geometry rejects, and no RED promotion occurred.
>
> Next source-faithful DB45 work must open a fresh bounded brief and bring true target-surface evidence (pixel/track/pointmap mapping, multi-view consistency, or equivalent). Owner-camera VGGT confidence alone is now known insufficient. Do not reopen prompt-only DiT/FLUX ground/curb/lane/right-line repair, and do not continue any direction after kill criteria without archiving it in `progress.md`.
