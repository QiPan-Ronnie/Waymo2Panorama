...
**Updated**: 2026-05-28 (Latest: dense-depth-aware DP seam routing was tested after dense Depth Anything V2 metadata, LiDAR depth-visibility, sparse stereo v5, semantic object-coherent hard_select, same-frame/temporal ground-plane replacement, DiT360 v14, region-coherent seam v3a/v3b, and DiT-as-oracle source selection. It is NEG: adding depth-edge risk to seam routing lowers seam dY but collapses source-fidelity NCC. Depth remains useful as seam-risk metadata, not as a local seam-path solver. Risk-gated local Y repair remains the stable POS optional polish.)
